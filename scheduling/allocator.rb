...
module Scheduling::Allocator
  ...
  class Allocation
    ...
    def self.candidate_hosts(request)
      ...
      # Skip boot_image join when booting from a machine image (boot_image will be nil or empty)
      if request.boot_image && !request.boot_image.empty?
    ...
    def create_storage_volumes(vm, vm_host)
      @request.storage_volumes.each do |disk_index, volume|
        ...
        image_id = if machine_image_id
          nil # No boot_image needed when using a machine image
        elsif volume["boot"]
