...
class CloverAdmin < Roda
  ...
  plugin :autoforme do
    ...
    model PostgresServerExtension do
      ...
      columns do |type_symbol, request|
        cs = [:postgres_server, :name, :state, :target_version, :installed_version, :last_transition_at, :last_error]
        cs.prepend(:ubid) unless type_symbol == :search_form
        cs
      end
      column_options postgres_server: ubid_input.call("PostgresServer"),
        state: {type: "select", options: %w[install_pending installing sync_pending config_pending restart_pending verifying ready failed], add_blank: true},
