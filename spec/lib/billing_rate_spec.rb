...
RSpec.describe BillingRate do
  ...
  describe ".line_item_description" do
    it "returns for VmCpu" do
      expect(described_class.line_item_description("VmCpu", "standard", 4)).to eq("standard-8 Virtual Machine")
    end
  ...
  describe ".line_item_usage" do
    it "returns usage by duration" do
      expect(described_class.line_item_usage("VmCpu", "standard", 5, 1)).to eq("1 minutes")
    end
