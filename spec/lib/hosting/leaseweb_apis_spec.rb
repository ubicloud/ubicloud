...
RSpec.describe Hosting::LeasewebApis do
  ...
  def ip_info(ip_address, gateway, reserved: [])
    described_class::IpInfo.new(ip_address:, source_host_ip: "216.22.50.197", gateway:, reserved:)
  end
  ...
  describe "pull_ips" do
    it "collapses a routed block, keeps the main ip and both ipv6 prefixes, and pages" do
      ...
      expect(leaseweb_apis.pull_ips).to eq [
        ...
        ip_info("216.22.15.64/26", nil, reserved: ["216.22.15.64", "216.22.15.127"]),
      ]
    end

    # Server 91478 delivers its extra IPv4s as a switched /29: the infra rows are
    # typed, and every row carries the segment's gateway. The segment collapses
    # into one block like a routed one, reserving the addresses its router owns.
    it "collapses a switched segment into one block that reserves its typed infra rows" do
      stub_ips([segment_ip_rows])

      expect(leaseweb_apis.pull_ips.map { [it.ip_address, it.gateway, it.reserved] }).to eq [
        ["23.105.171.112/32", "23.105.171.126", []],
        ["2607:f5b7:1:30:9::/112", "2607:f5b7:1:30::1", []],
        ["23.105.176.0/29", "23.105.176.6", ["23.105.176.0", "23.105.176.4", "23.105.176.5", "23.105.176.6", "23.105.176.7"]],
      ]
    end

    # Only the main IP is the host's own. A switched segment and a routed block
    # are both what VMs draw from, and IPv6 never populates that pool.
    it "marks only the main ip host only" do
      ...
      expect(leaseweb_apis.pull_ips.map { [it.ip_address, it.host_only?] }).to eq [
        ...
        ["23.105.176.0/29", false],
        ["216.22.15.64/26", false],
      ]
    end

    # A /32 or /31 Leaseweb routes here is a standalone address, not a block:
    # reserving a network and a broadcast address would leave nothing behind.
    it "reserves nothing of a standalone address or a two address block" do
      stub_ips([[
        ip_row("216.22.50.197/26", prefix_length: 26, main_ip: true, gateway: "216.22.50.254"),
        ip_row("5.6.7.8/32", prefix_length: 32),
        ip_row("5.6.7.10/31", prefix_length: 31),
        ip_row("5.6.7.11/31", prefix_length: 31),
      ]])

      expect(leaseweb_apis.pull_ips.map { [it.ip_address, it.reserved] }).to eq [
        ["216.22.50.197/32", []],
        ["5.6.7.8/32", []],
        ["5.6.7.10/31", []],
      ]
