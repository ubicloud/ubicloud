  ...
  describe "#can use generic otel_batcher" do
    ...
    describe "#send_batch" do
      ...
      it "exits when failures exceed the allowed limit" do
        expect(connection).to receive(:post).exactly(5).times.and_return(instance_double(Excon::Response, status: 400, reason_phrase: "Bad Request"))
        expect(otel_batcher).to receive(:puts).with("Failed to send logs: 400 Bad Request").exactly(5).times
        expect(otel_batcher).to receive(:puts).with("Too many failures sending logs, stopping the batcher.").once
