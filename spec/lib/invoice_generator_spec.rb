...
RSpec.describe InvoiceGenerator do
  def generate_billing_record(project, resource, span)
    ...
    when Vm
      vm = resource
      billing_rate_id = BillingRate.from_resource_properties("VmCpu", vm.family, vm.location)["id"]
      amount = vm.cores
      name = vm.name
    when GithubRunner
      gr = resource
  ...
  def check_invoice_for_single_vm(invoices, project, vm, duration)
    ...
    br = BillingRate.from_resource_properties("VmCpu", vm.family, vm.location)
    duration_mins = (duration / 60).ceil
  ...
  it "handles project and github runner credits together" do
    ...
    expect(before["cost"]).to eq((before["subtotal"] - 1).round(3))
