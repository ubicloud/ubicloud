...
RSpec.describe MetricsTargetResource do
  ...
  def create_victoria_metrics_setup(project)
    ps = PrivateSubnet.create(name: "test-ps", project_id: project.id, location_id: Location::HETZNER_FSN1_ID,
    ...
    vmr = VictoriaMetricsResource.create(project_id: project.id, location_id: Location::HETZNER_FSN1_ID,
      name: "test-vmr", admin_user: "admin", admin_password: "pass", target_vm_size: "standard-2",
      target_storage_size_gib: 100, root_cert_1: "cert")
    VictoriaMetricsServer.create(victoria_metrics_resource_id: vmr.id, vm_id: vm.id, cert: "cert", cert_key: "key")
    vmr
  end
  ...
  describe "#initialize" do
    ...
    it "initializes with a resource and nil tsdb client when VictoriaMetrics is not found" do
      ...
      expect(resource.resource).to eq(postgres_server)
