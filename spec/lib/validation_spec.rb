...
RSpec.describe Validation do
  describe "#validate_name" do
    ...
    describe "#validate_network_volume_config" do
      ...
      it "bounds size by the volume type's limits" do
        expect { described_class.validate_network_volume_config("gp3", 65_537, nil, nil) }
          .to raise_error(described_class::ValidationFailed) { |e|
            expect(e.details[:network_volume_size_gib]).to eq("Size for gp3 must be between 1 and 65536 GiB.")
          }
        expect { described_class.validate_network_volume_config("gp3", 1, nil, nil) }.not_to raise_error
        expect { described_class.validate_network_volume_config("gp3", 65_536, nil, nil) }.not_to raise_error
        expect { described_class.validate_network_volume_config("gp3", 0, nil, nil) }.to raise_error described_class::ValidationFailed
        expect { described_class.validate_network_volume_config("io2", 3, nil, nil) }.to raise_error described_class::ValidationFailed
        expect { described_class.validate_network_volume_config("io2", 4, nil, nil) }.not_to raise_error
        expect { described_class.validate_network_volume_config("hyperdisk-balanced", 5, nil, nil) }.to raise_error described_class::ValidationFailed
        expect { described_class.validate_network_volume_config("hyperdisk-balanced", 6, nil, nil) }.not_to raise_error
      end

      it "rejects a missing size, which the storage volume path can reach" do
        expect { described_class.validate_network_volume_config("gp3", nil, nil, nil) }.to raise_error described_class::ValidationFailed
      end
