...
module AdminModelSpecHelper
  if ENV["UNUSED_ASSOCIATIONS"]
    ...
    def create_trusted_jwt_issuer
      account = create_account
      project = account.projects.first
      TrustedJwtIssuer.create(project_id: project.id, account_id: account.id, name: "test-issuer", issuer: "https://auth.example.com", jwks_uri: "https://auth.example.com/.well-known/jwks.json")
    end
