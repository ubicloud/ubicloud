  ...
  describe "authenticated" do
    ...
    describe "config" do
      ...
      it "can update configuration" do
        ...
        # Read csrf token from hidden input field
        csrf_token = find(".pg-config input[name='_csrf']", visible: false).value

        page.driver.submit :post, "#{project.path}#{pg.path}/config",
          {
            "pg_config_keys[]" => "max_connections",
            "pg_config_values[]" => "240",
            "_csrf" => csrf_token
          }
      ...
      it "shows errors when an unknown configuration is provided" do
        ...
        page.driver.submit :post, "#{project.path}#{pg.path}/config",
          {
            "pg_config_keys[]" => "invalid",
            "pg_config_values[]" => "invalid",
            "_csrf" => csrf_token
          }

        expect(page).to have_content "Unknown configuration parameter"
        expect(page).to have_flash_error "Validation failed for following fields: invalid"
      end
      ...
      it "shows errors when an invalid configuration is provided" do
        ...
        page.driver.submit :post, "#{project.path}#{pg.path}/config",
          {
            "pg_config_keys[]" => "work_mem",
            "pg_config_values[]" => "16iB",
            "_csrf" => csrf_token
          }

        expect(page).to have_flash_error "Validation failed for following fields: work_mem"
