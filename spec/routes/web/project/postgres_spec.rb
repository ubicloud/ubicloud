...
RSpec.describe Clover, "postgres" do
  ...
  describe "authenticated" do
    ...
    describe "create" do
      ...
      it "resolves the default WAL drive card to the server-side default" do
        project.set_ff_private_locations(true)
        project.set_ff_postgres_network_cache_storage(true)
        create_private_location(project:)
        ...
        # The form checks the first available radio, so the sentinel leads and
        # names the type the server would pick. It is form-only: the API enum
        # does not accept it, callers there simply omit the field.
        expect(page).to have_content "Default (General Purpose SSD (gp3))"
      end
