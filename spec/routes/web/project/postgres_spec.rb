  ...
  describe "authenticated" do
    ...
    describe "upgrade" do
      ...
      it "starts the upgrade when user clicks on start upgrade button" do
        ...
        expect(pg.reload.version_int).to eq(old_version + 1)
      end
      ...
      it "shows upgrade progress when upgrade is in progress" do
        ...
        pg.strand.children_dataset.where(prog: "Postgres::UpgradePostgresResource").first&.update(label: "wait_for_maintenance_window")
      ...
      it "shows database unavailability when upgrade is in progress" do
        ...
        pg.strand.children_dataset.where(prog: "Postgres::UpgradePostgresResource").first&.update(label: "upgrade_standby")
      ...
      it "shows failed upgrade indication when upgrade fails" do
        ...
        pg.strand.children_dataset.where(prog: "Postgres::UpgradePostgresResource").first&.update(label: "upgrade_failed")
      ...
      it "shows latest version message if the database is already on the latest version" do
        pg.strand.update(label: "wait")
        pg.update(version: PostgresResource::LATEST_VERSION)
