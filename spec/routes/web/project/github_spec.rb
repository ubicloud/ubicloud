...
RSpec.describe Clover, "github" do
  ...
  describe "custom-label" do
    ...
    it "shows custom labels section on settings page" do
      GithubCustomLabel.create(installation_id: installation.id, name: "my-label", alias_for: "ubicloud-standard-4")
      GithubCustomLabel.create(installation_id: installation.id, name: "big-runner", alias_for: "ubicloud-standard-16", concurrent_runner_count_limit: 5)
      ...
      expect(page).to have_content "ubicloud-standard-4"
      expect(page).to have_content "ubicloud-standard-16"
    ...
    it "shows empty state when no custom labels" do
      ...
      expect(page).to have_content "Create custom runner labels to define friendly aliases for your team."
    ...
    it "fails to create with validation error" do
      GithubCustomLabel.create(installation_id: installation.id, name: "existing-label", alias_for: "ubicloud-standard-4")
    ...
    it "renders edit form with existing values" do
      ...
      expect(page).to have_select "alias_for", selected: "ubicloud-standard-4"
      expect(page).to have_field "concurrent_runner_count_limit", with: "5"
    end

    it "updates a custom label" do
      label = GithubCustomLabel.create(installation_id: installation.id, name: "my-label", alias_for: "ubicloud-standard-4")

      visit "#{project.path}/github/#{installation.ubid}/custom-label/#{label.ubid}"

      fill_in "name", with: "updated-label"
      select "ubicloud-standard-16", from: "alias_for"
      ...
      expect(label.alias_for).to eq("ubicloud-standard-16")
