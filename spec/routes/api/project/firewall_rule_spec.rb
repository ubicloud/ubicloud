...
RSpec.describe Clover, "firewall" do
  ...
  describe "unauthenticated" do
    it "not post" do
      post "/project/#{project.ubid}/location/#{TEST_LOCATION}/firewall/id/#{firewall.ubid}/firewall-rule"

      expect(last_response).to have_api_error(401, "Please login to continue")
    end

    it "not delete" do
      delete "/project/#{project.ubid}/location/#{TEST_LOCATION}/firewall/id/#{firewall.ubid}/firewall-rule/#{firewall_rule.ubid}"

      expect(last_response).to have_api_error(401, "Please login to continue")
    end

    it "not get" do
      get "/project/#{project.ubid}/location/#{TEST_LOCATION}/firewall/id/#{firewall.ubid}/firewall-rule/#{firewall_rule.ubid}"
  ...
  describe "authenticated" do
    ...
    it "firewall rule delete does not exist" do
      delete "/project/#{project.ubid}/location/#{TEST_LOCATION}/firewall/id/#{firewall.ubid}/firewall-rule/fooubid"
      expect(last_response.status).to eq(204)
    end
    ...
    it "get does not exist" do
      get "/project/#{project.ubid}/location/#{TEST_LOCATION}/firewall/id/#{firewall.ubid}/firewall-rule/fooubid"
