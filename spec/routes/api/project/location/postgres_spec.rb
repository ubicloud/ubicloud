...
RSpec.describe Clover, "postgres" do
  ...
  describe "authenticated" do
    ...
    describe "upgrade" do
      ...
      it "success post" do
        old_pg_version = pg.version_int
        post "/project/#{project.ubid}/location/#{pg.display_location}/postgres/#{pg.name}/upgrade"

        expect(last_response.status).to eq(200)
        response = JSON.parse(last_response.body)
        expect(response["upgrade_status"]).to eq("running")
        expect(response["current_version"]).to eq(pg.current_version)
        expect(response["desired_version"]).to eq(pg.reload.version)
        expect(pg.reload.version_int).to eq(old_pg_version + 1)
      end
      ...
      it "failed post" do
        ...
        expect(last_response.status).to eq(400)
        expect(pg.reload.version_int).to eq(old_pg_version)
      end
