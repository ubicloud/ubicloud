  ...
  describe "authenticated" do
    ...
    describe "metrics" do
      ...
      it "fails when end timestamp is before start timestamp" do
        get "/project/#{project.ubid}/location/#{pg.display_location}/postgres/#{pg.name}/metrics?start=1619719600&end=1619712000"
        expect(last_response.status).to eq(400)
      ...
      it "fails when time range is too large" do
        start_time = Time.now.utc.to_i - 32 * 24 * 60 * 60
        end_time = Time.now.utc.to_i
        get "/project/#{project.ubid}/location/#{pg.display_location}/postgres/#{pg.name}/metrics?start=#{start_time}&end=#{end_time}"
        expect(last_response.status).to eq(400)
