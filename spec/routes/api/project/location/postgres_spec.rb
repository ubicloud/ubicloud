  ...
  describe "authenticated" do
    ...
    describe "config" do
      it "read" do
        ...
        expect(response_body["computed_config"]).to include("shared_buffers", "work_mem", "max_connections", "effective_cache_size")
      end

      it "read skips computed_config when resource has no representative_server" do
        ...
        expect(response_body["pg_config"]).to eq({"max_connections" => "100"})
        expect(response_body["pgbouncer_config"]).to eq({"max_client_conn" => "100"})
        expect(response_body["computed_config"]).to be_nil
      end
