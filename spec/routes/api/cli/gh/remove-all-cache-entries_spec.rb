...
RSpec.describe Clover, "cli gh remove-all-cache-entries" do
  it "removes all cache entries" do
    ...
    ge1 = GithubCacheEntry.create_with_id("967f7e02-68f8-8a0e-9917-fd13d5f33501", repository_id: gp.id, key: "test-key-1", version: "test-version", scope: "test-scope", size: 10987654321, created_by: gp.id)
    ge2 = GithubCacheEntry.create_with_id("967f7e02-68f8-8a0e-9917-fd13d5f33502", repository_id: gp.id, key: "test-key-2", version: "test-version", scope: "test-scope", size: 10987654321, created_by: gp.id)
    ge3 = GithubCacheEntry.create_with_id("967f7e02-68f8-8a0e-9917-fd13d5f33503", repository_id: gp.id, key: "test-key-3", version: "test-version", scope: "test-scope", size: 10987654321, created_by: gp.id)
    ...
    expect(client).to receive(:abort_multipart_upload).with(bucket: gp.bucket_name, key: ge1.blob_key, upload_id: nil)
    expect(client).to receive(:delete_object).with(bucket: gp.bucket_name, key: ge2.blob_key)
    expect(client).to receive(:abort_multipart_upload).with(bucket: gp.bucket_name, key: ge2.blob_key, upload_id: nil)
