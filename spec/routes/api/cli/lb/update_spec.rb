...
RSpec.describe Clover, "cli lb update" do
  ...
  it "updates the load balancer" do
    ...
    expect(cli(%W[lb eu-central-h1/test-lb update hash_based 1234 5432 /up2 true #{@vm1.ubid}])).to eq "Updated load balancer with id #{@lb.ubid}\n"
    @lb.reload
  ...
  it "adds new VMs the load balancer by id" do
    expect(@lb.vms.map(&:ubid)).to eq [@vm1.ubid]
    expect(cli(%W[lb eu-central-h1/test-lb update hash_based 1234 5432 /up2 false #{@vm1.ubid} #{@vm2.ubid}])).to eq "Updated load balancer with id #{@lb.ubid}\n"
    @lb.reload
    expect(@lb.ports.first.src_port).to eq 1234
    expect(@lb.ports.first.dst_port).to eq 5432
    expect(@lb.algorithm).to eq "hash_based"
    ...
    expect(@lb.cert_enabled).to be false
  end

  it "adds new VMs the load balancer by name" do
    expect(@lb.vms.map(&:ubid)).to eq [@vm1.ubid]
    expect(cli(%W[lb eu-central-h1/test-lb update hash_based 1234 5432 /up2 true test-vm test-vm2])).to eq "Updated load balancer with id #{@lb.ubid}\n"
    @lb.reload
    expect(@lb.ports.first.src_port).to eq 1234
    expect(@lb.ports.first.dst_port).to eq 5432
    expect(@lb.algorithm).to eq "hash_based"
  ...
  it "removes VMs not given from the load balancer" do
    expect(@lb.vms.map(&:ubid)).to eq [@vm1.ubid]
    expect(cli(%W[lb eu-central-h1/test-lb update hash_based 1234 5432 /up2 false])).to eq "Updated load balancer with id #{@lb.ubid}\n"
    @lb.reload
    expect(@lb.ports.first.src_port).to eq 1234
    expect(@lb.ports.first.dst_port).to eq 5432
    expect(@lb.algorithm).to eq "hash_based"
