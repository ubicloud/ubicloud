# frozen_string_literal: true

require_relative "../spec_helper"

RSpec.describe Clover, "cli mi destroy" do
  before do
    @mi = MachineImage.create(
      name: "test-mi",
      description: "test desc",
      project_id: @project.id,
      location_id: Location::HETZNER_FSN1_ID,
      state: "available",
      s3_bucket: "test-bucket",
      s3_prefix: "images/test/",
      s3_endpoint: "https://r2.example.com",
      size_gib: 20
    )
    Strand.create(id: @mi.id, prog: "MachineImage::Nexus", label: "wait", stack: [{"subject_id" => @mi.id}])
  end

  it "destroys mi directly if -f option is given" do
    expect(cli(%w[mi eu-central-h1/test-mi destroy -f])).to eq "Machine image, if it exists, is now scheduled for destruction\n"
  end

  it "asks for confirmation if -f option is not given" do
    expect(cli(%w[mi eu-central-h1/test-mi destroy], confirm_prompt: "Confirmation")).to eq <<~END
      Destroying this machine image is not recoverable.
      Enter the following to confirm destruction of the machine image: #{@mi.name}
    END
  end

  it "works on correct confirmation" do
    expect(cli(%w[--confirm test-mi mi eu-central-h1/test-mi destroy])).to eq "Machine image, if it exists, is now scheduled for destruction\n"
  end

  it "fails on incorrect confirmation" do
    expect(cli(%w[--confirm foo mi eu-central-h1/test-mi destroy], status: 400)).to eq "! Confirmation of machine image name not successful.\n"
  end
end
