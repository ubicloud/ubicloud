...
RSpec.describe Clover, "cli mi destroy-version" do
  ...
  it "asks for confirmation when -f is not given" do
    body = cli(%W[mi eu-central-h1/#{@mi.name} destroy-version v2], confirm_prompt: "Confirmation")
  ...
  it "fails on incorrect confirmation" do
    body = cli(%W[--confirm wrong mi eu-central-h1/#{@mi.name} destroy-version v2], status: 400)
    expect(body).to eq "! Confirmation of machine image version label not successful.\n"
  end

  it "fails if version argument is missing" do
    expect(cli(%W[mi eu-central-h1/#{@mi.name} destroy-version], status: 400)).to start_with("! Invalid number of arguments for mi destroy-version subcommand")
  end
end
