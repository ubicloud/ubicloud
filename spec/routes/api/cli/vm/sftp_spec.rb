...
RSpec.describe Clover, "cli vm sftp" do
  ...
  it "provides headers to connect to vm via sftp" do
    expect(cli_exec(["vm", "sftp", @vm.display_location, @vm.name], tail: false)).to eq %w[sftp ubi@[128:1234::2]]
  end

  it "IPv4 address is used by default if available" do
    add_ipv4_to_vm(@vm, "128.0.0.1")
    expect(cli_exec(["vm", "sftp", @vm.display_location, @vm.name], tail: false)).to eq %w[sftp ubi@128.0.0.1]
  end
end
