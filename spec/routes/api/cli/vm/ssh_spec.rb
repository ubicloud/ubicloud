...
RSpec.describe Clover, "cli vm ssh" do
  ...
  it "provides headers to connect to vm" do
    expect(cli_exec(["vm", "ssh", @vm.display_location, @vm.name])).to eq %w[ssh ubi@128:1234::2 0]
  end

  it "IPv4 address is used by default if available" do
    add_ipv4_to_vm(@vm, "128.0.0.1")
    expect(cli_exec(["vm", "ssh", @vm.display_location, @vm.name])).to eq %w[ssh ubi@128.0.0.1 0]
  end

  it "uses IPv4 address if available and connection is made via IPv4" do
    add_ipv4_to_vm(@vm, "128.0.0.1")
    @socket = UDPSocket.new(Socket::AF_INET)
    expect(cli_exec(["vm", "ssh", @vm.display_location, @vm.name], env: {"puma.socket" => @socket})).to eq %w[ssh ubi@128.0.0.1 0]
  end

  it "uses IPv6 address if connection is made via IPv6" do
    add_ipv4_to_vm(@vm, "128.0.0.1")
    @socket = UDPSocket.new(Socket::AF_INET6)
    expect(cli_exec(["vm", "ssh", @vm.display_location, @vm.name], env: {"puma.socket" => @socket})).to eq %w[ssh ubi@128:1234::2 0]
  end
  ...
  it "-4 option uses IPv4 even if connection is made via IPv6" do
    add_ipv4_to_vm(@vm, "128.0.0.1")
    @socket = UDPSocket.new(Socket::AF_INET6)
    expect(cli_exec(["vm", "ssh", "-4", @vm.display_location, @vm.name], env: {"puma.socket" => @socket})).to eq %w[ssh ubi@128.0.0.1 0]
  end

  it "-6 option uses IPv6 even if connection is made via IPv4" do
    add_ipv4_to_vm(@vm, "128.0.0.1")
    @socket = UDPSocket.new(Socket::AF_INET)
    expect(cli_exec(["vm", "ssh", "-6", @vm.display_location, @vm.name], env: {"puma.socket" => @socket})).to eq %w[ssh ubi@128:1234::2 0]
  end

  it "-u option overrides user to connect with" do
    expect(cli_exec(["vm", "ssh", "-ufoo", @vm.display_location, @vm.name])).to eq %w[ssh foo@128:1234::2 0]
  end

  it "returns the number of additional arguments in argv that should be used for the command" do
    expect(cli_exec(["vm", "ssh", @vm.display_location, @vm.name, "id"])).to eq %w[ssh ubi@128:1234::2 1]
  end

  it "handles multiple options" do
    add_ipv4_to_vm(@vm, "128.0.0.1")
    expect(cli_exec(["vm", "ssh", "-6u", "foo", @vm.display_location, @vm.name])).to eq %w[ssh foo@128:1234::2 0]
  end
