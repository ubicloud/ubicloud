...
RSpec.describe Clover, "cli vm scp" do
  ...
  it "provides headers to copy local file to remote" do
    expect(cli_exec(["vm", "scp", @vm.display_location, @vm.name, "local", ":remote"], tail: false, initial: true)).to eq %w[scp ubi@[128:1234::2]:remote 2]
  end

  it "IPv4 address is used by default if available" do
    add_ipv4_to_vm(@vm, "128.0.0.1")
    expect(cli_exec(["vm", "scp", @vm.display_location, @vm.name, "local", ":remote"], tail: false, initial: true)).to eq %w[scp ubi@128.0.0.1:remote 2]
  end

  it "provides headers to copy remote file to local" do
    expect(cli_exec(["vm", "scp", @vm.display_location, @vm.name, ":remote", "local"])).to eq %w[scp ubi@[128:1234::2]:remote 1]
  end
