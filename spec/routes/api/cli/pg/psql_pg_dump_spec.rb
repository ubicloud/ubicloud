...
%w[psql pg_dump].each do |cmd|
  RSpec.describe Clover, "cli pg #{cmd}" do
    ...
    it "puts password in ubi-pgpassword if client is 1.1.0+" do
      expect(cli_exec(["pg", @ref, cmd], env: {"HTTP_X_UBI_VERSION" => "1.1.0"}, command_pgpassword: @pg.superuser_password)).to eq %W[#{cmd} -- postgres://postgres@test-pg.#{@pg.ubid}.pg.example.com:5432/postgres?channel_binding=require]
    end
