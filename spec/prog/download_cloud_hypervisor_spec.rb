...
RSpec.describe Prog::DownloadCloudHypervisor do
  ...
  describe "#start" do
    ...
    it "fails if version is nil" do
      strand.update(stack: [{"version" => nil, "sha256_ch_bin" => "thesha", "sha256_ch_remote" => "anothersha"}])
      expect { df.start }.to raise_error RuntimeError, "Version is required"
    end

    it "fails if sha256 for cloud-hypervisor is nil" do
      strand.update(stack: [{"version" => "35.0", "sha256_ch_bin" => nil, "sha256_ch_remote" => "anothersha"}])
    ...
    it "fails if sha256 for ch-remote is nil" do
      strand.update(stack: [{"version" => "35.0", "sha256_ch_bin" => "thesha", "sha256_ch_remote" => nil}])
