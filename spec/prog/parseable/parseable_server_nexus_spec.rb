...
RSpec.describe Prog::Parseable::ParseableServerNexus do
  ...
  describe "#wait_bootstrap_rhizome" do
    ...
    it "hops to create_parseable_user when done" do
      Strand.create(parent_id: st.id, prog: "BootstrapRhizome", label: "wait", stack: [{}], exitval: {"msg" => "rhizome installed"})
      expect { nx.wait_bootstrap_rhizome }.to hop("create_parseable_user")
    end
  end
  ...
  describe "#configure" do
    it "runs configure_parseable when NotStarted" do
      ...
      expected_config = {
        ...
        blob_storage_url: parseable_resource.blob_storage_endpoint,
        blob_storage_bucket: parseable_resource.bucket_name,
  ...
  describe "#configure_metrics" do
    it "writes the collector config and systemd units, then hops to wait" do
      expect(sshable).to receive(:_cmd).with("mkdir -p /home/ubi/parseable/metrics")
      expect(sshable).to receive(:write_file).with("/home/ubi/parseable/metrics/config.json", anything, user: :current)
      expect(sshable).to receive(:_cmd).with("chmod 600 /home/ubi/parseable/metrics/config.json")
      ...
      expect(sshable).to receive(:_cmd).with("sudo systemctl enable --now parseable-metrics.timer")
  ...
  describe "#wait" do
    ...
    it "clears restart state, calls daemonized_restart and naps when restart semaphore is set" do
      nx.incr_restart
  ...
  describe "#refresh_certificates" do
    ...
    it "creates new cert, updates server, increments reconfigure, and hops to wait" do
      parseable_server.update(cert: "old-cert", cert_key: "old-key", certificate_last_checked_at: Time.now - 60 * 60 * 24 * 31)

      expect { nx.refresh_certificates }.to hop("wait")

      expect(parseable_server.certificate_last_checked_at).to be_within(1).of(Time.now)
      expect(Semaphore.where(strand_id: nx.strand.id, name: "reconfigure").count).to eq(1)

      cert = OpenSSL::X509::Certificate.new(parseable_server.cert)
      expect(cert.verify(OpenSSL::X509::Certificate.new(parseable_resource.root_cert_1).public_key)).to be true
      expect(parseable_server.cert_key).not_to eq("old-key")
    end
  end
  ...
  describe "#unavailable" do
    ...
    it "restarts and naps when server is still down" do
      client = instance_double(Parseable::Client)
      expect(nx.parseable_server).to receive(:client).and_return(client)
      expect(client).to receive(:healthy?).and_return(false)
