...
RSpec.describe Prog::Parseable::ParseableResourceNexus do
  ...
  describe ".assemble" do
    ...
    it "creates a parseable resource with blob_storage_access_key and blob_storage_secret_key" do
      ...
      expect(pr.blob_storage_access_key.length).to eq(32)
      expect(pr.blob_storage_secret_key.length).to eq(64)
    end
  ...
  describe "#configure_blob_storage" do
    ...
    it "sets up MinIO user/policy/bucket and hops to wait_servers when blob storage is ready" do
      ...
      expect(admin_client).to receive(:admin_add_user).with(pr.blob_storage_access_key, pr.blob_storage_secret_key)
  ...
  describe "#wait_servers" do
    it "hops to wait when all servers are in wait state" do
      server = nx.parseable_resource.servers.first
  ...
  describe "#wait" do
    ...
    it "hops to destroy when semaphore is set" do
      nx.incr_destroy
  ...
  describe "#wait_servers_destroyed" do
    ...
    it "destroys the parseable resource when all servers are gone" do
      ...
      nx.parseable_resource.servers.each(&:destroy)
  ...
  describe "#destroy" do
    ...
    it "cleans up MinIO user and policy before destroying" do
      ...
      expect(admin_client).to receive(:admin_remove_user).with(pr.blob_storage_access_key)
