...
RSpec.describe Prog::SetupGrafana do
  ...
  let(:sshable) { Sshable.create_with_id(sshable_id, host: "test.localhost", raw_private_key_1: SshKey.generate.keypair) }
  let(:st) {
    sshable
    Strand.create_with_id(sshable_id, prog: "SetupGrafana", label: "start", stack: [{"subject_id" => sshable_id, "cert_email" => "email@gmail.com", "domain" => "grafana.domain.com"}])
  }
  ...
  describe "#install_grafana" do
    it "starts to install the grafana if its the first time" do
      ...
      expect(sn.sshable).to receive(:d_check).and_return("NotStarted")
      expect(sn.sshable).to receive(:d_run).with("install_grafana", "sudo", "host/bin/setup-grafana", "grafana.domain.com", "email@gmail.com")
      expect { sn.install_grafana }.to nap(10)
    end

    it "cleans up and pops when the installation is done" do
      expect(sn.sshable).to receive(:d_check).and_return("Succeeded")
      expect(sn.sshable).to receive(:d_clean).with("install_grafana")
      expect { sn.install_grafana }.to exit({"msg" => "grafana was setup"})
    end

    it "naps when strand is in the middle of execution" do
      expect(sn.sshable).to receive(:d_check).and_return("InProgress")
      expect { sn.install_grafana }.to nap(10)
    end

    it "naps for a long time when the installation fails" do
      expect(sn.sshable).to receive(:d_check).and_return("Failed")
      expect { sn.install_grafana }.to nap(65536)
    end

    it "naps forever if the daemonizer check returns something unknown" do
      expect(sn.sshable).to receive(:d_check).and_return("Unknown")
      expect { sn.install_grafana }.to nap(65536)
    end
  end
end
