...
RSpec.describe Prog::SetupGrafana do
  ...
  describe "#start" do
    it "checks the initial variables and proceeds to installing rhizome" do
      expect { sn.start }.to hop("install_rhizome")
    end

    it "fails if domain is not provided" do
      expect(sn).to receive(:domain).and_return("")
      expect { sn.start }.to raise_error(RuntimeError)
    end

    it "fails if cert_email is not provided" do
      expect(sn).to receive(:domain).and_return("domain.com")
      expect(sn).to receive(:cert_email).and_return("")
      expect { sn.start }.to raise_error(RuntimeError)
    end
  end
  ...
  describe "#install_rhizome" do
    it "buds a bootstrap rhizome prog and hops to wait_for_rhizome" do
      ...
      expect { sn.install_rhizome }.to hop("wait_for_rhizome")
    end
  end
  ...
  describe "#install_grafana" do
    ...
    it "naps when strand is in the middle of exectuion" do
