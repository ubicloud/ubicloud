...
RSpec.describe Prog::Vnet::CertNexus do
  ...
  describe "#start" do
    ...
    it "registers a deadline and starts the certificate creation process" do
      ...
      expect(OpenSSL::PKey::RSA).to receive(:new).with(4096).and_return("private-key")
      expect(Acme::Client).to receive(:new).with(private_key: "private-key", directory: Config.acme_directory).and_return(client)
      ...
      expect(cert).to receive(:update).with(kid: "test-kid", private_key: "private-key", order_url: "test-order-url")
  ...
  describe "#wait_dns_update" do
    it "waits for dns_record to be seen by all servers" do
      ...
      expect(DnsRecord).to receive(:[]).with(dns_zone_id: dns_zone.id, name: "test-record-name.cert-hostname.", tombstoned: false).and_return(dns_record)
      expect { nx.wait_dns_update }.to nap(10)
    end

    it "requests validation when dns_record is seen by all servers" do
      challenge = instance_double(Acme::Client::Resources::Challenges::DNS01, record_name: "test-record-name")
      expect(nx).to receive(:dns_zone).and_return(dns_zone)
      expect(nx).to receive(:dns_challenge).and_return(challenge).at_least(:once)
      dns_record = DnsRecord.create_with_id(dns_zone_id: dns_zone.id, name: "test-record-name.cert-hostname.", type: "test-record-type", ttl: 600, data: "test-record-content")
  ...
  describe "#wait_dns_validation" do
    let(:challenge) {
      instance_double(Acme::Client::Resources::Challenges::DNS01, status: "pending")
    }

    before do
      expect(nx).to receive(:dns_challenge).and_return(challenge)
    end
    ...
    it "fails if dns_challenge validation fails" do
      expect(challenge).to receive(:status).and_return("failed")
      expect { nx.wait_dns_validation }.to raise_error RuntimeError, "DNS validation failed"
    end

    it "finalizes the certificate when dns_challenge is valid" do
      expect(challenge).to receive(:status).and_return("valid")

      expect(OpenSSL::PKey::RSA).to receive(:new).with(4096).and_return("csr-key")
      ...
      expect(Acme::Client::CertificateRequest).to receive(:new).with(private_key: "csr-key", common_name: "cert-hostname").and_return(csr)
      expect(acme_order).to receive(:finalize).with(csr: csr)
      expect(cert).to receive(:update).with(csr_key: "csr-key")
  ...
  describe "#wait_cert_finalization" do
    ...
    it "fails if certificate finalization fails" do
      expect(acme_order).to receive(:status).and_return("failed")
      expect { nx.wait_cert_finalization }.to raise_error RuntimeError, "Certificate finalization failed"
    end
  ...
  describe "#destroy" do
    it "revokes the certificate and deletes the dns record" do
      ...
      expect(client).to receive(:revoke).with(certificate: "test-cert")
      expect(nx).to receive(:dns_zone).and_return(dns_zone)
      expect(dns_zone).to receive(:delete_record).with(record_name: "test-record-name.cert-hostname")
    ...
    it "fails if revocation fails" do
      client = instance_double(Acme::Client)
      expect(cert).to receive(:cert).and_return("test-cert").at_least(:once)
      expect(nx).to receive(:acme_client).and_return(client)
      expect(client).to receive(:revoke).with(certificate: "test-cert").and_raise(Sequel::Error)

      expect { nx.destroy }.to raise_error RuntimeError, "Failed to revoke certificate"
    end
  ...
  describe "#acme_client" do
    it "returns a new acme client" do
      expect(cert).to receive(:private_key).and_return("test-private-key")
      expect(cert).to receive(:kid).and_return("test-kid")
      expect(OpenSSL::PKey::RSA).to receive(:new).with("test-private-key").and_return("private-key")
      expect(Acme::Client).to receive(:new).with(private_key: "private-key", directory: Config.acme_directory, kid: "test-kid").and_return("client")

      expect(nx.acme_client).to eq "client"
    end

    it "returns nil if private_key is not set" do
      expect(cert).to receive(:private_key).and_return(nil)
  ...
  describe "#acme_order" do
    it "returns the acme order" do
      expect(cert).to receive(:order_url).and_return("test-order-url")
      client = instance_double(Acme::Client)
      expect(nx).to receive(:acme_client).and_return(client)
    ...
    it "returns nil if acme_client is nil" do
      expect(nx).to receive(:acme_client).and_return(nil)
  ...
  describe "#dns_challenge" do
    ...
    it "returns nil if acme_order is nil" do
      expect(nx).to receive(:acme_order).and_return(nil)
      expect(nx.dns_challenge).to be_nil
    end
  end
