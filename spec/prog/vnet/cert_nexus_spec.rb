...
RSpec.describe Prog::Vnet::CertNexus do
  ...
  describe "#start" do
    ...
    it "registers a deadline and starts the certificate creation process" do
      ...
      expect(cert).to receive(:update).with(kid: "test-kid", private_key: key.to_der, order_url: "test-order-url")
  ...
  describe "#acme_client" do
    it "returns a new acme client" do
      expect(cert).to receive(:private_key).and_return("test-private-key").at_least(:once)
      expect(cert).to receive(:kid).and_return("test-kid")
      expect(OpenSSL::PKey::EC).to receive(:new).with("test-private-key").and_return("private-key")
    ...
    it "returns nil if private key is not set" do
