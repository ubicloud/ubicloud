...
RSpec.describe Prog::Vnet::Gcp::SubnetNexus do
  ...
  describe "secure tag helpers" do
    ...
    describe "#wait_crm_op_inline" do
      it "returns immediately when operation is already done" do
        ...
        result = nx.send(:wait_crm_op_inline, op)
        expect(result.response["name"]).to eq("result")
      end

      it "raises when done operation has error" do
        error = Google::Apis::CloudresourcemanagerV3::Status.new(message: "Permission denied")
        op = Google::Apis::CloudresourcemanagerV3::Operation.new(
          name: "operations/err-op", done: true, error:
        )

        expect { nx.send(:wait_crm_op_inline, op) }
      ...
      it "naps when operation is not done" do
        ...
        expect { nx.send(:wait_crm_op_inline, op) }.to nap(5)
      end
    end
