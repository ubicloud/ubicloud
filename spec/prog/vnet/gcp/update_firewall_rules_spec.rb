...
RSpec.describe Prog::Vnet::Gcp::UpdateFirewallRules do
  ...
  describe "ensure_firewall_tag_key" do
    ...
    it "naps when CRM operation is not done" do
  ...
  describe "helper methods" do
    ...
    it "wait_crm_op_inline returns immediately when done" do
      done = instance_double(Google::Apis::CloudresourcemanagerV3::Operation, done?: true, name: "op-1", error: nil, response: {"name" => "result"})
      result = nx.send(:wait_crm_op_inline, done)
      expect(result).to eq(done)
    end

    it "wait_crm_op_inline naps when operation is not done" do
      pending_op = instance_double(Google::Apis::CloudresourcemanagerV3::Operation, done?: false, name: "op-pending")
      expect { nx.send(:wait_crm_op_inline, pending_op) }.to nap(5)
    end

    it "wait_crm_op_inline raises when done with error" do
      ...
      error_op = instance_double(Google::Apis::CloudresourcemanagerV3::Operation,
        done?: true, name: "crm-op-err", error: error_status)

      expect { nx.send(:wait_crm_op_inline, error_op) }.to raise_error(/Permission denied/)
    end
  end
