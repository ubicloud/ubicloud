...
RSpec.describe Prog::Vnet::Gcp::UpdateFirewallRules do
  ...
  let(:location) { instance_double(Location, name: "gcp-us-central1", location_credential: credential) }
  ...
  let(:ps) { instance_double(PrivateSubnet, ubid: "subnetubid1", net4: NetAddr::IPv4Net.parse("10.0.0.0/26"), net6: NetAddr::IPv6Net.parse("fd10:9b0b:6b4b:8fbb::/64"), project:) }
  ...
  describe "#update_firewall_rules" do
    ...
    it "unbinds stale tags from firewalls no longer attached" do
      ...
      allow(regional_crm_client).to receive_messages(list_tag_bindings: existing_bindings, get_operation: unbind_op)
    ...
    it "re-raises 400 errors after retries when there are no stale bindings to free" do
      ...
      # Subnet tag create fails with persistent 400
      create_count = 0
      allow(regional_crm_client).to receive(:create_tag_binding) do
        create_count += 1
        raise Google::Apis::ClientError.new("bad request", status_code: 400)
      end
      ...
      # create_tag_binding retries 400 up to 3 times (4 total attempts)
      expect(create_count).to eq(4)
    end
    ...
    it "handles NOT_FOUND from CRM LRO during stale binding unbind" do
      stale_binding = instance_double(Google::Apis::CloudresourcemanagerV3::TagBinding,
        name: "tagBindings/stale-1", tag_value: "tagValues/old")
      ...
      error = instance_double(Google::Apis::CloudresourcemanagerV3::Status, message: "NOT_FOUND: binding not found")
      not_found_op = instance_double(Google::Apis::CloudresourcemanagerV3::Operation, done?: true, name: "op-del", error:)
      expect(regional_crm_client).to receive(:delete_tag_binding).and_return(not_found_op)
    ...
    it "re-raises non-NOT_FOUND LRO errors during stale binding unbind" do
      stale_binding = instance_double(Google::Apis::CloudresourcemanagerV3::TagBinding,
        name: "tagBindings/stale-1", tag_value: "tagValues/old")
      ...
      denied_op = instance_double(Google::Apis::CloudresourcemanagerV3::Operation, done?: true, name: "op-del", error:)
      expect(regional_crm_client).to receive(:delete_tag_binding).and_return(denied_op)

      expect { nx.update_firewall_rules }.to raise_error(RuntimeError, /PERMISSION_DENIED/)
    end
  end
  ...
  describe "ensure_firewall_tag_key" do
    ...
    it "waits for CRM operation to complete" do
      pending_op = instance_double(Google::Apis::CloudresourcemanagerV3::Operation, done?: false, name: "op-pending")
      done = instance_double(Google::Apis::CloudresourcemanagerV3::Operation, done?: true, name: "op-pending", response: {"name" => "tagKeys/waited-1"}, error: nil)
      expect(crm_client).to receive(:create_tag_key).and_return(pending_op)
      expect(crm_client).to receive(:get_operation).with("op-pending").and_return(done)
      expect(nx).to receive(:sleep).with(1)

      result = nx.send(:ensure_firewall_tag_key, firewall)
      expect(result).to eq("tagKeys/waited-1")
    end

    it "raises when CRM operation times out" do
      pending_op = instance_double(Google::Apis::CloudresourcemanagerV3::Operation, done?: false, name: "op-timeout")
      expect(crm_client).to receive(:create_tag_key).and_return(pending_op)
      expect(crm_client).to receive(:get_operation).exactly(30).times.and_return(pending_op)
      allow(nx).to receive(:sleep)

      expect { nx.send(:ensure_firewall_tag_key, firewall) }.to raise_error(/did not complete within timeout/)
    end
  ...
  describe "create_tag_binding" do
    ...
    it "retries transient 400 errors up to 3 times then succeeds" do
      binding_op = instance_double(Google::Apis::CloudresourcemanagerV3::Operation, done?: true, name: "bind-op", error: nil)
      call_count = 0
      allow(regional_crm_client).to receive(:create_tag_binding) do
        call_count += 1
        raise Google::Apis::ClientError.new("bad request", status_code: 400) if call_count <= 2
        binding_op
      end
      allow(nx).to receive(:sleep)
    ...
    it "re-raises 400 after exhausting retries" do
      allow(regional_crm_client).to receive(:create_tag_binding)
        .and_raise(Google::Apis::ClientError.new("bad request", status_code: 400))
      allow(nx).to receive(:sleep)

      expect { nx.send(:create_tag_binding, "//compute.googleapis.com/...", "tagValues/tv-1") }.to raise_error(Google::Apis::ClientError)
      expect(regional_crm_client).to have_received(:create_tag_binding).exactly(4).times
    end
  end
  ...
  describe "cleanup_old_ip_based_rules" do
    it "removes IP-based rules matching the VM's private IP" do
      ...
      expect(nfp_client).to receive(:get).and_return(policy)
      expect(nfp_client).to receive(:remove_rule).with(hash_including(priority: 12345)).and_return(lro_op)
      expect(nfp_client).not_to receive(:remove_rule).with(hash_including(priority: 54321))

      nx.send(:cleanup_old_ip_based_rules)
    end

    it "includes IPv6 dest range when ephemeral_net6 is set" do
      allow(vm).to receive(:ephemeral_net6).and_return(NetAddr::IPv6Net.parse("2600:1900:4000:1234::"))

      vm_ip = nic.private_ipv4.network.to_s
      ipv4_rule = Google::Cloud::Compute::V1::FirewallPolicyRule.new(
        priority: 10001,
        direction: "INGRESS",
        action: "allow",
        match: Google::Cloud::Compute::V1::FirewallPolicyRuleMatcher.new(
      ...
      ipv6_rule = Google::Cloud::Compute::V1::FirewallPolicyRule.new(
        priority: 10002,
        direction: "INGRESS",
        action: "allow",
        match: Google::Cloud::Compute::V1::FirewallPolicyRuleMatcher.new(
          dest_ip_ranges: ["2600:1900:4000:1234::/128"]
        )
      )
      policy = Google::Cloud::Compute::V1::FirewallPolicy.new(rules: [ipv4_rule, ipv6_rule])
      expect(nfp_client).to receive(:get).and_return(policy)
      expect(nfp_client).to receive(:remove_rule).with(hash_including(priority: 10001)).and_return(lro_op)
      expect(nfp_client).to receive(:remove_rule).with(hash_including(priority: 10002)).and_return(lro_op)

      nx.send(:cleanup_old_ip_based_rules)
    end

    it "returns when VM has no NIC" do
    ...
    it "returns when VM has no private IP" do
      allow(vm).to receive(:nics).and_return([instance_double(Nic, private_ipv4: nil)])
      expect(nfp_client).not_to receive(:get)
    ...
    it "handles errors gracefully" do
      ...
      expect { nx.send(:cleanup_old_ip_based_rules) }.not_to raise_error
    end
    ...
    it "skips rules with nil match" do
      nil_match_rule = Google::Cloud::Compute::V1::FirewallPolicyRule.new(
        priority: 33333,
        direction: "INGRESS",
        action: "allow"
      )
      policy = Google::Cloud::Compute::V1::FirewallPolicy.new(rules: [nil_match_rule])
      expect(nfp_client).to receive(:get).and_return(policy)
      expect(nfp_client).not_to receive(:remove_rule)
  ...
  describe "cleanup_old_per_vm_tag_artifacts" do
    let(:vpc_tag_key_short) { "ubicloud-fw-myprojectubid" }
    let(:vpc_tag_key_name) { "tagKeys/vpc-123" }
    let(:vm_tag_value_name) { "tagValues/vm-456" }

    it "deletes per-VM tag rules and tag value" do
      vpc_tk = instance_double(Google::Apis::CloudresourcemanagerV3::TagKey, short_name: vpc_tag_key_short, name: vpc_tag_key_name)
      vm_tv = instance_double(Google::Apis::CloudresourcemanagerV3::TagValue, short_name: "vm-vmubid1", name: vm_tag_value_name)
      ...
      vm_tag_rule = Google::Cloud::Compute::V1::FirewallPolicyRule.new(
        priority: 10001,
        direction: "INGRESS",
        action: "allow",
        target_secure_tags: [Google::Cloud::Compute::V1::FirewallPolicyRuleSecureTag.new(name: vm_tag_value_name)]
      )
      policy = Google::Cloud::Compute::V1::FirewallPolicy.new(rules: [vm_tag_rule])
      allow(nfp_client).to receive(:get).and_return(policy)
      expect(nfp_client).to receive(:remove_rule).with(hash_including(priority: 10001)).and_return(lro_op)
      expect(crm_client).to receive(:delete_tag_value).with(vm_tag_value_name).and_return(crm_done_op)

      nx.send(:cleanup_old_per_vm_tag_artifacts)
    end

    it "returns early when VPC tag key does not exist" do
    ...
    it "returns early when VM tag value does not exist" do
      vpc_tk = instance_double(Google::Apis::CloudresourcemanagerV3::TagKey, short_name: vpc_tag_key_short, name: vpc_tag_key_name)
      allow(crm_client).to receive_messages(list_tag_keys: instance_double(Google::Apis::CloudresourcemanagerV3::ListTagKeysResponse, tag_keys: [vpc_tk]), list_tag_values: instance_double(Google::Apis::CloudresourcemanagerV3::ListTagValuesResponse, tag_values: []))
    ...
    it "handles errors gracefully" do
      ...
      expect { nx.send(:cleanup_old_per_vm_tag_artifacts) }.not_to raise_error
    end

    it "handles RuntimeError from CRM LRO gracefully" do
      allow(crm_client).to receive(:list_tag_keys)
    ...
    it "handles Google::Cloud::Error gracefully" do
      vpc_tk = instance_double(Google::Apis::CloudresourcemanagerV3::TagKey, short_name: vpc_tag_key_short, name: vpc_tag_key_name)
      vm_tv = instance_double(Google::Apis::CloudresourcemanagerV3::TagValue, short_name: "vm-vmubid1", name: vm_tag_value_name)

      allow(crm_client).to receive_messages(list_tag_keys: instance_double(Google::Apis::CloudresourcemanagerV3::ListTagKeysResponse, tag_keys: [vpc_tk]), list_tag_values: instance_double(Google::Apis::CloudresourcemanagerV3::ListTagValuesResponse, tag_values: [vm_tv]))
      allow(nfp_client).to receive(:get).and_raise(Google::Cloud::Error.new("error"))

      expect { nx.send(:cleanup_old_per_vm_tag_artifacts) }.not_to raise_error
    end

    it "skips non-allow rules" do
      vpc_tk = instance_double(Google::Apis::CloudresourcemanagerV3::TagKey, short_name: vpc_tag_key_short, name: vpc_tag_key_name)
      vm_tv = instance_double(Google::Apis::CloudresourcemanagerV3::TagValue, short_name: "vm-vmubid1", name: vm_tag_value_name)
    ...
    it "skips allow rules whose tags do not match the VM tag value" do
      vpc_tk = instance_double(Google::Apis::CloudresourcemanagerV3::TagKey, short_name: vpc_tag_key_short, name: vpc_tag_key_name)
      vm_tv = instance_double(Google::Apis::CloudresourcemanagerV3::TagValue, short_name: "vm-vmubid1", name: vm_tag_value_name)
  ...
  describe "delete_tag_value" do
    it "deletes the tag value via CRM client" do
      expect(crm_client).to receive(:delete_tag_value).with("tagValues/tv-1").and_return(crm_done_op)
      nx.send(:delete_tag_value, "tagValues/tv-1")
    end

    it "ignores 404 errors" do
      allow(crm_client).to receive(:delete_tag_value)
        .and_raise(Google::Apis::ClientError.new("not found", status_code: 404))
      expect { nx.send(:delete_tag_value, "tagValues/tv-1") }.not_to raise_error
    end

    it "ignores NOT_FOUND from CRM LRO" do
      error = instance_double(Google::Apis::CloudresourcemanagerV3::Status, message: "NOT_FOUND: tag value not found")
      op = instance_double(Google::Apis::CloudresourcemanagerV3::Operation, done?: true, name: "op-del", error:)
      allow(crm_client).to receive(:delete_tag_value).and_return(op)
      expect { nx.send(:delete_tag_value, "tagValues/tv-1") }.not_to raise_error
    end
    ...
    it "re-raises non-NOT_FOUND LRO errors" do
      ...
      op = instance_double(Google::Apis::CloudresourcemanagerV3::Operation, done?: true, name: "op-del", error:)
      allow(crm_client).to receive(:delete_tag_value).and_return(op)
      expect { nx.send(:delete_tag_value, "tagValues/tv-1") }.to raise_error(RuntimeError, /PERMISSION_DENIED/)
    end

    it "raises non-404 errors" do
      allow(crm_client).to receive(:delete_tag_value)
        .and_raise(Google::Apis::ClientError.new("forbidden", status_code: 403))
      expect { nx.send(:delete_tag_value, "tagValues/tv-1") }.to raise_error(Google::Apis::ClientError)
    end
  end

  describe "delete_tag_key" do
    it "deletes the tag key via CRM client" do
      expect(crm_client).to receive(:delete_tag_key).with("tagKeys/tk-1").and_return(crm_done_op)
      nx.send(:delete_tag_key, "tagKeys/tk-1")
    end

    it "ignores 404 errors" do
      allow(crm_client).to receive(:delete_tag_key)
        .and_raise(Google::Apis::ClientError.new("not found", status_code: 404))
      expect { nx.send(:delete_tag_key, "tagKeys/tk-1") }.not_to raise_error
    end

    it "ignores NOT_FOUND from CRM LRO" do
      error = instance_double(Google::Apis::CloudresourcemanagerV3::Status, message: "NOT_FOUND: tag key not found")
      op = instance_double(Google::Apis::CloudresourcemanagerV3::Operation, done?: true, name: "op-del", error:)
      allow(crm_client).to receive(:delete_tag_key).and_return(op)
      expect { nx.send(:delete_tag_key, "tagKeys/tk-1") }.not_to raise_error
    end
    ...
    it "re-raises non-NOT_FOUND LRO errors" do
      ...
      op = instance_double(Google::Apis::CloudresourcemanagerV3::Operation, done?: true, name: "op-del", error:)
      allow(crm_client).to receive(:delete_tag_key).and_return(op)
      expect { nx.send(:delete_tag_key, "tagKeys/tk-1") }.to raise_error(RuntimeError, /PERMISSION_DENIED/)
    end

    it "raises non-404 errors" do
      allow(crm_client).to receive(:delete_tag_key)
        .and_raise(Google::Apis::ClientError.new("forbidden", status_code: 403))
      expect { nx.send(:delete_tag_key, "tagKeys/tk-1") }.to raise_error(Google::Apis::ClientError)
    end
  end
  ...
  describe "helper methods" do
    ...
    it "waits for regional CRM operation" do
      pending_op = instance_double(Google::Apis::CloudresourcemanagerV3::Operation, done?: false, name: "op-1")
      done = instance_double(Google::Apis::CloudresourcemanagerV3::Operation, done?: true, name: "op-1", error: nil)
      expect(regional_crm_client).to receive(:get_operation).with("op-1").and_return(done)
      expect(nx).to receive(:sleep).with(1)

      result = nx.send(:wait_for_regional_crm_operation, pending_op)
      expect(result).to eq(done)
    end

    it "raises when regional CRM operation times out" do
      pending_op = instance_double(Google::Apis::CloudresourcemanagerV3::Operation, done?: false, name: "op-timeout")
      expect(regional_crm_client).to receive(:get_operation).exactly(30).times.and_return(pending_op)
      allow(nx).to receive(:sleep)

      expect { nx.send(:wait_for_regional_crm_operation, pending_op) }.to raise_error(/did not complete within timeout/)
    end

    it "raises when CRM operation completes with error" do
      error_status = instance_double(Google::Apis::CloudresourcemanagerV3::Status, message: "Permission denied")
      error_op = instance_double(Google::Apis::CloudresourcemanagerV3::Operation,
        done?: true, name: "crm-op-err", error: error_status, response: nil)

      expect { nx.send(:wait_for_crm_operation, error_op) }.to raise_error(/Permission denied/)
    end

    it "raises when regional CRM operation completes with error" do
      error_status = instance_double(Google::Apis::CloudresourcemanagerV3::Status, message: "Quota exceeded")
      error_op = instance_double(Google::Apis::CloudresourcemanagerV3::Operation,
        done?: true, name: "crm-op-err", error: error_status, response: nil)

      expect { nx.send(:wait_for_regional_crm_operation, error_op) }.to raise_error(/Quota exceeded/)
    end
  end
