...
RSpec.describe Prog::Vnet::NicNexus do
  ...
  describe ".assemble" do
    ...
    it "hops to create_aws_nic if location is aws" do
      expect(ps).to receive(:location).and_return(instance_double(Location, aws?: true, gcp?: false)).at_least(:once)
    ...
    it "creates a GCP nic if location is gcp" do
      ...
      expect(Nic).to receive(:create_with_id).with(id,
        ...
        private_subnet_id: "57afa8a7-2357-4012-9632-07fbe13a3133",
        name: "demonic", state: "active").and_return(nic)
      expect(Strand).to receive(:create_with_id).with(id, prog: "Vnet::Gcp::NicNexus", label: "start", stack: [{"exclude_availability_zones" => [], "availability_zone" => nil, "ipv4_addr" => "10.0.0.0/26"}]).and_return(Strand.new)
      described_class.assemble(ps.id, name: "demonic")
    end
  end
end
