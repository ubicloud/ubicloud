...
RSpec.describe Prog::Vnet::Aws::VpcNexus do
  ...
  describe "#create_az_subnets" do
    ...
    it "creates AWS subnets and updates records for each AZ and hops to wait" do
      ...
      expect(client).to receive(:associate_route_table).twice.and_call_original
      expect { nx.create_az_subnets }.to hop("wait")
    ...
    it "skips subnets that already have subnet_id set" do
  ...
  describe "#destroy" do
    ...
    describe "#delete_az_subnets" do
      ...
      it "deletes all aws subnets and hops to delete_vpc" do
        ...
        expect(AwsSubnet[aws_subnet.id]).to be_nil
      end
      ...
      it "continues if subnet not found" do
      ...
      it "also deletes subnets found in AWS that are not tracked in database" do
        AwsSubnet.where(private_subnet_aws_resource_id: aws_resource.id).destroy
        client.stub_responses(:describe_subnets, subnets: [{subnet_id: "subnet-untracked"}])
        client.stub_responses(:delete_subnet)
        expect(client).to receive(:delete_subnet).with({subnet_id: "subnet-untracked"}).and_call_original
        expect { nx.delete_az_subnets }.to hop("delete_vpc")
      end
    end
