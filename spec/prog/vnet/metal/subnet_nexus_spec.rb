...
RSpec.describe Prog::Vnet::Metal::SubnetNexus do
  ...
  describe "#wait_old_state_drop" do
    ...
    it "fails if a NIC is destroyed between read and release" do
      nic2 = Prog::Vnet::NicNexus.assemble(ps.id, name: "b").subject
      nic2.update(rekey_payload: {}, rekey_coordinator_id: ps.id, rekey_phase: "old_drop")
      nic.update(rekey_phase: "old_drop")
      ps.update(last_rekey_at: Time.now - 100)

      expect(nx).to receive(:get_locked_nics).and_wrap_original do |m|
        result = m.call
      ...
      expect { nx.wait_old_state_drop }.to raise_error RuntimeError, /released .* but expected/
    end

    it "updates last_rekey_at on all connected subnets and signals released owners" do
      Strand.create(prog: "Vnet::Metal::SubnetNexus", label: "wait", id: ps2.id)
      ps.connect_subnet(ps2)
      ...
      Semaphore.where(strand_id: ps2.id, name: "refresh_keys").destroy
      ...
      expect(Semaphore.where(strand_id: ps2.id, name: "refresh_keys").count).to eq(1)
    end
  end
