...
RSpec.describe Prog::Vnet::LoadBalancerNexus do
  ...
  describe "#rewrite_dns_records" do
    def expect_dns_records(ip4: "192.168.1.0", ip6: "fd10:9b0b:6b4b:8fb0::2", expect_a: true, expect_aaaa: true)
      vms = [instance_double(Vm, ip4_string: ip4, ip6_string: ip6)]
      ...
      if expect_a
        expect(dns_zone).to receive(:insert_record).with(record_name: st.subject.hostname, type: "A", ttl: 10, data: ip4)
      else
        expect(dns_zone).not_to receive(:insert_record).with(record_name: st.subject.hostname, type: "A", ttl: 10, data: anything)
      end
      if expect_aaaa
        expect(dns_zone).to receive(:insert_record).with(record_name: st.subject.hostname, type: "AAAA", ttl: 10, data: ip6)
      else
        expect(dns_zone).not_to receive(:insert_record).with(record_name: st.subject.hostname, type: "AAAA", ttl: 10, data: anything)
      end
    ...
    it "does not create A record if ip4_string is nil" do
      expect_dns_records(ip4: nil, expect_a: false)
    end

    it "does not create AAAA record if ip6_string is nil" do
      expect_dns_records(ip6: nil, expect_aaaa: false)
    end

    it "does not create A record if stack is ipv6" do
      nx.load_balancer.update(stack: "ipv6")
      expect_dns_records(ip4: nil, expect_a: false)
    end

    it "does not create AAAA record if stack is ipv4" do
