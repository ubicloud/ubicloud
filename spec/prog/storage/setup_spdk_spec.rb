...
RSpec.describe Prog::Storage::SetupSpdk do
  ...
  describe "#update_database" do
    it "updates the database and exits" do
      hugepages = 3
      SpdkInstallation.create_with_id(vm_host.id, version: spdk_version, vm_host_id: vm_host.id, hugepages: hugepages, allocation_weight: 0)
    ...
    it "doesn't reserve a hugepage if service didn't start" do
      SpdkInstallation.create_with_id(vm_host.id, version: spdk_version, vm_host_id: vm_host.id, hugepages: 3, allocation_weight: 0)
