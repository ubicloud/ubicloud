...
RSpec.describe Prog::Storage::SetupSpdk do
  ...
  let(:sshable) { setup_spdk.sshable }
  ...
  describe "#start" do
    ...
    it "fails if version/arch combination is not supported" do
      ss = described_class.new(described_class.assemble(vm_host.id, "v1.0"))
      expect { ss.start }.to raise_error RuntimeError, "Unsupported version: v1.0, x64"
    end

    it "fails if already contains 2 installations" do
      SpdkInstallation.create(vm_host_id: vm_host.id, version: "v1", allocation_weight: 0, cpu_count: 4, hugepages: 4)
      SpdkInstallation.create(vm_host_id: vm_host.id, version: "v2", allocation_weight: 0, cpu_count: 4, hugepages: 4)
    ...
    it "fails if not enough hugepages" do
      vm_host.update(used_hugepages_1g: 17)
  ...
  describe "#update_database" do
    it "updates the database and exits" do
      hugepages = 3
      SpdkInstallation.create(version: spdk_version, vm_host_id: vm_host.id, hugepages:, allocation_weight: 0)
    ...
    it "doesn't reserve a hugepage if service didn't start" do
      SpdkInstallation.create(version: spdk_version, vm_host_id: vm_host.id, hugepages: 3, allocation_weight: 0)
      ss = described_class.new(described_class.assemble(vm_host.id, spdk_version, start_service: false, allocation_weight: 50))
