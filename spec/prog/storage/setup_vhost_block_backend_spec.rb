...
RSpec.describe Prog::Storage::SetupVhostBlockBackend do
  ...
  describe "#install_vhost_backend" do
    it "starts the daemonizer if not started" do
      expect(sshable).to receive(:cmd).with("common/bin/daemonizer --check setup-vhost-block-backend-v0.1-5").and_return("NotStarted")
      expect(sshable).to receive(:cmd).with("common/bin/daemonizer 'sudo host/bin/setup-vhost-block-backend install v0.1-5' setup-vhost-block-backend-v0.1-5")
      expect { setup_vhost_block_backend.install_vhost_backend }.to nap(5)
    end

    it "starts the daemonizer if failed" do
      VhostBlockBackend.create(version: "v0.1-5", allocation_weight: 0, vm_host_id: vm_host.id)
      expect(sshable).to receive(:cmd).with("common/bin/daemonizer --check setup-vhost-block-backend-v0.1-5").and_return("Failed")
      expect(sshable).to receive(:cmd).with("common/bin/daemonizer 'sudo host/bin/setup-vhost-block-backend install v0.1-5' setup-vhost-block-backend-v0.1-5")
    ...
    it "updates and pops if succeeded" do
      VhostBlockBackend.create(version: "v0.1-5", allocation_weight: 0, vm_host_id: vm_host.id)
      expect(sshable).to receive(:cmd).with("common/bin/daemonizer --check setup-vhost-block-backend-v0.1-5").and_return("Succeeded")
    ...
    it "naps if the daemonizer is already running" do
      VhostBlockBackend.create(version: "v0.1-5", allocation_weight: 0, vm_host_id: vm_host.id)
      expect(sshable).to receive(:cmd).with("common/bin/daemonizer --check setup-vhost-block-backend-v0.1-5").and_return("InProgress")
      expect { setup_vhost_block_backend.install_vhost_backend }.to nap(5)
    end
  end
end
