...
RSpec.describe Prog::DeletedRecordArchiver do
  ...
  describe "slice upload" do
    ...
    it "ships the same bytes however the copy is batched into the write buffer" do
      chunks = [("x" * described_class::WRITE_BUFFER_BYTES) << "\n", "one\n", "two\n"]
      expect(DB).to receive(:copy_table) { |_ds, **, &block| chunks.each(&block) }

      expect { prog.archive }.to nap(1)

      expect(uncompressed(uploads[0])).to eq chunks.join
