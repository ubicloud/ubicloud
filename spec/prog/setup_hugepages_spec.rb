...
RSpec.describe Prog::SetupHugepages do
  ...
  describe "#start" do
    it "pops after installing hugepages" do
      ...
      sshable = Sshable.new
      expect(sshable).to receive(:_cmd).with("sudo sed -i '/^GRUB_CMDLINE_LINUX=\"/ s/\"$/ hugetlb_free_vmemmap=on default_hugepagesz='1G' hugepagesz='1G' hugepages='59'&/' /etc/default/grub")
