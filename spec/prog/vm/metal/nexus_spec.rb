...
RSpec.describe Prog::Vm::Metal::Nexus do
  ...
  describe ".assemble" do
    ...
    it "uses a machine image if a base boot image is requested and $boot_image-$region@latest exists in the machine images service project" do
      ...
      miv = create_machine_image_version_metal(project_id: mi_project.id, location_id: Location::HETZNER_FSN1_ID, name: "ubuntu-jammy-eu").machine_image_version
      ...
      expect(emit_args).to include([
        "No suitable machine image version found for boot image, falling back to using boot image as BootImage name",
        {
          base_machine_image_version_not_found: {
            boot_image: "ubuntu-jammy-eu",
            location_id: Location::HETZNER_FSN1_ID,
            error: {machine_image: "Machine image with name \"ubuntu-jammy-eu\" does not exist in the specified project and any of the following locations: eu-central-h1, eu-north-h1"},
          },
        },
      ])
    end
    ...
    it "falls back to using boot_image as a BootImage name if the location is not supported for base machine images" do
      mi_project = Project.create(name: "machine-images-service-project")
      expect(Config).to receive(:machine_images_service_project_id).at_least(:once).and_return(mi_project.id)
      vm = Prog::Vm::Nexus.assemble("some_ssh key", project.id, boot_image: "ubuntu-jammy", location_id: Location[name: "hetzner-ai"].id).subject
      expect(vm.vm_storage_volumes.first.machine_image_version_id).to be_nil
      expect(vm.boot_image).to eq("ubuntu-jammy")
    end
