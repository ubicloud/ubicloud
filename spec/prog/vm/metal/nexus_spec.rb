...
RSpec.describe Prog::Vm::Metal::Nexus do
  ...
  def create_stale_kek_volume(disk_index: 0, rotating: false)
    ...
    key_2 = StorageKeyEncryptionKey.create(algorithm: "aes-256-gcm", key: "k2", init_vector: "iv", auth_data: "ad") if rotating
    VmStorageVolume.create(vm_id: vm.id, boot: disk_index == 0, size_gib: 20, disk_index:,
      storage_device_id: sd.id, key_encryption_key_1_id: kek.id, key_encryption_key_2_id: key_2&.id)
  end
  ...
  describe "#wait" do
    ...
    it "does not rotate a storage KEK that is already mid-rotation" do
      create_stale_kek_volume(rotating: true)
      expect { nx.wait }.to nap(6 * 60 * 60)
    end
  end

  describe "#rotate_storage_keys" do
    it "starts a RotateKek for each stale volume and waits for them" do
