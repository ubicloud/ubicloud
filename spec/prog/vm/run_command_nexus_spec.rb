...
RSpec.describe Prog::Vm::RunCommandNexus do
  ...
  describe ".assemble" do
    ...
    it "reuses the existing run's row for the same vm and command instead of archiving a new one" do
      first = described_class.assemble(vm_id: vm.id, command: "fetch_serial_log").subject
      first.update(status: "succeeded", output: "old output", run_at: Time.now)
