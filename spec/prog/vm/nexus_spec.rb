...
RSpec.describe Prog::Vm::Nexus do
  ...
  describe "#before_run" do
    ...
    it "hops to recreate if recreate is set, can_recreate is set, and strand label is not recreate" do
      expect(nx).to receive(:when_recreate_set?).and_yield
      expect(nx).to receive(:when_can_recreate_set?).and_yield
      expect(nx.strand).to receive(:label).and_return("not_recreate").at_least(:once)
      expect { nx.before_run }.to hop("recreate")
    end

    it "does not hop to recreate if strand label is recreate" do
      expect(nx).to receive(:when_recreate_set?).and_yield
      expect(nx.strand).to receive(:label).and_return("recreate").at_least(:once)
      expect { nx.before_run }.not_to hop("recreate")
    end

    it "does not hop to recreate if can_recreate is not set" do
      expect(nx).to receive(:when_recreate_set?).and_yield
      expect(nx.strand).to receive(:label).and_return("not_recreate").at_least(:once)
    ...
    it "does not hop to recreate if recreate is not set" do
  ...
  describe "#recreate" do
    ...
    context "when has vm_host" do
      ...
      before do
        ...
        allow(dev).to receive(:update).with(available_storage_gib: 105)
        allow(vol).to receive_messages(storage_device: dev, size_gib: 5)
        allow(vm).to receive_messages(vm_host: vm_host, vm_storage_volumes: [vol], nics: [nic], assigned_vm_address: assigned_adr)
      end
      ...
      it "hops to start" do
        ...
        expect { nx.recreate }.to hop("start")
      end
