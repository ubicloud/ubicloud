...
RSpec.describe Prog::Vm::VmHostSliceNexus do
  ...
  describe ".assemble_with_host" do
    it "fails with an empty host" do
      expect { described_class.assemble_with_host("standard", nil, allowed_cpus: "", memory_gib: 0) }.to raise_error RuntimeError, "Must provide a VmHost."
    end
    ...
    it "fails with an empty invalid vm_host_slice name" do
      ...
      expect { described_class.assemble_with_host(nil, host, allowed_cpus: "", memory_gib: 0) }.to raise_error RuntimeError, "Must provide slice name."
      expect { described_class.assemble_with_host("", host, allowed_cpus: "", memory_gib: 0) }.to raise_error RuntimeError, "Must provide slice name."
      expect { described_class.assemble_with_host("user", host, allowed_cpus: "", memory_gib: 0) }.to raise_error RuntimeError, "Slice name cannot be 'user' or 'system'."
      expect { described_class.assemble_with_host("system", host, allowed_cpus: "", memory_gib: 0) }.to raise_error RuntimeError, "Slice name cannot be 'user' or 'system'."
      expect { described_class.assemble_with_host("invalid-name", host, allowed_cpus: "", memory_gib: 0) }.to raise_error RuntimeError, "Slice name cannot contain a hyphen (-)."
    end
    ...
    it "creates vm host slice" do
      ...
      st_rg = described_class.assemble_with_host("standard", host, allowed_cpus: "2-3", memory_gib: 4)
      rg = st_rg.subject
