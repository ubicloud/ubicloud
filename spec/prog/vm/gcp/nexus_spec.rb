...
RSpec.describe Prog::Vm::Gcp::Nexus do
  ...
  def ensure_nic_gcp_resource(nic, **overrides)
    ...
    NicGcpResource.create_with_id(
      nic.id,
      network_name: Prog::Vnet::Gcp::SubnetNexus.vpc_name(ps.project),
  ...
  before do
    ...
    %w[a b c].each do |suffix|
      LocationGcpAz.create_with_id(SecureRandom.uuid, location_id: location.id, az: suffix, zone_name: "us-central1-#{suffix}")
    end
  end
  ...
  describe "#start" do
    ...
    it "creates a GCE instance without tags and hops to wait_create_op" do
      ...
      expect(compute_client).to receive(:insert) do |args|
        ...
        expect(ni.network).to eq("projects/test-gcp-project/global/networks/ubicloud-#{project.ubid}")
  ...
  describe "#cleanup_vm_policy_rules" do
    ...
    it "removes policy rules matching the VM's private IP" do
      ...
      expect(nfp_client).to receive(:get).with(
        project: "test-gcp-project",
        firewall_policy: "ubicloud-#{project.ubid}"
      ).and_return(policy)
    ...
    it "removes policy rules matching the VM's IPv6 dest range" do
      vm.update(ephemeral_net6: "2600:1900:4000:1234::")
      vm_ipv6_dest = "2600:1900:4000:1234::/128"
      ...
      ipv6_rule = Google::Cloud::Compute::V1::FirewallPolicyRule.new(
        ...
        action: "allow",
        match: Google::Cloud::Compute::V1::FirewallPolicyRuleMatcher.new(
          dest_ip_ranges: [vm_ipv6_dest]
        )
      )
      policy = Google::Cloud::Compute::V1::FirewallPolicy.new(rules: [ipv6_rule])
      expect(nfp_client).to receive(:get).and_return(policy)
      expect(nfp_client).to receive(:remove_rule).with(hash_including(priority: 12346))
    ...
    it "returns when nic is nil" do
      ...
      # nil-nic guard is at the top — no GCP calls should be made
    ...
    it "skips adding IPv6 dest range when ephemeral_net6 is nil" do
      ensure_nic_gcp_resource(vm.nics.first)
    ...
    it "removes per-VM tag rules and deletes the tag value" do
      ...
      ip_rule = Google::Cloud::Compute::V1::FirewallPolicyRule.new(
        priority: 11111,
        direction: "INGRESS",
        action: "allow",
        match: Google::Cloud::Compute::V1::FirewallPolicyRuleMatcher.new(
      ...
      policy = Google::Cloud::Compute::V1::FirewallPolicy.new(rules: [ip_rule, tag_rule])
      expect(nfp_client).to receive(:get).and_return(policy)
      expect(nfp_client).to receive(:remove_rule).with(hash_including(priority: 11111))
      expect(nfp_client).to receive(:remove_rule).with(hash_including(priority: 22222))
