...
RSpec.describe Prog::DnsZone::DnsZoneNexus do
  subject(:nx) { described_class.new(Strand[dns_zone.id]) }
  ...
  let(:vm) {
    v = Prog::Vm::Nexus.assemble("pub key", prj.id, name: "dns-vm").subject
    Sshable.create_with_id(v.id, unix_user: "root", host: "test-host")
    dns_server.add_vm(v)
    v
  }
  ...
  describe "#refresh_dns_servers" do
    ...
    def sshable
      nx.dns_zone.dns_servers.first.vms.first.sshable
    end
