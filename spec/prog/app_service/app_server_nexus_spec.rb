...
RSpec.describe Prog::AppService::AppServerNexus do
  ...
  describe "#deploy" do
    ...
    it "resolves and pins the commit, then starts the build when NotStarted" do
      ...
      expect(sshable).to receive(:d_run).with("deploy_app", "/home/ubi/app_service/bin/deploy", app_resource.repo_url, app_resource.branch, "abc123", app_resource.secret_store_path, "https://api.ubicloud.com", "web")
    ...
    it "does not re-resolve the commit when it is already pinned" do
      ...
      expect(sshable).to receive(:d_run).with("deploy_app", "/home/ubi/app_service/bin/deploy", app_resource.repo_url, app_resource.branch, "pinned1", app_resource.secret_store_path, "https://api.ubicloud.com", "web")
      expect { nx.deploy }.to nap(5)
    end
    ...
    it "passes the configured API url to the deploy script when set" do
      ...
      deployment.update(commit_sha: "pinned1")
      expect(sshable).to receive(:d_check).with("deploy_app").and_return("NotStarted")
      expect(sshable).to receive(:d_run).with("deploy_app", "/home/ubi/app_service/bin/deploy", app_resource.repo_url, app_resource.branch, "pinned1", app_resource.secret_store_path, "http://api.example.test", "web")
      expect { nx.deploy }.to nap(5)
    end
    ...
    it "passes the database connection config to the deploy script when a database is attached" do
      ...
      expect(sshable).to receive(:d_check).with("deploy_app").and_return("NotStarted")
      expect(sshable).to receive(:d_run).with("deploy_app", "/home/ubi/app_service/bin/deploy", app_resource.repo_url, app_resource.branch, "pinned1", app_resource.secret_store_path, "https://api.ubicloud.com", "web", app_resource.database_deploy_config.to_json)
      expect { nx.deploy }.to nap(5)
    end
