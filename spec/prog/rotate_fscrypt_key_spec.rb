...
RSpec.describe Prog::RotateFscryptKey do
  ...
  let(:kek_1) {
    JSON.generate({
      ...
      "auth_data" => "clover-fscrypt:vmabc123"
    })
  }
  ...
  describe "#start" do
    ...
    it "registers a 10-minute deadline" do
      expect(rfk).to receive(:register_deadline).with(nil, 10 * 60)
      allow(vm_metal).to receive(:update)
    ...
    it "generates a new KEK, stores in fscrypt_key_2, and hops to install" do
      expect(vm_metal).to receive(:update) do |args|
        ...
        expect(kek["auth_data"]).to eq("clover-fscrypt:vmabc123")
      end
      expect { rfk.start }.to hop("install")
    end
  end
