...
RSpec.describe Prog::Postgres::PostgresServerNexus do
  ...
  describe "#wait_synchronization" do
    ...
    it "hops to wait if sync replication is established" do
      ...
      expect(representative_sshable).to receive(:_cmd).with("PGOPTIONS='-c statement_timeout=60s' psql -U ubi_monitoring -d ubi_admin -t --csv -v 'ON_ERROR_STOP=1'", stdin: anything).and_return("quorum", "sync")
    ...
    it "naps and registers a deadline while sync replication is not established" do
      expect(standby_nx).to receive(:register_deadline).with("wait", 5 * 60).twice
      expect(representative_sshable).to receive(:_cmd).with("PGOPTIONS='-c statement_timeout=60s' psql -U ubi_monitoring -d ubi_admin -t --csv -v 'ON_ERROR_STOP=1'", stdin: anything).and_return("", "async")
