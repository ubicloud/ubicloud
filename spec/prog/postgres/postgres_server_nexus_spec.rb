...
RSpec.describe Prog::Postgres::PostgresServerNexus do
  ...
  describe "#configure_metrics" do
    ...
    it "uses bearer token auth when auth_type is bearer" do
      server
      PostgresMetricDestination.create(
        postgres_resource:,
        url: "https://metrics.example.com/write",
    ...
    it "includes tls_config when mtls is enabled" do
      server
      PostgresMetricDestination.create(
        postgres_resource:,
        url: "https://metrics.example.com/write",
      ...
      expect(standby_sshable).to receive(:_cmd).with("sudo -u prometheus tee /home/prometheus/web-config.yml > /dev/null", stdin: anything)
      expect(standby_sshable).to receive(:_cmd).with("sudo -u prometheus tee /home/prometheus/prometheus.yml > /dev/null", stdin: /tls_config:.*cert_file:.*client\.crt.*key_file:.*client\.key/m)
