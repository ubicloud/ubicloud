...
RSpec.describe Prog::Postgres::PostgresServerNexus do
  ...
  describe ".assemble" do
    ...
    it "picks correct base image for AWS-pg16" do
      ...
      ami = PgAwsAmi.first(aws_location_name: "us-west-2", pg_version: "16", arch: "x64") ||
        PgAwsAmi.create(aws_location_name: "us-west-2", pg_version: "16", arch: "x64", aws_ami_id: "ami-pg16-x64")
    ...
    it "picks correct base image for AWS-pg17" do
      ...
      ami = PgAwsAmi.first(aws_location_name: "us-west-2", pg_version: "17", arch: "x64") ||
        PgAwsAmi.create(aws_location_name: "us-west-2", pg_version: "17", arch: "x64", aws_ami_id: "ami-pg17-x64")
  ...
  describe "#before_run" do
    ...
    it "pops additional operations from stack" do
      ...
      nx.strand.update(label: "destroy")
      # Stack needs >1 frame and frame["link"] must be set for pop to work correctly
      # The frame is cached at nexus creation, so we must stub the stack count check
  ...
  describe "#bootstrap_rhizome" do
    ...
    it "sets longer deadline for non-primary servers" do
      ...
      expect(standby_nx).to receive(:register_deadline).with("wait", 120 * 60)
  ...
  describe "#initialize_database_from_backup" do
    ...
    it "triggers initialize_database_from_backup with LATEST as backup_label for standbys" do
      ...
      expect(standby_sshable).to receive(:d_run).with("initialize_database_from_backup", "sudo", "postgres/bin/initialize-database-from-backup", "16", "LATEST")
  ...
  describe "#refresh_certificates" do
    it "waits for certificate creation by the parent resource" do
      expect(server.resource).to receive(:server_cert).and_return(nil)
    ...
    it "pushes certificates to vm and hops to configure_prometheus during initial provisioning" do
      ...
      expect(nx.postgres_server.resource).to receive(:trusted_ca_certs).and_return(nil)
    ...
    it "hops to wait at times other than the initial provisioning" do
      expect(server.resource).to receive(:trusted_ca_certs).and_return(nil)
      expect(sshable).to receive(:_cmd).with("sudo tee /etc/ssl/certs/ca.crt > /dev/null", stdin: "root_cert_1\nroot_cert_2")
      expect(sshable).to receive(:_cmd).with("sudo tee /etc/ssl/certs/server.crt > /dev/null", stdin: "server_cert")
      expect(sshable).to receive(:_cmd).with("sudo tee /etc/ssl/certs/server.key > /dev/null", stdin: "server_cert_key")
      expect(sshable).to receive(:_cmd).with("sudo chgrp cert_readers /etc/ssl/certs/ca.crt && sudo chmod 640 /etc/ssl/certs/ca.crt")
  ...
  describe "#configure_metrics" do
    ...
    it "configures prometheus and metrics during initial provisioning and hops to setup_cloudwatch if timeline is AWS" do
      ...
      aws_timeline_id = PostgresTimeline.generate_uuid
      aws_timeline = PostgresTimeline.create_with_id(aws_timeline_id,
        location_id: aws_location.id,
        access_key: "dummy-access-key",
        secret_key: "dummy-secret-key")
      Strand.create_with_id(aws_timeline_id, prog: "Postgres::PostgresTimelineNexus", label: "wait")
      server.update(timeline_id: aws_timeline.id)
      ...
      expect(nx.postgres_server.resource.project).to receive(:get_ff_aws_cloudwatch_logs).and_return(true)
    ...
    it "includes remote_write config when metric_destinations exist" do
      ...
      PostgresMetricDestination.create(
        postgres_resource_id: postgres_resource.id,
  ...
  describe "#configure" do
    ...
    it "hops to wait_recovery_completion if configure command is succeeded during the initial provisioning and if the server is doing pitr" do
      ...
      expect(pitr_sshable).to receive(:d_clean).with("configure_postgres").and_return("Succeeded")
      expect(pitr_sshable).to receive(:d_check).with("configure_postgres").and_return("Succeeded")
    ...
    it "hops to wait for read replicas if configure command is succeeded" do
      ...
      expect(replica_sshable).to receive(:d_clean).with("configure_postgres").and_return("Succeeded")
      expect(replica_sshable).to receive(:d_check).with("configure_postgres").and_return("Succeeded")
  ...
  describe "#wait_recovery_completion" do
    it "naps if it is still in recovery and wal replay is not paused" do
      ...
      expect(server).to receive(:run_query).with("SELECT pg_get_wal_replay_pause_state()").and_return("not paused")
      expect { nx.wait_recovery_completion }.to nap(5)
    end

    it "naps if it cannot connect to database due to recovery" do
      expect(server).to receive(:run_query).with("SELECT pg_is_in_recovery()").and_raise(Sshable::SshError.new("", nil, "Consistent recovery state has not been yet reached.", nil, nil))
      expect { nx.wait_recovery_completion }.to nap(5)
    end

    it "raises error if it cannot connect to database due a problem other than to continueing recovery" do
      expect(server).to receive(:run_query).with("SELECT pg_is_in_recovery()").and_raise(Sshable::SshError.new("", nil, "Bogus", nil, nil))
    ...
    it "stops wal replay and switches to new timeline if it is still in recovery but wal replay is paused" do
      expect(server).to receive(:run_query).with("SELECT pg_is_in_recovery()").and_return("t")
      expect(server).to receive(:run_query).with("SELECT pg_get_wal_replay_pause_state()").and_return("paused")
      expect(server).to receive(:run_query).with("SELECT pg_wal_replay_resume()")
    ...
    it "switches to new timeline if the recovery is completed" do
      expect(server).to receive(:run_query).with("SELECT pg_is_in_recovery()").and_return("f")
      expect(server).to receive(:switch_to_new_timeline)
      expect { nx.wait_recovery_completion }.to hop("configure")
    end
  end
  ...
  describe "#wait" do
    ...
    describe "read replica" do
      ...
      it "checks if it was already lagging and the lag continues, if so, starts recycling" do
        ...
        expect(replica_nx.strand).to receive(:stack).and_return([{"lsn" => "1/A"}]).at_least(:once)
      ...
      it "checks if it wasn't already lagging but the lag exists, if so, update the stack and nap" do
        ...
        expect(replica_nx.strand).to receive(:stack).and_return([{}]).at_least(:once)
      ...
      it "checks if there was a lag, and it still exist but we are progressing, so, we update the stack and nap" do
        ...
        expect(replica_nx.strand).to receive(:stack).and_return([{"lsn" => "1/9"}]).at_least(:once)
  ...
  describe "#fence" do
    it "runs checkpoints and perform lockout" do
      ...
      expect(server).to receive(:run_query).with("CHECKPOINT; CHECKPOINT; CHECKPOINT;")
  ...
  describe "#prepare_for_planned_take_over" do
    let(:standby_nx) { @standby_nx ||= create_standby_nexus }
  ...
  describe "#available?" do
    ...
    it "returns true if health check is successful" do
      expect(server).to receive(:run_query).with("SELECT 1").and_return("1")
      expect(nx.available?).to be(true)
    end

    it "returns true if the database is in crash recovery" do
      expect(server).to receive(:run_query).with("SELECT 1").and_raise(Sshable::SshError)
