...
RSpec.describe Prog::Postgres::PostgresServerNexus do
  ...
  describe "#wait_lockout_attempt" do
    ...
    it "ignores non-lockout children when checking completion" do
      Strand.create(parent_id: st.id, prog: "Postgres::Restart", label: "start", stack: [{}])
      child = Strand.create(parent_id: st.id, prog: "Postgres::PostgresLockout", label: "start", stack: [{"mechanism" => "pg_stop"}])
      child.update(exitval: Sequel.pg_jsonb_wrap("lockout_succeeded"))
      expect { nx.wait_lockout_attempt }.to hop("wait_locked_out")
    end
  end
