...
RSpec.describe Prog::Postgres::PostgresServerNexus do
  ...
  describe "#restart" do
    ...
    it "cleans up, restarts services and exits when daemonizer reports Succeeded" do
      ...
      expect(nx.restart_set?).to be false
    end

    it "cleans up and naps when daemonizer reports Failed" do
      expect(sshable).to receive(:d_check).with("postgres_restart").and_return("Failed")
      expect(sshable).to receive(:d_clean).with("postgres_restart")
      expect { nx.restart }.to nap(5)
    end

    it "starts restart via daemonizer when NotStarted" do
    ...
    it "naps when restart is InProgress" do
  ...
  describe "#available?" do
    ...
    it "returns true if the database is in crash recovery" do
      ...
      expect(sshable).to receive(:_cmd).with("sudo tail -n 10 /dat/16/data/pg_log/postgresql.log").and_return("redo in progress")
      expect(nx.available?).to be(true)
    end

    it "returns false otherwise" do
      expect(server).to receive(:_run_query).with("SELECT 1").and_raise(Sshable::SshError)
      expect(sshable).to receive(:_cmd).with("sudo tail -n 10 /dat/16/data/pg_log/postgresql.log").and_return("not doing redo")
