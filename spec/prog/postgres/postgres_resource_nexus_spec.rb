...
RSpec.describe Prog::Postgres::PostgresResourceNexus do
  ...
  describe "#update_billing_records" do
    it "creates billing record for cores and storage then hops" do
      ...
      expect(BillingRecord).to receive(:create_with_id).with(
        ...
        billing_rate_id: BillingRate.from_resource_properties("PostgresVCpu", "standard-standard", postgres_resource.location)["id"],
      ...
      expect(BillingRecord).to receive(:create_with_id).with(
        ...
        resource_id: postgres_resource.id,
        resource_name: postgres_resource.name,
        billing_rate_id: BillingRate.from_resource_properties("PostgresStandbyVCpu", "standard-standard", postgres_resource.location)["id"],
        amount: postgres_resource.representative_server.vm.vcpus
      )
  ...
  describe "#wait" do
    ...
    it "buds ConvergePostgresResource prog if needs_convergence? is true" do
      ...
      expect(nx).to receive(:bud).with(Prog::Postgres::ConvergePostgresResource, {}, :provision_servers)
