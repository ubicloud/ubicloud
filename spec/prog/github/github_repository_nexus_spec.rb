...
RSpec.describe Prog::Github::GithubRepositoryNexus do
  subject(:nx) {
    described_class.new(repository.strand).tap {
      it.instance_variable_set(:@github_repository, repository)
    }
  }

  let(:repository) {
    repository = GithubRepository.create(name: "ubicloud/ubicloud", installation_id: installation.id, last_job_at: Time.now)
    Strand.create_with_id(repository.id, prog: "Github::GithubRepositoryNexus", label: "wait")
    repository
  }
  let(:installation) {
    project = Project.create(name: "default")
    GithubInstallation.create(installation_id: 123, project_id: project.id, name: "ubicloud", type: "Organization")
  }
  ...
  describe ".cleanup_cache" do
    ...
    it "deletes oldest cache entries if the total usage exceeds the custom limit" do
      installation.project.add_quota(quota_id: ProjectQuota.default_quotas["GithubRunnerCacheStorage"]["id"], value: 20)
    ...
    it "deletes blob storage if there are no cache entries" do
      expect(repository).to receive(:destroy_blob_storage)
      nx.cleanup_cache
    end
  end
  ...
  describe "#wait" do
    ...
    it "does not check queued jobs but check cache usage if 6 hours passed from the last job" do
      repository.access_key = "key"
      repository.last_job_at = now - 7 * 60 * 60
      expect(nx).to receive(:cleanup_cache)
    ...
    it "does not destroys repository and if not found but has active runners" do
      ...
      GithubRunner.create(installation_id: installation.id, repository_id: repository.id, repository_name: "ubicloud/ubicloud", label: "ubicloud")
      expect(repository).not_to receive(:incr_destroy)
      expect { nx.wait }.to nap(5 * 60)
    end
  ...
  describe "#destroy" do
    ...
    it "deletes resource and pops" do
      expect { nx.destroy }.to exit({"msg" => "github repository destroyed"})
      expect(GithubRepository[repository.id]).to be_nil
    end
  end
end
