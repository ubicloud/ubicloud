...
RSpec.describe Prog::Github::DockerRegistryMirrorNexus do
  ...
  let(:vm_host) {
    ...
    VmHost.create(location_id: Location::GITHUB_RUNNERS_ID, arch: "x64") { _1.id = sa.id }
  }

  let(:sshable) { instance_double(Sshable) }

  let(:vm) { Vm.create(vm_host:, unix_user: "ubi", public_key: "key", name: "vm1", location_id: Location::GITHUB_RUNNERS_ID, boot_image: "github-ubuntu-2204", family: "standard", arch: "arm64", cores: 2, vcpus: 2, memory_gib: 8, project_id:) }

  let(:docker_registry_mirror) { DockerRegistryMirror.create(vm_id: vm.id) { _1.id = "3f6e67c7-6b42-81b4-be47-d9c4be9bb781" } }
  ...
  describe ".assemble" do
    ...
    it "creates the mirror server strand" do
      ...
      expect(Strand.where(prog: "Github::DockerRegistryMirrorNexus").count).to eq(1)
    end
  end
  ...
  describe "#install_docker" do
    it "installs docker" do
      ...
      expect { mirror_server_nexus.install_docker }.to hop("update_docker_registry_config")
    end
  end
  ...
  describe "#update_docker_registry_config" do
    it "updates the docker registry config" do
      ...
      expect { mirror_server_nexus.update_docker_registry_config }.to hop("fetch_certificate")
    end
  end
  ...
  describe "#fetch_certificate" do
    it "fetches the certificate" do
      ...
      expect { mirror_server_nexus.fetch_certificate }.to hop("start_or_restart_docker_registry")
    end
  end

  describe "#start_or_restart_docker_registry" do
    it "starts the docker registry" do
      expect(sshable).to receive(:cmd).with("sudo docker ps -a --filter name=registry-mirror --format '{{.Status}}'").and_return("")
      expect(sshable).to receive(:cmd).with(<<~COMMAND)
        sudo docker run -d \
          ...
          registry:2
      COMMAND
      expect { mirror_server_nexus.start_or_restart_docker_registry }.to hop("wait")
    end

    it "restarts the docker registry" do
      expect(sshable).to receive(:cmd).with("sudo docker ps -a --filter name=registry-mirror --format '{{.Status}}'").and_return("Up 30 seconds")
      expect(sshable).to receive(:cmd).with("sudo docker restart registry-mirror")
      expect { mirror_server_nexus.start_or_restart_docker_registry }.to hop("wait")
    end
  end
  ...
  describe "#wait" do
    it "naps" do
      expect(docker_registry_mirror).to receive(:last_certificate_reset_at).and_return(Time.now - 30 * 60 * 24 * 30)
      expect { mirror_server_nexus.wait }.to nap(60 * 60 * 24)
    end

    it "hops to fetch_certificate if needed" do
      expect(docker_registry_mirror).to receive(:last_certificate_reset_at).and_return(Time.now - 60 * 60 * 24 * 35)
      expect { mirror_server_nexus.wait }.to hop("fetch_certificate")
    end
  end
  ...
  describe "#destroy" do
    it "destroys the mirror server" do
      ...
      expect(docker_registry_mirror).to receive(:destroy)
