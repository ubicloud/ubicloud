  ...
  describe "#wait" do
    ...
    it "buds redelivery children and hops" do
      ...
      expect(prog).to receive(:failed_deliveries).with(time).and_return(
        Array.new(51).map.with_index(1) { |_, i| {guid: i, id: i, status: "Fail", delivered_at: time + 5} }
      )
      ...
      expect(prog.strand.children.last.stack.first["delivery_ids"]).to eq([51])
    end
  end
