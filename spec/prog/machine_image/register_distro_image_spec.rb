...
RSpec.describe Prog::MachineImage::RegisterDistroImage do
  ...
  let(:machine_image) {
    MachineImage.create(
      ...
      s3_prefix: "public/ubuntu-noble/20250502/mi123/",
  ...
  let(:temp_creds) { {access_key_id: "temp-key-id", secret_access_key: "temp-secret-key", session_token: "temp-session-token"} }

  before do
    allow(CloudflareR2).to receive(:generate_temp_credentials).and_return(temp_creds)
  end
  ...
  describe "#destroy" do
    ...
    it "handles paginated S3 listing and deletes objects in batches" do
      s3_client = instance_double(Aws::S3::Client)
      expect(Aws::S3::Client).to receive(:new).and_return(s3_client)

      obj1 = instance_double(Aws::S3::Types::Object, key: "public/ubuntu-noble/obj1")
      obj2 = instance_double(Aws::S3::Types::Object, key: "public/ubuntu-noble/obj2")
      page1 = instance_double(Aws::S3::Types::ListObjectsV2Output, contents: [obj1], is_truncated: true, next_continuation_token: "token1")
      page2 = instance_double(Aws::S3::Types::ListObjectsV2Output, contents: [obj2], is_truncated: false)

      expect(s3_client).to receive(:list_objects_v2).with(bucket: "test-bucket", prefix: machine_image.s3_prefix).and_return(page1)
      expect(s3_client).to receive(:list_objects_v2).with(bucket: "test-bucket", prefix: machine_image.s3_prefix, continuation_token: "token1").and_return(page2)
      expect(s3_client).to receive(:delete_objects).with(
        bucket: "test-bucket",
        delete: {objects: [{key: "public/ubuntu-noble/obj1"}, {key: "public/ubuntu-noble/obj2"}]}
      )
