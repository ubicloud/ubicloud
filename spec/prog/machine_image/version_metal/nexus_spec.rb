...
RSpec.describe Prog::MachineImage::VersionMetal::Nexus do
  ...
  let(:store) {
    MachineImageStore.create(
      ...
      endpoint: "https://r2.example.com/", bucket: "test-bucket",
  ...
  describe ".assemble_from_vm" do
    ...
    it "creates a version with actual_size_mib=nil and a strand at archive_from_vm" do
      ...
      expect(s.prog).to eq("MachineImage::VersionMetal::Nexus")
  ...
  describe "#archive_from_url" do
    ...
    let(:url_strand) {
      Strand.create_with_id(mi_version_metal,
        prog: "MachineImage::VersionMetal::Nexus", label: "archive_from_url",
  ...
  describe "#finish" do
    def stub_finish(p, extra: {})
      refresh_frame(p, new_values: {"physical_size_bytes" => 10 * 1048576, "logical_size_bytes" => 100 * 1048576, **extra})
      allow(p).to receive_messages(source_vm:, vm_host:)
      expect(vm_host.sshable).to receive(:_cmd).with("sudo rm -f /tmp/archive_stats_#{mi_version.ubid}.json")
    end

    it "marks version ready, sets archive_size_mib and actual_size_mib from stats, creates billing, hops to wait" do
      stub_finish(prog)
      expect { prog.finish }.to hop("wait")
    ...
    it "kicks the source VM destroy when destroy_source_after is set" do
      stub_finish(prog, extra: {"destroy_source_after" => true})
      expect { prog.finish }.to hop("wait").and change { source_vm.reload.destroy_set? }.from(false).to(true)
    end

    it "sets latest_version_id when set_as_latest is true and no destroy is queued" do
      stub_finish(prog)
      expect { prog.finish }.to hop("wait")
      expect(machine_image.reload.latest_version_id).to eq(mi_version_metal.id)
    end

    it "skips latest assignment if a destroy semaphore was set during archive" do
      # Build the strand row first, then incr destroy, then build the prog —
      # the prog's snap captures the live semaphore at construction time.
      strand
  ...
  describe "#wait" do
    let(:ready_strand) {
      Strand.create_with_id(mi_version_metal,
        prog: "MachineImage::VersionMetal::Nexus", label: "wait", stack: [{}])
    }
    let(:ready_prog) { described_class.new(ready_strand) }

    before do
      mi_version_metal.update(status: "ready", archive_size_mib: 1)
      ready_strand # ensure the strand row exists so incr_destroy can attach
    end

    it "naps when no destroy semaphore is set" do
      expect { ready_prog.wait }.to nap(6 * 60 * 60)
    end

    it "hops to destroy when destroy semaphore is set" do
      mi_version_metal.incr_destroy
      expect { ready_prog.wait }.to hop("destroy")
    end
  end
  ...
  describe "#destroy" do
    let(:destroy_strand) {
      Strand.create_with_id(mi_version_metal,
        prog: "MachineImage::VersionMetal::Nexus", label: "destroy", stack: [{}])
    }
    let(:destroy_prog) { described_class.new(destroy_strand) }
    ...
    it "flips status to destroying and hops to wait_vms" do
      expect { destroy_prog.destroy }.to hop("wait_vms")
      expect(mi_version_metal.reload.status).to eq("destroying")
    end

    it "decrements the destroy semaphore so it doesn't refire on retry" do
      expect { destroy_prog.destroy }.to hop("wait_vms")
        .and change { mi_version_metal.reload.destroy_set? }.from(true).to(false)
    end

    it "finalizes any active billing records" do
    ...
    it "reassigns latest_version_id to the newest ready sibling when destroying the latest" do
      ...
      args = {machine_image_id: machine_image.id, project_id: project.id, machine_image_store_id: store.id}
      newer = create_machine_image_version_metal(**args, version: "2")
      newer.machine_image_version.update(created_at: Time.now - 60)
      expect { destroy_prog.destroy }.to hop("wait_vms")
      expect(machine_image.reload.latest_version_id).to eq(newer.id)
    end

    it "clears latest_version_id when no other ready sibling exists" do
      machine_image.update(latest_version_id: mi_version.id)
      expect { destroy_prog.destroy }.to hop("wait_vms")
      expect(machine_image.reload.latest_version_id).to be_nil
    end
  ...
  describe "#wait_vms" do
    let(:waiting_strand) {
      Strand.create_with_id(mi_version_metal,
        prog: "MachineImage::VersionMetal::Nexus", label: "wait_vms", stack: [{}])
    }
    let(:waiting_prog) { described_class.new(waiting_strand) }
    ...
    it "naps while a VM still references the MIV" do
      ...
      expect { waiting_prog.wait_vms }.to nap(30)
    end

    it "hops to destroy_objects when no VMs reference the MIV" do
      expect { waiting_prog.wait_vms }.to hop("destroy_objects")
    end
  end
  ...
  describe "#destroy_objects" do
    let(:objects_strand) {
      Strand.create_with_id(mi_version_metal,
        prog: "MachineImage::VersionMetal::Nexus", label: "destroy_objects", stack: [{}])
    }
    let(:objects_prog) { described_class.new(objects_strand) }
    ...
    it "hops to update_database when no objects" do
      s3_client.stub_responses(:list_objects_v2, {contents: [], is_truncated: false})
      expect { objects_prog.destroy_objects }.to hop("update_database")
    end

    it "deletes a page of objects and naps" do
      s3_client.stub_responses(:list_objects_v2, {contents: [{key: "a"}, {key: "b"}], is_truncated: true})
      expect(s3_client).to receive(:delete_objects).with(
        bucket: store.bucket, delete: {objects: [{key: "a"}, {key: "b"}]},
      ).and_call_original
      expect { objects_prog.destroy_objects }.to nap(0)
    end

    it "logs and naps 30 when delete_objects returns per-object errors" do
      s3_client.stub_responses(:list_objects_v2, {contents: [{key: "a"}], is_truncated: false})
      expect(s3_client).to receive(:delete_objects).and_return(
        Aws::S3::Types::DeleteObjectsOutput.new(
          deleted: [], errors: [Aws::S3::Types::Error.new(key: "a", code: "AccessDenied", message: "no")],
        ),
      )
      expect(Clog).to receive(:emit).with("Failed to delete some machine image archive objects",
        hash_including(machine_image: mi_version.machine_image.ubid, version: "1.0", count: 1))
      expect { objects_prog.destroy_objects }.to nap(30)
    end
  end
  ...
  describe "#update_database" do
    let(:update_strand) {
      Strand.create_with_id(mi_version_metal,
        prog: "MachineImage::VersionMetal::Nexus", label: "update_database", stack: [{}])
    }
    let(:update_prog) { described_class.new(update_strand) }

    before { mi_version_metal.update(status: "destroying") }

    it "destroys the metal row, kek, and version and pops" do
      expect { update_prog.update_database }.to exit({"msg" => "Metal machine image version is destroyed"})
