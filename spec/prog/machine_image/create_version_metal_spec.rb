...
RSpec.describe Prog::MachineImage::CreateVersionMetal do
  ...
  let(:mi_version_metal) {
    MachineImageVersionMetal.create(
      enabled: false,
  ...
  describe "#archive" do
    ...
    %w[Failed NotStarted].each do |status|
      it "starts daemon when status is #{status}" do
        expect(sshable).to receive(:_cmd).with("common/bin/daemonizer --check #{daemon_name}").and_return(status)
        expect(sshable).to receive(:_cmd).with(
          "common/bin/daemonizer sudo\\ host/bin/archive-storage-volume\\ #{source_vm.inhost_name}\\ vda\\ 0\\ v0.4.0 #{daemon_name}",
          stdin: "{}"
        )
  ...
  describe "#finish" do
    ...
    it "enables machine image version metal and updates latest version" do
  ...
  describe "#archive_size_bytes" do
    it "sums object sizes across pages" do
      ...
      allow(s3).to receive(:list_objects_v2).with(bucket: "test-bucket", prefix: mi_version_metal.store_prefix).and_return([page1, page2])
