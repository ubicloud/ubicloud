...
RSpec.describe Prog::Minio::SetupMinio do
  subject(:nx) { described_class.new(Strand[minio_server.id]) }
  ...
  let(:minio_server) {
    ...
    ms_id = MinioServer.generate_uuid
    vm = create_vm
    Sshable.create_with_id(vm.id)
    VmStorageVolume.create(vm_id: vm.id, boot: false, size_gib: 100, disk_index: 1)
    Strand.create_with_id(ms_id, prog: "Minio::SetupMinio", label: "install_minio")
