...
RSpec.describe Prog::Kubernetes::UpgradeKubernetesNode do
  ...
  describe "#drain_old_node" do
    ...
    before do
      ...
      cp_node = KubernetesNode.create(vm_id: create_vm.id, kubernetes_cluster_id: kubernetes_cluster.id)
      expect(kubernetes_cluster).to receive(:nodes).and_return([old_node, cp_node])
      allow(cp_node).to receive(:sshable).and_return(sshable)
  ...
  describe "#remove_old_node_from_cluster" do
    before do
      ...
      expect(kubernetes_cluster.reload.all_nodes.map(&:id)).to eq((kubernetes_cluster.nodes + kubernetes_nodepool.nodes).map(&:id))

      mock_sshable = instance_double(Sshable)
      expect(prog.old_node).to receive(:sshable).and_return(mock_sshable)
    ...
    it "removes the old node from the CP while upgrading the control plane" do
      ...
      expect(kubernetes_cluster).to receive(:remove_cp_vm).with(prog.old_node.vm)
    ...
    it "removes the old node from the nodepool while upgrading the node pool" do
      ...
      expect(prog.kubernetes_nodepool).to receive(:remove_vm).with(prog.old_node.vm)
      expect { prog.remove_old_node_from_cluster }.to hop("delete_node_object")
    end
  end
