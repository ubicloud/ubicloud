...
RSpec.describe Prog::Kubernetes::ProvisionKubernetesNode do
  ...
  let(:node) {
    ...
    node = KubernetesNode.create(vm_id: vm.id, kubernetes_cluster_id: kubernetes_cluster.id)
    kubernetes_cluster.add_cp_vm(node.vm)
    node
  }
  ...
  describe "#start" do
    it "creates a control plane node and hops if a nodepool is not given" do
      ...
      new_node = kubernetes_cluster.nodes.last
      new_vm = new_node.vm
    ...
    it "assigns the default storage size if not specified" do
      ...
      new_node = kubernetes_cluster.nodes.last
      expect(new_node.vm.strand.stack.first["storage_volumes"].first["size_gib"]).to eq 80
    end
  end
...
table ip6 pod_access {
  ...
  describe "#join_control_plane" do
    ...
    it "runs the join_control_plane script if it's not started" do
      ...
      sshable = instance_double(Sshable)
      allow(kubernetes_cluster.nodes.first).to receive(:sshable).and_return(sshable)
