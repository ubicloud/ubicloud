...
RSpec.describe Prog::Kubernetes::KubernetesClusterNexus do
  ...
  describe "#install_metrics_server" do
    ...
    before do
      allow(node).to receive(:sshable).and_return(sshable)
      allow(kubernetes_cluster).to receive(:nodes).and_return([node])
    end
  ...
  describe "#sync_worker_mesh" do
    ...
    before do
      expect(kubernetes_cluster).to receive(:worker_nodes).and_return([first_node, second_node])
    ...
    it "creates full mesh connectivity on cluster worker nodes" do
      expect(first_node.sshable).to receive(:cmd).with("tee ~/.ssh/id_ed25519 > /dev/null && chmod 0600 ~/.ssh/id_ed25519", stdin: first_ssh_key.private_key)
      expect(first_node.sshable).to receive(:cmd).with("tee ~/.ssh/authorized_keys > /dev/null && chmod 0600 ~/.ssh/authorized_keys", stdin: [first_node.sshable.keys.first.public_key, first_ssh_key.public_key, second_ssh_key.public_key].join("\n"))

      expect(second_node.sshable).to receive(:cmd).with("tee ~/.ssh/id_ed25519 > /dev/null && chmod 0600 ~/.ssh/id_ed25519", stdin: second_ssh_key.private_key)
      expect(second_node.sshable).to receive(:cmd).with("tee ~/.ssh/authorized_keys > /dev/null && chmod 0600 ~/.ssh/authorized_keys", stdin: [second_node.sshable.keys.first.public_key, first_ssh_key.public_key, second_ssh_key.public_key].join("\n"))
