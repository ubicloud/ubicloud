...
RSpec.describe Prog::Test::UpgradePostgresResource do
  ...
  describe "#test_postgres_with_read_replica" do
    ...
    it "fails if read queries on replica fail before upgrade" do
      ...
      allow(pgr_test.read_replica.representative_server).to receive(:run_query).and_return("")
      expect { pgr_test.test_postgres_with_read_replica }.to hop("destroy_postgres")
      refresh_frame(pgr_test)
    ...
    it "hops to trigger_upgrade if both primary and replica pass tests" do
      allow(pgr_test.representative_server).to receive(:run_query).and_return("4159.90\n415.99\n4.1")
      allow(pgr_test.read_replica.representative_server).to receive(:run_query).and_return("4159.90\n415.99\n4.1")
  ...
  describe "#trigger_upgrade" do
    ...
    it "updates target_version to 18 and hops to wait_upgrade" do
      expect { pgr_test.trigger_upgrade }.to hop("wait_upgrade")
  ...
  describe "#wait_upgrade" do
    it "naps for 1200 seconds the first time" do
      expect { pgr_test.wait_upgrade }.to nap(1200)
      refresh_frame(pgr_test)
      expect(frame_value(pgr_test, "upgrade_wait_started")).to be true
    end

    it "hops to check_upgrade_progress the second time" do
      refresh_frame(pgr_test, new_values: {"upgrade_wait_started" => true})
      expect { pgr_test.wait_upgrade }.to hop("check_upgrade_progress")
    end
  end
