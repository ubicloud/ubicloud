...
RSpec.describe Prog::Test::Kubernetes do
  ...
  describe "#test_data_write" do
    ...
    before do
      ...
      expect(client).to receive(:kubectl).with("exec -t ubuntu-statefulset-0 -- sh -c \"head -c 200M /dev/urandom > /etc/data/random-data\"")
    end

    it "writes data and tests the output file size and is ok" do
      expect(client).to receive(:kubectl).with("exec -t ubuntu-statefulset-0 -- du -sh /etc/data/random-data").and_return("200M	/etc/data/random-data\n")
      expect(kubernetes_test).not_to receive(:update_stack)
      expect { kubernetes_test.test_data_write }.to hop("destroy_kubernetes")
    end

    it "writes data and tests the output file size and is not ok" do
      expect(client).to receive(:kubectl).with("exec -t ubuntu-statefulset-0 -- du -sh /etc/data/random-data").and_return("something else\n")
      expect(kubernetes_test).to receive(:update_stack).with({"fail_message" => "wrong file size, expected: 200M\t/etc/data/random-data, got: something else"})
      expect { kubernetes_test.test_data_write }.to hop("destroy_kubernetes")
    end
  end
