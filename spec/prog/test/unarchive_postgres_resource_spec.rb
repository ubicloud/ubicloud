...
RSpec.describe Prog::Test::UnarchivePostgresResource do
  ...
  describe "#wait_wal_archive" do
    ...
    it "hops to destroy_resource_only once WAL is archived" do
      expect(postgres_resource.timeline).to receive(:any_archived_wal?).and_return(true)
    ...
    it "naps if no archives yet" do
      expect(postgres_resource.timeline).to receive(:any_archived_wal?).and_return(false)
      expect(nx).to receive(:postgres_resource).and_return(postgres_resource).at_least(:once)
