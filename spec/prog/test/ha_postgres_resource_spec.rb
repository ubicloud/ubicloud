...
RSpec.describe Prog::Test::HaPostgresResource do
  ...
  describe "#wait_failover" do
    it "naps if the failover did not happen yet" do
      frame = {"primary_ubid" => "1234"}
      expect(pgr_test).to receive(:frame).and_return(frame)
      expect(pgr_test).to receive(:postgres_resource).and_return(instance_double(PostgresResource, servers: servers))
      expect { pgr_test.wait_failover }.to nap(10)
    end

    it "hops to test_postgres_after_failover if the failover is successful" do
      expect(pgr_test).to receive(:postgres_resource).and_return(instance_double(PostgresResource, servers: servers_after_failover))
