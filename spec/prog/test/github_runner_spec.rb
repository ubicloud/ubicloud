...
RSpec.describe Prog::Test::GithubRunner do
  ...
  before do
    allow(Config).to receive(:github_runner_service_project_id).and_return("fabd95f8-d002-8ed2-9f4c-00625eb7f574")
    allow(Config).to receive(:vm_pool_project_id).and_return("c3fd495f-9888-82d2-8100-7fae94e87e27")
  ...
  describe "#trigger_test_run" do
    it "triggers test run" do
      ...
      expect(client).to receive(:post).with(
        ...
        {ref: "enes/simply-tests", inputs: {triggered_by: "12345", provider: "metal", runners: gr_test.frame["labels"].to_json}}
  ...
  describe "#check_test_run" do
    let(:created_at) { gr_test.frame["created_at"] }

    it "hops to clean_resources on success" do
      allow(ENV).to receive(:[]).and_call_original
      expect(ENV).to receive(:[]).with("GITHUB_RUN_ID").and_return("12345")
      expect(client).to receive(:workflow_runs).with("tahcloud/github-e2e-tests-metal", "test.yml", {branch: "enes/simply-tests"})
        .and_return({workflow_runs: [{conclusion: "success", created_at: Time.now + 10, name: "test 12345", id: 99}]})
    ...
    it "naps when conclusion is nil" do
      ...
      expect(ENV).to receive(:[]).with("GITHUB_RUN_ID").and_return("12345")
      expect(client).to receive(:workflow_runs).with("tahcloud/github-e2e-tests-metal", "test.yml", {branch: "enes/simply-tests"})
        .and_return({workflow_runs: [{conclusion: nil, created_at: Time.now + 10, name: "test 12345", id: 99}]})
    ...
    it "naps when conclusion is in_progress" do
      ...
      expect(ENV).to receive(:[]).with("GITHUB_RUN_ID").and_return("12345")
      expect(client).to receive(:workflow_runs).with("tahcloud/github-e2e-tests-metal", "test.yml", {branch: "enes/simply-tests"})
        .and_return({workflow_runs: [{conclusion: "in_progress", created_at: Time.now + 10, name: "test 12345", id: 99}]})
      expect { gr_test.check_test_run }.to nap(15)
    end
    ...
    it "hops to clean_resources on failure" do
      ...
      expect(ENV).to receive(:[]).with("GITHUB_RUN_ID").and_return("12345")
      expect(client).to receive(:workflow_runs).with("tahcloud/github-e2e-tests-metal", "test.yml", {branch: "enes/simply-tests"})
        .and_return({workflow_runs: [{conclusion: "failure", created_at: Time.now + 10, name: "test 12345", id: 99}]})
      expect { gr_test.check_test_run }.to hop("clean_resources")
    end

    it "hops to clean_resources when run is not found" do
      allow(ENV).to receive(:[]).and_call_original
      allow(ENV).to receive(:[]).with("GITHUB_RUN_ID").and_return("99999")
      expect(client).to receive(:workflow_runs).with("tahcloud/github-e2e-tests-metal", "test.yml", {branch: "enes/simply-tests"})
        .and_return({workflow_runs: [{conclusion: "failure", created_at: Time.now + 10, name: "unrelated run", id: 99}]})
      expect { gr_test.check_test_run }.to hop("clean_resources")
    end
  end
