...
EOS
      ...
      expect(sshable).to receive(:_cmd).with("df -B1 --output=source,target,size,avail /var/storage").and_return(<<EOS)
Filesystem     Mounted on                   1B-blocks        Avail
/dev/sdb       /var/storage/devices/stor1   205520896     99571712
/dev/sdc       /var/storage/devices/stor2  3331416064   3331276800
EOS
...
EOS
      ...
      expect(sshable).to receive(:_cmd).with("cat /proc/mdstat").and_return(<<EOS)
