...
RSpec.describe Prog::Ai::InferenceRouterReplicaNexus do
  ...
  describe "#ping_inference_router" do
    ...
    it "for public routers" do
      ...
      expect(sshable).to receive(:cmd).with(
        "mkdir -p /ir/workdir && sudo tee /ir/workdir/config.json > /dev/null",
        hash_including(stdin: a_string_matching(/"projects":/))
      ) do |command, options|
        json_sent = JSON.parse(options[:stdin])
        expect(json_sent["basic"]).to eq({})
        expect(json_sent["certificates"].map { |h| h.transform_keys(&:to_sym) }).to eq([{
          tls_cert: cert.cert,
          tls_key: OpenSSL::PKey.read(cert.csr_key).to_pem,
          is_default: true,
          is_ca: false
        }])
        ...
        expect(json_sent["routes"].map { |h| h.transform_keys(&:to_sym).except(:endpoints) }).to eq([{
          model_name: "test-model",
          project_rps_limit: 100,
          project_tps_limit: 10000
        }])
        expect(json_sent["routes"][0]["endpoints"].map { |h|
      ...
      expect(sshable).to receive(:cmd).with(
        ...
        inference_router: replica.ubid,
        replica: replica.ubid,
        projects: usage
      }.to_json)
    ...
    it "skips config update when unchanged" do
      ...
      expect(sshable).to receive(:cmd).with(
        "md5sum /ir/workdir/config.json | awk '{ print $1 }'"
      ).and_return("7dc8cf6ad7abd3eccd6f4fcf5ffafc08")
