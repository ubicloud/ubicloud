...
RSpec.describe Prog::Ai::InferenceRouterReplicaNexus do
  ...
  describe "#setup" do
    it "hops to wait_router_up if setup command has succeeded" do
      ...
      expect(Config).to receive(:inference_router_asset_name).and_return(
        "inference-router-v0.1.0-x86_64-unknown-linux-gnu"
      )
      expect(Config).to receive(:inference_router_asset_url).and_return(
        "https://github.com/ubicloud/inference-router/releases/download/v0.1.0/inference-router-v0.1.0-x86_64-unknown-linux-gnu.tar.gz"
      )
      expect(sshable).to receive(:cmd).with(
        "sudo useradd --system --no-create-home --shell /usr/sbin/nologin inference-router"
      )
      expect(sshable).to receive(:cmd).with(
        "sudo wget -O /ir/workdir/inference-router-v0.1.0-x86_64-unknown-linux-gnu.tar.gz https://github.com/ubicloud/inference-router/releases/download/v0.1.0/inference-router-v0.1.0-x86_64-unknown-linux-gnu.tar.gz"
      )
      expect(sshable).to receive(:cmd).with(
  ...
  describe "#ping_inference_router" do
    ...
    it "for public routers" do
      ...
      expect(nx).to receive(:update_billing_records).with(
        usage, "output", "completion_token_count"
      )
    ...
    it "skips config update when unchanged" do
      ...
      expect(sshable).to receive(:cmd).with(
        "md5sum /ir/workdir/config.json | awk '{ print $1 }'"
      ).and_return("6c7ddf6896e3202b6a3ce49c4cb9e76e") # md5sum of the test config.
