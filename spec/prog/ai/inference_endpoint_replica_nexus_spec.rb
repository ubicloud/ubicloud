...
RSpec.describe Prog::Ai::InferenceEndpointReplicaNexus do
  ...
  describe "#ping_gateway" do
    ...
    it "for private endpoints" do
      ...
      expect(nx).to receive(:update_billing_records).with(
        JSON.parse("[{\"ubid\":\"theubid\",\"request_count\":1,\"prompt_token_count\":10,\"completion_token_count\":20},{\"ubid\":\"anotherubid\",\"request_count\":0,\"prompt_token_count\":0,\"completion_token_count\":0}]"),
        []
      )
  ...
  describe "#update_billing_records" do
    ...
    it "updates billing records" do
      ...
      expect(br.amount).to eq(0)
      expect(br.free_quota_amount).to eq(30)

      nx.update_billing_records([{"ubid" => p1.ubid, "request_count" => 1, "prompt_token_count" => 1, "completion_token_count" => 2}], [])
      expect(BillingRecord.count).to eq(1)
      expect(Integer(br.reload.amount)).to eq(0)
      expect(Integer(br.reload.free_quota_amount)).to eq(33)

      nx.update_billing_records([{"ubid" => p1.ubid, "request_count" => 1, "prompt_token_count" => 1, "completion_token_count" => 2}], [p1.id])
      expect(BillingRecord.count).to eq(1)
      expect(Integer(br.reload.amount)).to eq(3)
      expect(Integer(br.reload.free_quota_amount)).to eq(33)

      nx.update_billing_records([{"ubid" => p1.ubid, "request_count" => 1, "prompt_token_count" => 1, "completion_token_count" => 2}], [p1.id])
      expect(BillingRecord.count).to eq(1)
      expect(Integer(br.reload.amount)).to eq(6)
      expect(Integer(br.reload.free_quota_amount)).to eq(33)
    end

    it "does not update for zero tokens" do
      expect(BillingRecord.count).to eq(0)
      nx.update_billing_records([{"ubid" => p1.ubid, "request_count" => 0, "prompt_token_count" => 0, "completion_token_count" => 0}], [p1.id])
      expect(BillingRecord.count).to eq(0)
    end
    ...
    it "does not update if price is zero" do
      ...
      expect(BillingRecord.count).to eq(0)
      nx.update_billing_records([{"ubid" => p1.ubid, "request_count" => 1, "prompt_token_count" => 2, "completion_token_count" => 3}], [p1.id])
      expect(BillingRecord.count).to eq(0)
    end
    ...
    it "failure in updating single record doesn't impact others" do
      ...
      nx.update_billing_records([
        ...
        {"ubid" => p2.ubid, "request_count" => 1, "prompt_token_count" => 2, "completion_token_count" => 3}
      ], [p1.id, p2.id])
      expect(BillingRecord.count).to eq(1)
      br = BillingRecord.first
