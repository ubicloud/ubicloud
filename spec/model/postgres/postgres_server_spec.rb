...
RSpec.describe PostgresServer do
  ...
  let(:vm) { create_vm_etc(project, private_subnet, "dummy-vm") }
  ...
  describe "#configure" do
    ...
    it "sets synchronous_standby_names for sync replication mode" do
      ...
      described_class.create(
        timeline:, resource_id: resource.id, vm_id: create_vm_etc(project, private_subnet, "standby1").id,
        synchronization_status: "catching_up", timeline_access: "fetch", version: "16"
      )
      standby2 = described_class.create(
        timeline:, resource_id: resource.id, vm_id: create_vm_etc(project, private_subnet, "standby2").id,
    ...
    it "sets synchronous_standby_names as empty if there is no caught up standby" do
      ...
      described_class.create(
        timeline:, resource: create_standby_resource("1"), vm_id: create_vm_etc(project, private_subnet, "standby1").id, representative_at: Time.now,
        synchronization_status: "catching_up", timeline_access: "fetch", version: "16"
      )
      described_class.create(
        timeline:, resource: create_standby_resource("2"), vm_id: create_vm_etc(project, private_subnet, "standby2").id, representative_at: Time.now,
        synchronization_status: "catching_up", timeline_access: "fetch", version: "16"
      )
  ...
  describe "#trigger_failover" do
    ...
    it "returns true only when failover is successfully triggered" do
      standby = described_class.create(
        timeline:, resource_id: resource.id, vm_id: create_vm_etc(project, private_subnet, "standby").id,
        synchronization_status: "ready", timeline_access: "fetch", version: "16"
      )
  ...
  describe "lsn_caught_up" do
    before do
      ...
      parent_vm = create_vm_etc(project, private_subnet, "parent-vm")
      described_class.create(
