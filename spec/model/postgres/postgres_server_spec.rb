...
RSpec.describe PostgresServer do
  ...
  describe "#observe_archival_backlog" do
    ...
    it "does not page when backlog is moderate and upload rate is above threshold" do
      ...
      session[:last_oldest_pending_wal] = "000000010000000000000005.ready"
    ...
    it "pages when backlog is moderate but upload rate is below threshold" do
      ...
      session[:last_oldest_pending_wal] = "000000010000000000000005.ready"
      session[:last_archival_check_time] = Time.now - 2
      allow(session[:ssh_session]).to receive(:_exec!).and_return(
  ...
  describe "#estimate_archival_rate" do
    it "calculates bytes per second based on WAL segment progression" do
      now = Time.now
      previous_time = now - 1
      # 5 segments archived = 80MB in 1 second
      rate = postgres_server.estimate_archival_rate(
        "000000010000000000000005.ready",
        "00000001000000000000000A.ready",
        now,
        previous_time
      )
      expect(rate).to eq(80 * 1024 * 1024)
    end

    it "returns 0 when no progress was made" do
      now = Time.now
      previous_time = now - 1
      rate = postgres_server.estimate_archival_rate(
        "000000010000000000000005.ready",
    ...
    it "returns infinity when elapsed time is zero" do
      now = Time.now
      rate = postgres_server.estimate_archival_rate(
        "000000010000000000000005.ready",
        "00000001000000000000000A.ready",
      ...
      expect(rate).to eq(Float::INFINITY)
    end
  end
