...
RSpec.describe PostgresServer do
  ...
  describe "#configure" do
    ...
    it "includes both legacy cert_auth_users and managed cert roles in the configure hash" do
      resource.update(cert_auth_users: ["legacy_user"])
      PostgresManagedRole.create(postgres_resource_id: resource.id, name: "app_rw", auth_type: "cert", state: "active")
      hash = postgres_server.configure_hash
      expect(hash[:cert_auth_users]).to eq(["legacy_user"])
      expect(hash[:managed_cert_roles]).to eq(["app_rw"])
    end
