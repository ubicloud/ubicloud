...
RSpec.describe PostgresResource do
  ...
  it "is in the window on an enabled day" do
    postgres_resource.update(maintenance_window_start_at: 10, maintenance_window_days: 1 << 3)
    expect(Time).to receive(:now).and_return(Time.parse("2025-05-01 10:30:00Z"))
    expect(postgres_resource.in_maintenance_window?).to be(true)
  end

  it "is not in the window on a disabled day" do
    postgres_resource.update(maintenance_window_start_at: 10, maintenance_window_days: 1 << 0) # Monday only
    expect(Time).to receive(:now).and_return(Time.parse("2025-05-01 10:30:00Z"))
    expect(postgres_resource.in_maintenance_window?).to be(false)
  end

  it "treats 0 days as every day" do
    postgres_resource.update(maintenance_window_start_at: 10, maintenance_window_days: 0)
    expect(Time).to receive(:now).and_return(Time.parse("2025-05-01 10:30:00Z"))
    expect(postgres_resource.in_maintenance_window?).to be(true)
  end
  ...
  it "is not in the window outside the hours regardless of day" do
    postgres_resource.update(maintenance_window_start_at: 10, maintenance_window_days: 1 << 3)
    expect(Time).to receive(:now).and_return(Time.parse("2025-05-01 13:00:00Z"))
    expect(postgres_resource.in_maintenance_window?).to be(false)
  end
  ...
  it "checks the day the window opened on when it wraps past midnight" do
    ...
    # the Thursday window.
    postgres_resource.update(maintenance_window_start_at: 23, maintenance_window_days: 1 << 3)
    expect(Time).to receive(:now).and_return(Time.parse("2025-05-02 00:30:00Z")) # Friday 00:30
    expect(postgres_resource.in_maintenance_window?).to be(true)
  end

  it "converts day names to and from the bitmask" do
    expect(PostgresResource.maintenance_window_days_mask(nil)).to be_nil
    expect(PostgresResource.maintenance_window_days_mask([])).to be_nil

    mask = PostgresResource.maintenance_window_days_mask(["mon", "Wed", "fri"])
    expect(mask).to eq((1 << 0) | (1 << 2) | (1 << 4))

    postgres_resource.update(maintenance_window_days: mask)
    expect(postgres_resource.maintenance_window_day_names).to eq(["mon", "wed", "fri"])

    postgres_resource.update(maintenance_window_days: 0)
    expect(postgres_resource.maintenance_window_day_names).to eq([])

    postgres_resource.update(maintenance_window_days: nil)
    expect(postgres_resource.maintenance_window_day_names).to eq([])
  end

  it "raises on an invalid day name" do
    expect { PostgresResource.maintenance_window_days_mask(["funday"]) }.to raise_error(Validation::ValidationFailed, /maintenance_window_days/)
  end
