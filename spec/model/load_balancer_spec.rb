...
RSpec.describe LoadBalancer do
  ...
  describe "util funcs" do
    ...
    it "returns hyper_tag_name" do
      prj = lb.private_subnet.projects.first
      expect(lb.hyper_tag_name(prj)).to eq("project/#{prj.ubid}/load-balancer/test-lb")
    end
  ...
  describe "add_vm" do
    it "increments update_load_balancer and rewrite_dns_records" do
      ...
      lb.add_vm(vm1)
      expect(DB[:load_balancers_vms].count).to eq(1)
    end
  end
  ...
  describe "detach_vm" do
    ...
    it "increments update_load_balancer and rewrite_dns_records" do
      expect(lb).to receive(:incr_update_load_balancer)
      expect(lb).to receive(:incr_rewrite_dns_records)
      health_probe = instance_double(Strand, stack: [{"load_balancer_id" => lb.id, "subject_id" => vm1.id}])
      expect(Strand).to receive(:where).and_return(instance_double(Sequel::Dataset, all: [health_probe]))
      expect(health_probe).to receive(:destroy)
      lb.detach_vm(vm1)
      expect(DB[:load_balancers_vms].first[:state]).to eq("evacuating")
    end
  end
  ...
  describe "remove_vm" do
    ...
    it "deletes the vm" do
      lb.remove_vm(vm1)
      expect(DB[:load_balancers_vms].count).to eq(0)
    end
  end
end
