...
RSpec.describe LoadBalancerVmPort do
  subject(:lb_vm_port) {
    ...
    load_balancer_ports = [{"src_port" => 80, "dst_port" => 8080}]
    lb = Prog::Vnet::LoadBalancerNexus.assemble_with_multiple_ports(private_subnet.id, name: "test-lb", load_balancer_ports:).subject
    lb.add_cert(cert)
    lb.add_vm(vm)
    lb.vm_ports.first
  }
  ...
  describe "#health_probe" do
    ...
    describe "health_check_cmd" do
      it "returns the correct command" do
        ...
        expect(lb_vm_port.health_check_cmd(:ipv4)).to eq("sudo ip netns exec #{vm.inhost_name} nc -z -w 15 192.168.1.1 8080 && echo 200 || echo 400")
        expect(lb_vm_port.health_check_cmd(:ipv6)).to eq("sudo ip netns exec #{vm.inhost_name} nc -z -w 15 2a01:4f8:10a:128b:814c::2 8080 && echo 200 || echo 400")
