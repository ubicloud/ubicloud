...
RSpec.describe LoadBalancerVmPort do
  subject(:lb_vm_port) {
    ...
    lb.add_cert(cert)
    lb.add_vm(vm)
    lb.vm_ports.find { |vm_port| vm_port.stack == "ipv4" }
  }
  ...
  describe "#health_probe" do
    ...
    describe "#check_probe" do
      ...
      it "raises an exception if health check type is ipv4 but the load balancer is not ipv4 enabled" do
        lb_vm_port.load_balancer_port.load_balancer.update(stack: "ipv6")
        expect { lb_vm_port.check_probe(nil, :ipv4) }.to raise_error("This entity should not exist: #{lb_vm_port.ubid}")
      end

      it "raises an exception if health check type is ipv6 but the load balancer is not ipv6 enabled" do
        lb_vm_port.load_balancer_port.load_balancer.update(stack: "ipv4")
        expect { lb_vm_port.check_probe(nil, :ipv6) }.to raise_error("This entity should not exist: #{lb_vm_port.ubid}")
      end
    ...
    describe "#check_pulse" do
      ...
      it "doesn't perform update if the state is down, the pulse is up, but the reading count is above the threshold and the reading change is too recent" do
        lb_vm_port.update(state: "down", stack: "ipv4")
        lb_vm_port.load_balancer_port.load_balancer.update(health_check_down_threshold: 3)
        lb_vm_port.load_balancer_port.load_balancer.update(health_check_interval: 30)
        expect(lb_vm_port).to receive(:check_probe).with(session, :ipv4).and_return("up")
        expect(lb_vm_port).not_to receive(:update)
