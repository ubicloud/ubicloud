...
RSpec.describe Vm do
  ...
  describe "#add_vm_firewall GCP cap" do
    let(:project_id) { Project.create(name: "fw-cap").id }
    let(:gcp_location) {
      Location.create(name: "gcp-us-central1", provider: "gcp",
        display_name: "GCP US Central 1", ui_name: "GCP US Central 1",
        visible: true, project_id:)
    }
    let(:gcp_ps) {
    ...
    let(:gcp_vm) {
      v = create_vm(project_id:, location_id: gcp_location.id, name: "gcp-vm")
    ...
    def gcp_fw(suffix)
      Firewall.create(name: "vm-fw-#{suffix}", description: "d",
        location_id: gcp_location.id, project_id:)
    end
    ...
    it "allows VM-level firewalls on non-GCP VMs beyond the GCP cap" do
      hetzner_ps = PrivateSubnet.create(name: "hz-ps", location_id: Location::HETZNER_FSN1_ID,
        project_id:, net6: "fd10:9b0b:6b4b:1fff::/64", net4: "10.8.0.0/26", state: "active")
      hz_vm = create_vm(project_id:, location_id: Location::HETZNER_FSN1_ID, name: "hz-vm")
