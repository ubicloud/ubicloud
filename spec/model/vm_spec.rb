...
RSpec.describe Vm do
  ...
  describe "#display_state" do
    let(:real_vm) {
      ...
      Strand.create_with_id(v.id, prog: "Vm::Nexus", label: "wait")
      v
    }

    it "returns deleting if destroy semaphore increased" do
      real_vm.incr_destroy
      expect(real_vm.display_state).to eq("deleting")
    end

    it "returns restarting if restart semaphore increased" do
      real_vm.incr_restart
      expect(real_vm.display_state).to eq("restarting")
    end

    it "returns stopped if stop semaphore increased" do
      real_vm.incr_stop
      expect(real_vm.display_state).to eq("stopped")
    end

    it "returns waiting for capacity if semaphore increased" do
      real_vm.incr_waiting_for_capacity
      expect(real_vm.display_state).to eq("waiting for capacity")
    end

    it "returns no capacity available if it's waiting capacity more than 15 minutes" do
      real_vm.update(created_at: Time.now - 16 * 60)
      real_vm.incr_waiting_for_capacity
      expect(real_vm.display_state).to eq("no capacity available")
    end
  ...
  describe "#update_spdk_version" do
    ...
    it "can update spdk version" do
      ...
      VmStorageVolume.create(vm_id: test_vm.id, boot: true, size_gib: 20, disk_index: 0)

      test_vm.update_spdk_version("b")

      expect(test_vm.vm_storage_volumes.first.spdk_installation_id).to eq(spdk_installation.id)
      expect(test_vm.reload.update_spdk_dependency_set?).to be true
    end

    it "fails if spdk installation not found" do
      expect { test_vm.update_spdk_version("b") }.to raise_error RuntimeError, "SPDK version b not found on host"
    end
  end
  ...
  describe "#utility functions" do
    ...
    it "can compute the ipv4 addresses" do
      test_vm = create_vm(project_id: project.id)
      AssignedVmAddress.create(dst_vm_id: test_vm.id, ip: "1.1.1.0/32")
      expect(test_vm.ip4_string).to eq("1.1.1.0")
    end

    it "can compute nil if ipv4 is not assigned" do
      test_vm = create_vm(project_id: project.id)
      expect(test_vm.ip4).to be_nil
    end

    it "can compute the ipv6 addresses for non-aws location" do
      test_vm = create_vm(project_id: project.id, location_id: Location::HETZNER_FSN1_ID, ephemeral_net6: "2001:db8::/64")
      expect(test_vm.ip6_string).to eq("2001:db8::2")

      test_vm.update(ephemeral_net6: nil)
      expect(test_vm.ip6_string).to be_nil
    end
    ...
    it "can compute the ipv6 addresses for aws location" do
      ...
      test_vm = create_vm(project_id: project.id, location_id: aws_location.id, ephemeral_net6: "2001:db8::/128")
      expect(test_vm.ip6_string).to eq("2001:db8::")

      test_vm.update(ephemeral_net6: nil)
      expect(test_vm.ip6).to be_nil
    end
    ...
    it "returns the right private_ipv4 based on the netmask" do
      ...
      test_vm = create_vm(project_id: project.id, name: "vm-ipv4-test-1")
      nic.update(vm_id: test_vm.id)
      expect(test_vm.private_ipv4.to_s).to eq("192.168.12.13")
      ...
      test_vm2 = create_vm(project_id: project.id, name: "vm-ipv4-test-2")
      nic2.update(vm_id: test_vm2.id)
      expect(test_vm2.private_ipv4.to_s).to eq("10.10.240.1")
    end
  end

  it "initiates a new health monitor session" do
    vmh = create_vm_host
    test_vm = create_vm(vm_host_id: vmh.id)
    sshable = test_vm.vm_host.sshable
    expect(sshable).to receive(:start_fresh_session)
    test_vm.init_health_monitor_session
  end
  ...
  describe "#check_pulse" do
    ...
    it "returns up when both services are active" do
      ...
      expect(pulse_vm.check_pulse(session: session, previous_pulse: pulse)[:reading]).to eq("up")
    end

    it "returns down and increments checkup when a service is inactive" do
      expect(session[:ssh_session]).to receive(:_exec!).and_return("active\ninactive\n")
      result = pulse_vm.check_pulse(session: session, previous_pulse: pulse)
  ...
  it "includes init_script in params_json if set" do
    ...
    test_vm = Prog::Vm::Nexus.assemble("a a", project_id).subject
    test_vm.update(vm_host_id: vmh.id)

    expect(JSON.parse(test_vm.params_json)["init_script"]).to eq ""
    VmInitScript.create_with_id(test_vm, init_script: "c")
    expect(JSON.parse(test_vm.reload.params_json)["init_script"]).to eq "c"
  end
  ...
  describe "#save_with_ephemeral_net6_error_retrying" do
    ...
    let(:real_vm_host) { create_vm_host }

    before do
      allow(real_vm_host).to receive(:ip6_random_vm_network).and_return(NetAddr::IPv6Net.parse("fd80:1:2:3::/64"))
    end

    it "saves object if there are no exceptions raised" do
      vm.ephemeral_net6 = real_vm_host.ip6_random_vm_network.to_s
      expect(real_vm_host).not_to receive(:ip6_random_vm_network)
      vm.save_with_ephemeral_net6_error_retrying(real_vm_host)
    ...
    it "retries if there is an Sequel::ValidationFailed exceptions where the only error is for ephemeral_net6" do
      vm2.ephemeral_net6 = vm.ephemeral_net6 = real_vm_host.ip6_random_vm_network.to_s
      vm2.save_changes
      expect(real_vm_host).to receive(:ip6_random_vm_network).and_return(NetAddr::IPv6Net.parse("fd80:1:2:4::/64"))
      vm.save_with_ephemeral_net6_error_retrying(real_vm_host)
    ...
    it "raises for non-Sequel::ValidationFailed exceptions" do
      ...
      expect { vm.save_with_ephemeral_net6_error_retrying(real_vm_host) }.to raise_error(Sequel::NoExistingObject)
    end

    it "raises for Sequel::ValidationFailed exceptions for other columns" do
      vm.project_id = nil
      expect(real_vm_host).not_to receive(:ip6_random_vm_network)
      expect { vm.save_with_ephemeral_net6_error_retrying(real_vm_host) }.to raise_error(Sequel::ValidationFailed)
    end
    ...
    it "raises instead of retrying more than max_retries times" do
      ...
      vm2.save_changes
      expect(real_vm_host).not_to receive(:ip6_random_vm_network)
      expect { vm.save_with_ephemeral_net6_error_retrying(real_vm_host, max_retries: 0) }.to raise_error(Sequel::ValidationFailed)
    end
  end
  ...
  describe "#params_json" do
    ...
    let(:test_vm) { create_vm(project_id: project.id, vm_host_id: vmh.id, family: "standard", vcpus: 2) }
    let(:gpu_partition) { GpuPartition.create(vm_host_id: vmh.id, vm_id: test_vm.id, partition_id: 3, gpu_count: 1) }
    ...
    it "sets hypervisor to 'qemu' when a B200 GPU (device 2901) is present" do
      PciDevice.create(vm_host_id: vmh.id, vm_id: test_vm.id, slot: "00:00.0", device_class: "0300", vendor: "nvidia", device: "2901", numa_node: 0, iommu_group: 0)

      json = JSON.parse(test_vm.params_json)
      expect(json["hypervisor"]).to eq("qemu")
    end

    it "defaults hypervisor to 'ch' when no B200 GPU is present" do
      PciDevice.create(vm_host_id: vmh.id, vm_id: test_vm.id, slot: "00:00.0", device_class: "0300", vendor: "nvidia", device: "1234", numa_node: 0, iommu_group: 0)
    ...
    it "respects an explicit hypervisor argument even if a B200 GPU is present" do
      ...
      json = JSON.parse(test_vm.params_json(hypervisor: "ch"))
      expect(json["hypervisor"]).to eq("ch")
    end
