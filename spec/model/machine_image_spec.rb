...
RSpec.describe MachineImage do
  ...
  describe ".register_distro_image" do
    it "creates a machine image and strand" do
      vm_host = create_vm_host
      allow(Config).to receive_messages(
        machine_image_archive_bucket: "test-bucket",
        machine_image_archive_endpoint: "https://r2.example.com"
      )

      mi = described_class.register_distro_image(
        project_id:,
        location_id: Location::HETZNER_FSN1_ID,
        name: "ubuntu-noble",
      ...
      expect(mi).to be_a(MachineImage)
      expect(mi.name).to eq("ubuntu-noble")
      expect(mi.state).to eq("creating")
      expect(mi.encrypted).to be false
      expect(mi.visible).to be false
      expect(mi.arch).to eq("arm64")
      expect(mi.s3_bucket).to eq("test-bucket")
      expect(mi.s3_prefix).to include("public/ubuntu-noble/")
      expect(mi.s3_prefix).to end_with("#{mi.ubid}/")
      expect(mi.strand).not_to be_nil
      expect(mi.strand.prog).to eq("MachineImage::RegisterDistroImage")
      expect(mi.strand.label).to eq("start")
    end
  end
