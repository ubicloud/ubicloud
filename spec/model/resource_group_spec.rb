...
RSpec.describe ResourceGroup do
  ...
  describe "#to_cpu_bitmask" do
    ...
    it "resizes the bitmask to the number of cpus on the host" do
      ...
      expect(resource_group.to_cpu_bitmask.to_s).to eq("00110000")
    end
  end
  ...
  describe "#cpuset_to_bitmask_and_back" do
    it "converts disjoined cpu ranges" do
      bitmask = ResourceGroup.cpuset_to_bitmask("2-3,6-7")
      expect(ResourceGroup.bitmask_to_cpuset(bitmask)).to eq("2-3,6-7")
    end

    it "handles nil or empty cpuset" do
      expect{ ResourceGroup.cpuset_to_bitmask(nil) }.to raise_error RuntimeError, "Cpuset cannot be empty."
      expect{ ResourceGroup.cpuset_to_bitmask("") }.to raise_error RuntimeError, "Cpuset cannot be empty."
    end

    it "handles invalid cpuset" do
      expect{ ResourceGroup.cpuset_to_bitmask("1234 abcd") }.to raise_error RuntimeError, "Cpuset can only contains numbers, comma (,) , and hypen (-)."
      expect{ ResourceGroup.cpuset_to_bitmask("1234-234%") }.to raise_error RuntimeError, "Cpuset can only contains numbers, comma (,) , and hypen (-)."
      expect{ ResourceGroup.cpuset_to_bitmask("1234-234-456") }.to raise_error RuntimeError, "Unexpected list of cpus in the cpuset."
    end

    it "handles single cpu" do
      bitmask = ResourceGroup.cpuset_to_bitmask("2-3,7")
      expect(ResourceGroup.bitmask_to_cpuset(bitmask)).to eq("2-3,7")

      bitmask = ResourceGroup.cpuset_to_bitmask("7")
      expect(ResourceGroup.bitmask_to_cpuset(bitmask)).to eq("7")

      bitmask = ResourceGroup.cpuset_to_bitmask("7-7")
      expect(ResourceGroup.bitmask_to_cpuset(bitmask)).to eq("7")
    end

    it "handles incorrect cpu ranges" do
      expect{ ResourceGroup.cpuset_to_bitmask("7-4") }.to raise_error RuntimeError, "Invalid list of cpus in the cpuset."
    end
