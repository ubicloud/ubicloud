...
RSpec.describe KubernetesCluster do
  ...
  describe "#kubeconfig" do
    ...
    let(:nodes) { [KubernetesNode.create(vm_id: create_vm.id, kubernetes_cluster_id: kc.id)] }

    it "removes client certificate and key data from users and adds an RBAC token to users" do
      expect(nodes.first.vm).to receive(:sshable).and_return(sshable).twice
      expect(kc).to receive(:nodes).and_return(nodes)
  ...
  describe "vm_diff_for_lb" do
    it "finds the extra and missing nodes" do
      ...
      lb.add_vm(extra_vm)
      np = instance_double(KubernetesNodepool, nodes: [KubernetesNode.create(vm_id: missing_vm.id, kubernetes_cluster_id: kc.id)])
  ...
  describe "#all_nodes" do
    it "returns all nodes in the cluster, including control plane and worker nodes" do
  ...
  describe "#worker_nodes" do
    it "returns all worker nodes in the cluster" do
      expect(kc).to receive(:nodepools).and_return([instance_double(KubernetesNodepool, nodes: [3, 4]), instance_double(KubernetesNodepool, nodes: [5, 6])])
      expect(kc.worker_nodes).to eq([3, 4, 5, 6])
    end
  end
end
