...
RSpec.describe KubernetesNode do
  ...
  describe "#available?" do
    it "returns true when check_mesh_availability returns available" do
      expect(kn).to receive(:check_mesh_availability).and_return({available: true})
      expect(kn.available?).to be true
    end

    it "returns false when check_mesh_availability returns not available" do
      expect(kn).to receive(:check_mesh_availability).and_return({available: false})
      expect(kn.available?).to be false
    end

    it "returns false on any error" do
      expect(kn.sshable).to receive(:_cmd).and_raise(Sshable::SshError)
      expect(kn.available?).to be false
    end
  end
  ...
  describe "#check_mesh_availability" do
    it "returns available when file is empty" do
      expect(kn.sshable).to receive(:_cmd).and_return("")
    ...
    it "returns available when all pods are reachable" do
      ...
      expect(kn.sshable).to receive(:_cmd).and_return(status_json)
    ...
    it "uses ssh_session when provided" do
      expect(ssh_session).to receive(:_exec!).and_return("")
