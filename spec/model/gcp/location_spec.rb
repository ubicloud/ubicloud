  ...
  context "with GCP provider" do
    ...
    describe "#azs" do
      ...
      it "returns cached AZs when they exist" do
        LocationGcpAz.create_with_id(SecureRandom.uuid, location_id: location.id, az: "a")
        LocationGcpAz.create_with_id(SecureRandom.uuid, location_id: location.id, az: "b")
      ...
      it "fetches zones from GCP API when cache is empty" do
        ...
        expect(LocationGcpAz.where(location_id: location.id).count).to eq(5)
      end
