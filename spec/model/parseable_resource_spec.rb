...
RSpec.describe ParseableResource do
  ...
  describe ".for_project" do
    it "returns the resource matching project_id and location" do
      found = described_class.for_project(parseable_resource.project_id)
    ...
    it "falls back to parseable service project correctly" do
      ...
      expect(described_class.for_project(nil)).to eq(parseable_resource)
    end
  end

  describe ".client_for_project" do
    it "returns a nil client if no resource exists" do
      client = described_class.client_for_project(parseable_service_project.id)
      expect(client).to be_nil
    end

    it "returns a nil client if no server exists" do
      client = described_class.client_for_project(parseable_resource.project_id)
      expect(client).to be_nil
    end
