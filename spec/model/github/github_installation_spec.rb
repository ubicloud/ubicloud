...
RSpec.describe GithubInstallation do
  ...
  it "returns sum of used vm cores" do
    ...
    vms[..1].each do |vm|
      gr = GithubRunner.create(installation_id: installation.id, vm_id: vm.id, location_id: vm.location_id, repository_name: "test-repo", label: "ubicloud-standard-#{vm.cores}")
  ...
  it "returns sum of used vm cores for arm64" do
    ...
    vms.each do |vm|
      gr = GithubRunner.create(installation_id: installation.id, vm_id: vm.id, location_id: vm.location_id, repository_name: "test-repo", label: "ubicloud-standard-#{vm.cores}-arm")
      Strand.create_with_id(gr, prog: "Github::RunnerNexus", label: "allocate_vm")
    end
