  ...
  describe "#migrate_device_name_to_device_id" do
    context "when finding the disk id from device_name for SSD disks" do
      it "changes the unix_device_list to the device id and saves changes" do
        storage_device = described_class.create_with_id(vm_host_id: create_vm_host.id, name: "DEFAULT", total_storage_gib: 100, available_storage_gib: 100, unix_device_list: ["sda"])
    ...
    context "when finding the disk id from device_name for NVMe disks" do
      it "changes the unix_device_list to the device id and saves changes" do
        storage_device = described_class.create_with_id(vm_host_id: create_vm_host.id, name: "DEFAULT", total_storage_gib: 100, available_storage_gib: 100, unix_device_list: ["nvme0n1"])
