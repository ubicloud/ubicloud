...
RSpec.describe ResourceCredit do
  ...
  describe "check constraints" do
    it "rejects resource_id without resource_type" do
      ...
      expect {
        DB[:resource_credit].insert(
          ...
          initial_amount: 20, amount: 10, active_from: Time.utc(2026, 1, 1),
        )
    ...
    it "rejects initial_amount below or at 0" do
      expect { create(amount: -1) }.to raise_error(Sequel::ValidationFailed)
      expect { create(amount: 0) }.to raise_error(Sequel::ValidationFailed)
    end

    it "allows initial_amount of 1" do
      expect { create(amount: 1) }.not_to raise_error
    end
    ...
    it "allows amount of 0" do
      rc = create(amount: 1)
      expect { rc.update(amount: 0) }.not_to raise_error
    end
    ...
    it "rejects non-month-aligned values at the database level when validations are bypassed" do
      expect {
        DB[:resource_credit].insert(
          ...
          initial_amount: 10, amount: 10, active_from: Time.utc(2026, 1, 15),
        )
  ...
  describe ".active_project_ids_ds" do
    ...
    it "excludes projects whose credit is exhausted" do
      create(amount: 1).update(amount: 0)
