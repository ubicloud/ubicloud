...
RSpec.describe InferenceEndpoint do
  subject(:inference_endpoint) do
    ie_id = described_class.generate_uuid
    described_class.create_with_id(ie_id,
      name: "ie-name",
  ...
  describe "#display_states" do
    let!(:strand) { Strand.create_with_id(inference_endpoint.id, prog: "Ai::InferenceEndpointNexus", label: "wait") }
