...
RSpec.describe Page do
  ...
  describe ".group_by_vm_host" do
    it "groups pages by the VmHost they are related to" do
      ...
      p1 = described_class.create_with_id(tag: "a")
      expect(described_class.group_by_vm_host).to eq({nil => [p1]})

      p2 = described_class.create_with_id(tag: "b", details: {"related_resources" => []})
      expect(described_class.order(:tag).group_by_vm_host).to eq({nil => [p1, p2]})

      p3 = described_class.create_with_id(tag: "c", details: {"related_resources" => [p2.ubid]})
      expect(described_class.order(:tag).group_by_vm_host).to eq({nil => [p1, p2, p3]})

      vmh = Prog::Vm::HostNexus.assemble("1.2.3.4").subject
      p4 = described_class.create_with_id(tag: "d", details: {"related_resources" => [vmh.ubid]})
      ...
      p5 = described_class.create_with_id(tag: "e", details: {"related_resources" => [p3.ubid, vm.ubid]})
      ...
      p6 = described_class.create_with_id(tag: "f", details: {"related_resources" => [vm.nic.ubid]})
      ...
      p7 = described_class.create_with_id(tag: "g", details: {"related_resources" => [gr.ubid]})
