...
RSpec.describe VmStorageVolume do
  ...
  it "can render a device_path for gcp with persistent disk" do
    prj = Project.create(name: "test-project-gcp")
    vm = Vm.new(family: "burstable", vcpus: 2, location: Location.create(name: "us-central1", provider: "gcp", project_id: prj.id, display_name: "gcp-us-central1", ui_name: "GCP US Central 1", visible: true)).tap { it.id = "eb3dbcb3-2c90-8b74-8fb4-d62a244d7ae5" }
    expect(described_class.new(disk_index: 1, boot: false, vm:).device_path).to eq("/dev/disk/by-id/google-persistent-disk-1")
  end

  it "can render a device_path for gcp with local NVMe SSD" do
    prj = Project.create(name: "test-project-gcp-lssd")
    vm = Vm.new(family: "standard", vcpus: 8, location: Location.create(name: "us-central1-lssd", provider: "gcp", project_id: prj.id, display_name: "gcp-us-central1", ui_name: "GCP US Central 1", visible: true)).tap { it.id = "eb3dbcb3-2c90-8b74-8fb4-d62a244d7ae5" }
    expect(described_class.new(disk_index: 1, boot: false, vm:).device_path).to eq("/dev/disk/by-id/google-local-nvme-ssd-0")
  end

  it "can render a boot disk device_path for gcp with LSSD machine type" do
    prj = Project.create(name: "test-project-gcp-lssd-boot")
    vm = Vm.new(family: "standard", vcpus: 8, location: Location.create(name: "us-central1-boot", provider: "gcp", project_id: prj.id, display_name: "gcp-us-central1", ui_name: "GCP US Central 1", visible: true)).tap { it.id = "eb3dbcb3-2c90-8b74-8fb4-d62a244d7ae5" }
