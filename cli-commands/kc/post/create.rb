...
UbiCli.on("kc").run_on("create") do
  ...
  options("ubi kc location/cluster-name create [options]", key: :kc_create) do
    on("-v", "--version=version", "Kubernetes version")
    on("-c", "--cp-node-count=count", "Control plane node count")
    on("-w", "--worker-node-count=count", "Worker node count")
    on("-z", "--worker-size=size", "Worker size")
  end
  help_option_values("Version:", Option.kubernetes_versions)
  help_option_values("Control Plane Node Count:", Option::KubernetesCPOptions.map { |x| x.cp_node_count.to_s })
