...
require_relative "lib/aws/rds_iam_auth" if Config.clover_database_auth_type == :aws_rds_iam
...
clover_database_url = (Config.clover_database_auth_type == :aws_rds_iam) ? substitute_rds_iam_auth_token(Config.clover_database_url) : Config.clover_database_url
DB = Sequel.connect(clover_database_url, max_connections:, pool_timeout: Config.database_timeout, treat_string_list_as_untyped_array: true, pg_auto_parameterize_min_array_size:)
...
postgres_monitor_database_url = (Config.postgres_monitor_database_url == :aws_rds_iam) ? substitute_rds_iam_auth_token(Config.postgres_monitor_database_url) : Config.postgres_monitor_database_url
begin
  POSTGRES_MONITOR_DB = Sequel.connect(postgres_monitor_database_url, max_connections: Config.db_pool_monitor, pool_timeout: Config.database_timeout) if Config.postgres_monitor_database_url
