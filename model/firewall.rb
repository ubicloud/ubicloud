...
class Firewall < Sequel::Model
  ...
  def remove_firewall_rule(firewall_rule, request_id: nil)
    firewall_rules_dataset.where(id: firewall_rule.id).destroy
    update_private_subnet_firewall_rules(request_id)
  end

  def insert_firewall_rule(cidr, port_range, description: nil, request_id: nil)
    fwr = add_firewall_rule(cidr:, port_range:, description:)
    update_private_subnet_firewall_rules(request_id)
    fwr
  end

  def replace_firewall_rules(new_firewall_rules, request_id: nil)
  ...
  def associate_with_private_subnet(private_subnet, apply_firewalls: true, request_id: nil)
    add_private_subnet(private_subnet)
    private_subnet.incr_update_firewall_rules(request_id) if apply_firewalls
  end

  def disassociate_from_private_subnet(private_subnet, apply_firewalls: true, request_id: nil)
    remove_private_subnet(private_subnet)
    private_subnet.incr_update_firewall_rules(request_id) if apply_firewalls
  end

  def update_private_subnet_firewall_rules(request_id = nil)
    private_subnets.each { it.incr_update_firewall_rules(request_id) }
  end
end
