...
class LoadBalancer < Sequel::Model
  ...
  many_through_many :vms_to_dns, [[:load_balancer_port, :load_balancer_id, :id], [:load_balancer_vm_port, :load_balancer_port_id, :load_balancer_vm_id], [:load_balancers_vms, :id, :vm_id]], class: :Vm, conditions: {Sequel[:load_balancer_vm_port][:state] => "up"}
  ...
  def add_port(port_info)
    DB.transaction do
      port = LoadBalancerPort.create(load_balancer_id: id, src_port: port_info[0], dst_port: port_info[1])
