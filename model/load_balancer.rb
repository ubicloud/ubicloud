...
class LoadBalancer < Sequel::Model
  many_to_one :project
  many_to_many :vms
  many_to_many :active_vm_ports, class: :Vm, left_key: :load_balancer_id, right_key: :vm_id, join_table: :load_balancer_vm_port, conditions: {state: ["up"]}
  many_to_many :vms_to_dns, class: :Vm, left_key: :load_balancer_id, right_key: :vm_id, join_table: :load_balancer_vm_port, conditions: Sequel.~(state: ["evacuating", "detaching"])
  ...
  plugin :association_dependencies, ports: :destroy, certs_load_balancers: :destroy
  # plugin :association_dependencies, load_balancer_vm_port: :destroy, certs_load_balancers: :destroy, load_balancer_port: :destroy
  ...
  def add_vm(vm)
    DB.transaction do
      ...
      ports.map { |port| LoadBalancerVmPort.create_with_id(vm_id: vm.id, load_balancer_id: id, load_balancer_port_id: port.id) }
  ...
  def detach_vm(vm)
    vm_ports_dataset.where(vm_id: vm.id, state: ["up", "down", "evacuating"]).update(state: "detaching")
  ...
  def evacuate_vm(vm)
    DB.transaction do
      vm_ports_dataset.where(vm_id: vm.id, state: ["up", "down"]).update(state: "evacuating")
      Strand.create_with_id(prog: "Vnet::CertServer", label: "remove_cert_server", stack: [{subject_id: id, vm_id: vm.id}], parent_id: id)
      incr_update_load_balancer
      incr_rewrite_dns_records
    end
  ...
  def remove_vm(vm)
    vm_ports_dataset[vm_id: vm.id].destroy
    incr_rewrite_dns_records
  end
