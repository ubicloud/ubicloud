...
class LoadBalancer < Sequel::Model
  ...
  def need_certificates?
    ...
    certs.all? { |lbc| lbc.created_at && lbc.created_at < Time.now - 60 * 60 * 24 * 30 * 2 }
  end

  def active_cert
    certs.sort_by(&:created_at).reverse.find { |lbc| lbc.created_at && lbc.created_at > Time.now - 60 * 60 * 24 * 30 * 3 }
  end
end
