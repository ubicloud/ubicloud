...
class VmHost < Sequel::Model
  ...
  def check_storage_smartctl(ssh_session, devices)
    devices.map do |device_name|
      ...
      Clog.emit("Device #{device_name} failed smartctl check.") unless passed
      passed
    end.all?(true)
  end
  ...
  def check_storage_nvme(ssh_session, devices)
    devices.map do |device_name|
      ...
      Clog.emit("Device #{device_name} failed nvme smart-log check.") unless passed
      passed
    end.all?(true)
  end
  ...
  def check_storage_read_write(ssh_session, devices)
    ...
    all_mount_points.uniq.all? do |mount_point|
      ...
      Clog.emit("failed to perform read/write on mountpoint #{mount_point}") unless working
      working
    end
  end
  ...
  def check_storage_kernel_logs(ssh_session, devices)
    ...
    Clog.emit("found error on kernel logs. devices with error_count: #{error_count}") unless error_count.empty?
    error_count.empty?
  end
