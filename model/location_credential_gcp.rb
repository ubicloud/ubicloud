...
class LocationCredentialGcp < Sequel::Model
  ...
  def zones_client
    @zones_client ||= V1::Zones::Rest::Client.new(&config_block)
  end

  def subnetworks_client
    @subnetworks_client ||= V1::Subnetworks::Rest::Client.new(&config_block)
  end

  def zone_operations_client
    @zone_operations_client ||= V1::ZoneOperations::Rest::Client.new(&config_block)
  end

  def region_operations_client
    @region_operations_client ||= V1::RegionOperations::Rest::Client.new(&config_block)
  end

  def global_operations_client
    @global_operations_client ||= V1::GlobalOperations::Rest::Client.new(&config_block)
  end

  def addresses_client
    @addresses_client ||= V1::Addresses::Rest::Client.new(&config_block)
  end
  ...
  def networks_client
    @networks_client ||= V1::Networks::Rest::Client.new(&config_block)
  end
  ...
  def config_block
    @config_block ||= proc { |config| config.credentials = auth_credentials }
  end
  ...
  def auth_credentials
    @auth_credentials ||= if credentials_json
      ...
      # source_credentials, not base_credentials: base_credentials re-scopes the
      # ADC to IAM_SCOPE, which yields no token under GKE Workload Identity and
      # fails generateAccessToken with 401 CREDENTIALS_MISSING.
