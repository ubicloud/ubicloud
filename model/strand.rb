...
SQL
      ...
      Clog.emit("exception terminates strand run", Util.exception_to_hash(ex, into: {strand_error: {strand: ubid, duration:, from: prog_label}}))
    ...
    duration = Time.now - start_time
    Clog.emit("finished strand", {strand_finished: {strand: ubid, duration:, prog_label:}}) if duration > 1
  end
