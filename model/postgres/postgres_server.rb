...
class PostgresServer < Sequel::Model
  ...
  def observe_replica_lag(session)
    ...
    replay_lsn, replay_age = run_query("SELECT pg_last_wal_replay_lsn(), EXTRACT(EPOCH FROM (NOW() - pg_last_xact_replay_timestamp()))::int", user: "ubi_monitoring", dbname: "ubi_admin").split(",")
