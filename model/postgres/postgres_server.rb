...
class PostgresServer < Sequel::Model
  ...
  def restart_sensitive_target_values
    ...
    config = resource.user_config
    RESTART_SENSITIVE_PARAMS.to_h { |param| [param, (config[param] || validator.default(param)).to_i] }
  end

  def restart_sensitive_running_values
    rows = run_query(DB[:pg_settings].where(name: RESTART_SENSITIVE_PARAMS).select(:name, :setting)).split("\n")
