...
class Sshable < Sequel::Model
  ...
  def daemonizer_check(unit_name)
    cmd("common/bin/daemonizer2 check #{unit_name}")
  end

  def daemonizer_clean(unit_name)
    cmd("common/bin/daemonizer2 clean #{unit_name}")
  end

  def daemonizer_run(unit_name, run_command, stdin: nil)
    cmd("common/bin/daemonizer2 run #{unit_name} #{run_command}", stdin:)
  end
