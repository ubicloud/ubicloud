...
class GcpInstanceAvailabilityGenerator
  ...
  # A family present in only some zones of a region is still offered there, but
  ...
  def warn_partial_coverage(found)
    found.sort.each do |region, families|
      ...
      families.sort.each do |family, vcpus|
        family_zones = vcpus.values.flatten.uniq
        next if family_zones.size == zones.size

        log "  partial: #{family} in #{region} covers #{family_zones.size} of #{zones.size} zones (#{family_zones.sort.join(", ")})"
      end
    end
  end
end
...
if __FILE__ == $0
  ...
  credentials_path = credentials_flags.last&.delete_prefix("--credentials=")
