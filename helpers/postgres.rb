...
class Clover
  ...
  def validate_postgres_network_storage(flavor, storage_type, network_volume_size_gib, network_volume_iops, network_volume_throughput_mibps)
    ...
    # Apply PostgreSQL's product floor separately from provider limits.
    min_size_gib = Config.postgres_network_volume_min_size_gib
    if network_volume_size_gib < min_size_gib
      fail Validation::ValidationFailed.new({network_volume_size_gib: "network_volume_size_gib must be at least #{min_size_gib} GiB"})
    end
