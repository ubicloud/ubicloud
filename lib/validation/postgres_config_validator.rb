  ...
  class PostgresConfigValidator
    PG_17_CONFIG_SCHEMA = {
      ...
      "config_file" => {
        ...
        type: :string,
        default: "/etc/postgresql/17/main/postgresql.conf"
      },
      "constraint_exclusion" => {
        description: "Enables the planner to use constraints to optimize queries.",
        type: :enum,
        allowed_values: ["on", "off", "partition"],
        default: "partition"
      },
    ...
    def initialize(version)
      case version
      when "17"
        @config_schema = PG_17_CONFIG_SCHEMA
      when "16"
        @config_schema = PG_16_CONFIG_SCHEMA
      when "pgbouncer"
        @config_schema = PGBOUNCER_CONFIG_SCHEMA
      else
