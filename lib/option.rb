...
module Option
  ...
  VmSize = Struct.new(:name, :family, :cores, :vcpu, :vcpu_percent_limit, :vcpu_burst_percent_limit, :memory, :storage_size_options, :visible, :gpu, :arch) do
    alias_method :display_name, :name
  end
  VmSizes = [2, 4, 8, 16, 30, 60].map {
    storage_size_options = [_1 * 20, _1 * 40]
    VmSize.new("standard-#{_1}", "standard", _1 / 2, _1, _1 * 100, 0, _1 * 4, storage_size_options, true, false, "x64")
  }.concat([2, 4, 8, 16, 30, 60].map {
    storage_size_options = [_1 * 20, _1 * 40]
    VmSize.new("standard-#{_1}", "standard", _1, _1, _1 * 100, 0, (_1 * 3.2).to_i, storage_size_options, false, false, "arm64")
  }).concat([6].map {
    VmSize.new("standard-gpu-#{_1}", "standard-gpu", _1 / 2, _1, _1 * 100, _1, (_1 * 5.34).to_i, [_1 * 30], false, true, "x64")
  }).concat([[2, 50], [2, 100]].map {
    storage_size_options = [_1[1] * 20 / 100, _1[1] * 40 / 100]
    VmSize.new("burstable-#{_1[0]}-#{_1[1]}", "burstable", _1[0] / 2, _1[0], _1[1], _1[1], _1[1] * 4 / 100, storage_size_options, false, false, "x64")
  }).concat([[2, 50], [2, 100]].map {
    storage_size_options = [_1[1] * 20 / 100, _1[1] * 40 / 100]
    VmSize.new("burstable-#{_1[0]}-#{_1[1]}", "burstable", _1[0], _1[0], _1[1], _1[1], (_1[1] * 3.2 / 100).to_i, storage_size_options, false, false, "arm64")
  }).freeze
