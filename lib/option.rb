...
module Option
  ...
  VmSize = Struct.new(:name, :family, :vcpu, :vcpu_percent_limit, :vcpu_burst_percent_limit, :memory, :storage_size_options, :visible, :gpu, :arch) do
    alias_method :display_name, :name
  end
  VmSizes = [2, 4, 8, 16, 30, 60].map {
    storage_size_options = [_1 * 20, _1 * 40]
    VmSize.new("standard-#{_1}", "standard", _1, _1 * 100, 0, _1 * 4, storage_size_options, true, false, "x64")
