...
module Option
  ...
  def self.locations(only_visible: true, feature_flags: [])
    ProviderLocation.all.select { |pl| !only_visible || (pl.visible || feature_flags.include?("location_#{pl.internal_name.tr("-", "_")}")) }
  end

  def self.postgres_locations
    ProviderLocation.all.select { |pl| pl.internal_name == "hetzner-fsn1" || pl.internal_name == "leaseweb-wdc02" }
  end
