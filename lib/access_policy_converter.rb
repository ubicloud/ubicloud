...
module AccessPolicyConverter
  ...
  def self.convert_to_tag(klass, project_id, name, values, failures)
    ...
    unless issues.empty?
      failures << [:member_check_failed, project_id, tag_name, klass.name, issues, old_values - values]
    end
    [tag, values]
  end
  ...
  def self.parse_access_policy_rows(rows)
    ...
    rows.each do |row|
      ...
      acls.each_with_index do |acl, i|
        ...
        if subject_id.empty?
          failures << [:no_valid_subjects, project_id, ap_id]
          next
        end

        ace_hash = projects[project_id] ||= {admin: [], member: [], aces: []}
        if is_admin || (acls[0]["actions"] == admin_array && acls.length == 1)
