...
module Validation
  ...
  def self.validate_network_volume_config(volume_type, size_gib, iops, throughput_mibps)
    ...
    unless size_gib.is_a?(Integer) && size_gib.between?(limits.min_size_gib, limits.max_size_gib)
      fail ValidationFailed.new({network_volume_size_gib: "Size for #{volume_type} must be between #{limits.min_size_gib} and #{limits.max_size_gib} GiB."})
    end
