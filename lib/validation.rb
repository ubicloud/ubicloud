...
module Validation
  ...
  def self.validate_network_volume_config(volume_type, size_gib, iops, throughput_mibps)
    ...
    # The product floor excludes Hyperdisk's fixed-performance 4–5 GiB sizes.
    min_size_gib = Config.postgres_network_volume_min_size_gib
    unless size_gib.is_a?(Integer) && size_gib.between?(min_size_gib, limits.max_size_gib)
      fail ValidationFailed.new({network_volume_size_gib: "Size for #{volume_type} must be between #{min_size_gib} and #{limits.max_size_gib} GiB."})
    end
    ...
    # Apply ratio limits to explicit settings and defaults alike.
