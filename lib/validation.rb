...
module Validation
  ...
  def self.validate_postgres_cert_auth_user_name(name)
    ...
    if ["postgres", "ubi_replication"].include?(name)
      fail ValidationFailed.new({name: "Name must not be postgres or ubi_replication."})
    end
  end
  ...
  # system.
  def self.postgres_max_shared_memory_percent(memory_gib)
    (memory_gib <= 16) ? 50 : 75
  end
