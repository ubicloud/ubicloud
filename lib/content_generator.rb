...
module ContentGenerator
  ...
  module Postgres
    ...
    def self.wal_drive_type(flavor, location, family, vm_size, storage_type, network_volume_type, wal_drive_type)
      # Name and price the sentinel after the type the server would pick.
      if wal_drive_type == Option::POSTGRES_WAL_DRIVE_TYPE_DEFAULT
        resolved = PostgresResource.default_wal_drive_type(location, storage_type)
        label = "Default (#{Option::POSTGRES_WAL_DRIVE_TYPE_OPTIONS[resolved].description})"
      else
        resolved = wal_drive_type
        label = Option::POSTGRES_WAL_DRIVE_TYPE_OPTIONS[wal_drive_type].description
      end
      # Missing rates, such as NVMe WAL, are free.
      unit_price = BillingRate.unit_price_from_resource_properties("PostgresWalDrive", resolved, location.name, location.byoc).to_f
