...
class BillingRate
  ...
  def self.line_item_description(resource_type, resource_family, amount)
    ...
    when "PostgresWalDrive"
      "#{resource_family} WAL Drive for PostgreSQL Database"
    when "PostgresStandbyWalDrive"
      "#{resource_family} WAL Drive for PostgreSQL Database (HA Standby)"
