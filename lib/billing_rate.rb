...
class BillingRate
  ...
  def self.line_item_description(resource_type, resource_family, amount)
    ...
    when "PostgresCpuPercent"
      "#{(amount * 100).to_i}% of CPU for #{resource_family} backed PostgreSQL Database"
    when "PostgresStandbyCores"
      "#{resource_family}-#{(amount * 2).to_i} backed PostgreSQL Database (HA Standby)"
    when "PostgresStandbyCpuPercent"
      "#{(amount * 100).to_i}% of CPU for #{resource_family} backed PostgreSQL Database (HA Standby)"
