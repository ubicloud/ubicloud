...
class Kubernetes::Client
  ...
  def kubectl(cmd, **)
    raise "can only pass frozen string as kubectl cmd argument" unless cmd.frozen?
    output = @session.exec!("sudo kubectl --kubeconfig=/etc/kubernetes/admin.conf #{cmd}".freeze, **)
