...
to carry a TLL that does not fit. The program never drops: every
non-answer path returns `TC_ACT_UNSPEC` so the stack still sees the
packet, and when it does answer, the redirect consumes the NS so static
proxy entries cannot double-answer.
...
`ndp_needed`) waits for `LearnNetwork` to fill `net6` in; if that
finishes empty it pages and pops, so prep completes without the proxy
and the page resolves once a later install succeeds. It then runs
`host/bin/setup-ndp-proxy install <net6>`, which refuses kernels older
than 6.6 (TCX links need them; host-ebpf checks again at apply),
downloads the pinned host-ebpf release, verifies its SHA-256 the way
`CloudHypervisor` does, and writes and enables the units below. Both
units run under `/run/lock/ndp-proxy.lock` with a ten-minute start
timeout, so an overlapping boot apply, watch tick, or reinstall
serializes and a stalled download cannot wedge the timer.
...
- `ndp-proxy-watch.timer`, and the `ndp-proxy-watch.service` it
  ...
  it is about to lose, re-applies, and exits non-zero so the drift is
  recorded as a failed run in `journalctl -u ndp-proxy-watch`. The next
  clean tick resets the unit's state.
...
rhizome puts the new binary on hosts provisioned before the bump. The
program already attached keeps running until the next boot or a
`systemctl restart ndp-proxy.service`: `verify` compares the
attachment's configuration, not the program's identity.
...
Hosts provisioned before this landed never re-run `prep`. Install the
proxy on them with a detached strand per host:

```ruby
Strand.create(prog: "SetupNdpProxy", label: "start", stack: [{subject_id: vm_host.id}])
```

It pages if the host has no `net6`, and its ten-minute deadline pages
if the install keeps failing.
