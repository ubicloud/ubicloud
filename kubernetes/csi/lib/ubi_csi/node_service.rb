    ...
    class NodeService < Node::Service
      ...
      def node_get_info(req, _call)
        ...
        topology = Topology.new(
          segments: {
            "kubernetes.io/hostname" => ENV["NODE_ID"]
          }
        )
      ...
      def node_stage_volume(req, _call)
        ...
        begin
          unless File.exist?(backing_file)
            ...
            unless ok
              log_with_id(req_id, "[CSI NodeService] gRPC error in node_stage_volume: failed to fallocate: #{output}")
            ...
            unless ok
              log_with_id(req_id, "[CSI NodeService] gRPC error in node_stage_volume: failed to punchhole: #{output}")
          ...
          if req.volume_capability&.mount
            ...
            unless ok
              log_with_id(req_id, "[CSI NodeService] gRPC error in node_stage_volume: failed to mount loop device: #{output}")
              raise GRPC::Internal, "Failed to mount #{loop_device} to #{staging_path}: #{output}"
            end
          end
          # If block, do nothing else
        rescue GRPC::BadStatus => e
          log_with_id(req_id, "[CSI NodeService] gRPC error in node_stage_volume: #{e.class} - #{e.message}")
          raise e
        rescue => e
          log_with_id(req_id, "[CSI NodeService] Internal error in node_stage_volume: #{e.class} - #{e.message}\n#{e.backtrace.join("\n")}")
      ...
      def node_unstage_volume(req, _call)
        ...
        begin
          if is_mounted?(staging_path)
            ...
            unless ok
              log_with_id(req_id, "[CSI NodeService] gRPC error in node_unstage_volume: failed to umount device: #{output}")
          ...
          log_with_id(req_id, "[CSI NodeService] gRPC error in node_unstage_volume: #{e.class} - #{e.message}")
          raise e
        rescue => e
          log_with_id(req_id, "[CSI NodeService] Internal error in node_unstage_volume: #{e.class} - #{e.message}\n#{e.backtrace.join("\n")}")
      ...
      def node_publish_volume(req, _call)
        ...
        begin
          ...
          unless ok
            log_with_id(req_id, "[CSI NodeService] gRPC error in node_publish_volume: failed to bind mount device: #{output}")
            raise GRPC::Internal, "Failed to bind mount #{staging_path} to #{target_path}: #{output}"
          end
        rescue GRPC::BadStatus => e
          log_with_id(req_id, "[CSI NodeService] gRPC error in node_publish_volume: #{e.class} - #{e.message}")
          raise e
        rescue => e
          log_with_id(req_id, "[CSI NodeService] Internal error in node_publish_volume: #{e.class} - #{e.message}\n#{e.backtrace.join("\n")}")
      ...
      def node_unpublish_volume(req, _call)
        ...
        begin
          if is_mounted?(target_path)
            ...
            unless ok
              log_with_id(req_id, "[CSI NodeService] gRPC error in node_unpublish_volume: failed to umount device: #{output}")
          ...
          log_with_id(req_id, "[CSI NodeService] gRPC error in node_unpublish_volume: #{e.class} - #{e.message}")
          raise e
        rescue => e
          log_with_id(req_id, "[CSI NodeService] Internal error in node_unpublish_volume: #{e.class} - #{e.message}\n#{e.backtrace.join("\n")}")
