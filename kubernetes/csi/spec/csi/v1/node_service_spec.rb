...
RSpec.describe Csi::V1::NodeService do
  ...
  describe "perform_node_stage_volume branches" do
    ...
    before do
      allow(service).to receive(:log_with_id)
      allow(described_class).to receive(:backing_file_path).with(volume_id).and_return(backing_file)
    ...
    describe "filesystem creation logic" do
      ...
      it "runs nothing when requested storage is already mounted and needs no mkfs" do
        ...
        expect(service.perform_node_stage_volume(req_id, pvc, req, nil)).to eq(Csi::V1::NodeStageVolumeResponse.new)
      end
  ...
  describe "#node_unstage_volume" do
    ...
    it "unstages a volume successfully when node is schedulable" do
      ...
      allow(service).to receive(:is_mounted?).with(req.staging_target_path).and_return(true)
  ...
  describe "#perform_node_unstage_volume" do
    let(:req_id) { "test-req-id" }
    let(:req) { Csi::V1::NodeUnstageVolumeRequest.new(staging_target_path: "/var/lib/kubelet/plugins/kubernetes.io/csi/pv/vol-test-123/globalmount") }
    let(:response) { Csi::V1::NodeUnstageVolumeResponse.new }
