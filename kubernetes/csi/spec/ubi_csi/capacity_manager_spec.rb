...
RSpec.describe Csi::CapacityManager do
  ...
  describe "#parse_quantity" do
    it "parses plain integers" do
      expect(manager.parse_quantity("3260544000")).to eq(3260544000)
      expect(manager.parse_quantity("0")).to eq(0)
    end

    it "parses decimal SI suffixes (kube-apiserver normalizes byte counts to these)" do
      expect(manager.parse_quantity("3260544k")).to eq(3260544 * 1000)
      expect(manager.parse_quantity("16106127360")).to eq(16106127360)
      expect(manager.parse_quantity("1M")).to eq(1_000_000)
      expect(manager.parse_quantity("2.5G")).to eq(2_500_000_000)
    end

    it "parses binary suffixes" do
      expect(manager.parse_quantity("15Gi")).to eq(15 * 1024**3)
      expect(manager.parse_quantity("1Ki")).to eq(1024)
    end

    it "treats nil and empty strings as zero" do
      expect(manager.parse_quantity(nil)).to eq(0)
      expect(manager.parse_quantity("")).to eq(0)
      expect(manager.parse_quantity("  ")).to eq(0)
    end

    it "raises on unrecognized formats" do
      expect { manager.parse_quantity("garbage") }.to raise_error(ArgumentError, /Unrecognized Kubernetes quantity/)
    end
  end
