...
RSpec.describe Csi::MeshConnectivityChecker do
  ...
  describe "#run_mtr_for_target" do
    ...
    it "runs mtr command and stores output" do
      ...
      expect(checker).to receive(:run_cmd).with("timeout", "15", "mtr", "-c2", "-rw", "10.0.0.2", req_id: "mtr-check").and_return([mtr_output, process_status])
    ...
    it "stores non-zero exit status" do
      ...
      expect(checker).to receive(:run_cmd).with("timeout", "15", "mtr", "-c2", "-rw", "10.0.0.2", req_id: "mtr-check").and_return(["error output", failed_status])
