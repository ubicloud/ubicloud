  ...
  hash_branch(:project_prefix, "detachable-volume") do |r|
    ...
    r.web do
      ...
      r.get "create" do
        ...
        unit_price = BillingRate.unit_price_from_resource_properties("VmStorage", "standard", @project.default_location)
    ...
    r.on DETACHABLE_VOLUME_NAME_OR_UBID do |dv_name, dv_ubid|
      filter = dv_name ? {Sequel[:detachable_volume][:name] => dv_name} : {Sequel[:detachable_volume][:id] => UBID.to_uuid(dv_ubid)}
      ...
      r.delete true do
        ...
        if api?
          ...
          flash["notice"] = "'#{dv.name}' will be deleted in a few seconds"
          r.redirect "#{@project.path}/detachable-volume"
        end
      end
