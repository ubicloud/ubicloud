  ...
  hash_branch(:project_location_prefix, "load-balancer") do |r|
    ...
    r.on LOAD_BALANCER_NAME_OR_UBID do |lb_name, lb_id|
      ...
      r.delete true do
        ...
        DB.transaction do
          lb.incr_destroy(request.get_header("HTTP_X_REQUEST_ID"))
      ...
      r.patch api? do
        ...
        DB.transaction do
          ...
          lb.incr_update_load_balancer(request.get_header("HTTP_X_REQUEST_ID"))
      ...
      r.rename lb, perm: "LoadBalancer:edit", serializer: Serializers::LoadBalancer, template_prefix: "networking/load_balancer" do
        lb.incr_rewrite_dns_records(request.get_header("HTTP_X_REQUEST_ID"))
        lb.incr_refresh_cert(request.get_header("HTTP_X_REQUEST_ID"))
      end
