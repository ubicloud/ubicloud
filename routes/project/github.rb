...
class Clover
  hash_branch(:project_prefix, "github") do |r|
    r.web do
      ...
      r.on "runner" do
        r.get true do
          @runners = @project.github_runners_dataset.eager(:vm, :strand)
            .exclude(Sequel[:github_runner][:id] => Strand.where(prog: "Vm::GithubRunner", label: ["destroy", "wait_vm_destroy"]).select(:id))
