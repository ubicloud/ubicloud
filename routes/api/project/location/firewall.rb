...
class CloverApi
  hash_branch(:project_location_prefix, "firewall") do |r|
    firewall_endpoint_helper = Routes::Common::FirewallHelper.new(app: self, request: r, user: current_account, location: @location, resource: nil)
    ...
    r.on NAME_OR_UBID do |firewall_name, firewall_id|
      if firewall_name
        r.post true do
          firewall_endpoint_helper.post(firewall_name)
        end
      ...
      handle_firewall_requests(current_account, @firewall, @location)
    end
