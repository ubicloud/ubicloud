...
class Prog::MachineImage::RegisterDistroImage < Prog::Base
  ...
  def self.assemble(machine_image, vm_host_id:, url:, sha256:)
    Strand.create(
      ...
      label: "start",
      stack: [{"subject_id" => machine_image.id, "vm_host_id" => vm_host_id, "url" => url, "sha256" => sha256}]
  ...
  def register_params
...

    {
      ...
      "secrets_id" => "register-distro-#{machine_image.ubid}",
      "s3_bucket" => machine_image.s3_bucket,
      "s3_prefix" => machine_image.s3_prefix,
      "s3_endpoint" => machine_image.s3_endpoint,
