...
class Prog::Storage::RotateKek < Prog::Base
  ...
  def self.assemble(vm_storage_volume_id)
    DB.transaction do
      ...
      Strand.create(prog: "Storage::RotateKek", label: "back_up_key", stack: [{"subject_id" => vm_storage_volume_id}])
    end
  end
  ...
  def host_tool(action, stdin)
    sshable.cmd("sudo host/bin/rotate-storage-kek :vm_name :disk_index :action",
