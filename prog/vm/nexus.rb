...
class Prog::Vm::Nexus < Prog::Base
  ...
  def self.assemble(public_key, project_id, name: nil, size: DEFAULT_SIZE,
    ...
    use_separate_management_nic: false, use_eip: true, management_nic_use_eip: true, remote_storage_server_id: nil,
    ...
    DB.transaction do
      ...
      if nic_id
        ...
        nic = Prog::Vnet::NicNexus.assemble(subnet.id, name: "#{name}-nic", exclude_availability_zones:, availability_zone:, use_eip:, create_network_interface: use_eip).subject
      end
      ...
      if use_separate_management_nic
        Prog::Vnet::NicNexus.assemble(
          subnet.id, name: "#{name}-mgmt-nic", exclude_availability_zones:, availability_zone:, is_management: true, use_eip: management_nic_use_eip,
