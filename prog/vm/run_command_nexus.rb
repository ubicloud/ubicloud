...
class Prog::Vm::RunCommandNexus < Prog::Base
  ...
  RETRYABLE_ERRORS = [Sshable::SshError, Aws::EC2::Errors::ServiceError, Google::Cloud::Error].freeze
  ...
  def self.assemble(vm_id:, command:)
    ...
    DB.transaction do
      ...
      # fetches will mostly be identical.
      rc = RunCommand.find_or_create(vm_id:, command:)
      rc.strand&.destroy
      rc.update(status: "created", output: nil, run_at: nil)
  ...
  label def start
    ...
    if attempts < MAX_ATTEMPTS
      Clog.emit("run command failed, retrying", Util.exception_to_hash(ex).merge(attempt: attempts))
