...
class Prog::Vm::GithubRunner < Prog::Base
  ...
  label def start
    ...
    updated_rows = github_runner.installation_dataset
      .where { (Sequel[:used_vcpus] + requested_vcpus) <= quota_vcpus }
      .update(used_vcpus: Sequel[:used_vcpus] + requested_vcpus)
    ...
    if updated_rows != 1
      ...
      github_runner.installation_dataset.update(used_vcpus: Sequel[:used_vcpus] + requested_vcpus)
    end
