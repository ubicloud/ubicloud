...
class Prog::Vnet::CertNexus < Prog::Base
  ...
  label def start
    ...
    cert.update(kid: account.kid, private_key: private_key.to_der, order_url: order.url)
  ...
  def acme_client
    ...
    Acme::Client.new(private_key: OpenSSL::PKey::EC.new(cert.private_key), directory: Config.acme_directory, kid: cert.kid) if cert.private_key
  end
