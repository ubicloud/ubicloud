...
class Prog::Vnet::Aws::VpcNexus < Prog::Base
  ...
  label def wait_vpc_created
    ...
    private_subnet.firewalls(eager: :firewall_rules).flat_map(&:firewall_rules).each do |firewall_rule|
      if firewall_rule.port_range.first == 22 && firewall_rule.port_range.last - 1 == 22
        ssh_covered_ip4 = true if firewall_rule.cidr.to_s == "0.0.0.0/0"
        ssh_covered_ip6 = true if firewall_rule.cidr.to_s == "::/0"
      end
      allow_ingress(security_group_response.group_id, firewall_rule.port_range.first, firewall_rule.port_range.last - 1, firewall_rule.cidr.to_s, firewall_rule.ip6?)
    end
