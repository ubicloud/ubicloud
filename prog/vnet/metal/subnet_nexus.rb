...
class Prog::Vnet::Metal::SubnetNexus < Prog::Base
  ...
  # TLA   ∧ pc[s] = "idle"
  label def wait
    fail "BUG: locks held while in wait (NoOrphanedLocks)" unless get_locked_nics_dataset.empty?
    ...
    when_refresh_keys_set? do
      ...
      unless connected_leader?
        # TLA   ∧ refreshNeeded' = [refreshNeeded EXCEPT ![s] = FALSE, ![ConnectedLeader(s)] = TRUE]
  ...
  label def refresh_keys
    fail "BUG: locks held at idle" unless get_locked_nics_dataset.empty?
    ...
    # TLA   ∧ refreshNeeded' = [refreshNeeded EXCEPT ![s] = FALSE]
  ...
  # TLA \* Signals released NICs' owners to wake competing coordinators.
  # TLA FinishRekey(s) ==
  ...
  label def wait_old_state_drop
    ...
    if nics.all? { |nic| nic.rekey_phase == "old_drop" }
      ...
      released = get_locked_nics_dataset.update(encryption_key: nil, rekey_payload: nil,
        rekey_coordinator_id: nil, rekey_phase: "idle")
      fail "BUG: released #{released} NICs but expected #{nics.count}" unless released == nics.count
      # TLA   ∧ refreshNeeded' = [t ∈ Subnets ↦
      # TLA       IF t ≠ s ∧ ∃ n ∈ heldLocks[s] : NicOwner[n] = t
      # TLA       THEN TRUE
      # TLA       ELSE refreshNeeded[t]]
      (nics.map(&:private_subnet_id).uniq - [private_subnet.id]).each do |ps_id|
        PrivateSubnet[ps_id].incr_refresh_keys
      end
      # TLA   ∧ pc' = [pc EXCEPT ![s] = "idle"]
      # TLA   ∧ UNCHANGED ⟨edges, ops, activeNics⟩
      # TLA
      hop_wait
    end
  ...
  # TLA \* Destroy: remove all edges of s (must be idle with no locks).
  # TLA Destroy(s) ==
  # TLA   ∧ pc[s] = "idle"
  # TLA   ∧ heldLocks[s] = {}
  # TLA   ∧ ops < MaxOps
  label def destroy
    fail "BUG: locks held at destroy" unless get_locked_nics_dataset.empty?
    ...
    if private_subnet.nics.empty? && private_subnet.load_balancers.empty?
      # TLA   ∧ ops' = ops + 1
      # TLA   ∧ UNCHANGED ⟨pc, heldLocks, nicPhase, activeNics, refreshNeeded⟩
      # TLA
  ...
  def nics_to_rekey
    nics_with_state(%w[active creating])
  end
