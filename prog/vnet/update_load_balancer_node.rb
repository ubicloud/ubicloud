...
class Prog::Vnet::UpdateLoadBalancerNode < Prog::Base
  ...
  def generate_lb_based_nat_rules
    ...
    active_vm_ports_uniq_by_port = load_balancer.active_vm_ports { |ds| ds.eager(:load_balancer_port, load_balancer_vm: :vm) }
...
TEMPLATE
  ...
  def generate_lb_map_defs(current_port)
    ...
    items.sort_by { |address, port| [address, port] }.map.with_index do |(address, port), index|
      "#{index} : #{address} . #{port}"
    end
  end
