...
class Prog::Vnet::Gcp::UpdateFirewallRules < Prog::Base
  ...
  label def update_firewall_rules
    ...
    # Bind subnet tag for egress (rules already created by subnet_nexus)
    ...
    # Clean up legacy rules and tag artifacts from previous implementations
    cleanup_old_ip_based_rules
    cleanup_old_per_vm_tag_artifacts
  ...
  def ensure_firewall_tag_key(firewall)
    ...
    completed = wait_for_crm_operation(op)
    name = completed.response&.dig("name")
    return name if name

    lookup_tag_key_name(short_name) or
  ...
  def ensure_tag_value(tag_key_name, short_name)
    ...
    lookup_tag_value_name(tag_key_name, short_name) or
  ...
  def sync_tag_bindings(desired_tag_values)
    ...
    stale_bindings.each do |binding|
      ...
      wait_for_regional_crm_operation(op)
    rescue Google::Apis::ClientError => e
  ...
  def create_tag_binding(parent_resource, tag_value_name)
    ...
    op = begin
      ...
      # Transient 400 from GCP eventual consistency (tag key/value not
      # yet propagated to regional endpoint). Retry with backoff.
      retries_on_400 += 1
      if retries_on_400 <= 3
        Clog.emit("Transient 400 in create_tag_binding, retrying",
          {gcp_tag_binding_400: {tag_value: tag_value_name, retry: retries_on_400}})
        sleep retries_on_400
        retry
      end
      raise
    end
  ...
  def delete_tag_value(tag_value_name)
    op = credential.crm_client.delete_tag_value(tag_value_name)
    wait_for_crm_operation(op)
  rescue Google::Apis::ClientError => e
    raise unless e.status_code == 404
  rescue RuntimeError => e
    raise unless e.message.include?("NOT_FOUND")
  ...
  def delete_tag_key(tag_key_name)
    op = credential.crm_client.delete_tag_key(tag_key_name)
    wait_for_crm_operation(op)
  rescue Google::Apis::ClientError => e
    raise unless e.status_code == 404
  rescue RuntimeError => e
  ...
  # --- Old IP-based rule cleanup (transition period) ---

  def cleanup_old_ip_based_rules
    ...
    # Best effort cleanup during transition
  end

  # --- Old per-VM tag artifact cleanup ---
  # The previous implementation (never deployed) used a single per-VPC tag key
  # (ubicloud-fw-{project.ubid}) with per-VM tag values (vm-{vm.ubid}).
  # Clean up any leftover rules and tag values from that approach.

  def cleanup_old_per_vm_tag_artifacts
    vpc_tag_key_short = "ubicloud-fw-#{vm.nic.private_subnet.project.ubid}"
    vpc_tag_key_name = lookup_tag_key_name(vpc_tag_key_short)
    return unless vpc_tag_key_name

    vm_tag_short = "vm-#{vm.ubid}"
    vm_tag_value_name = lookup_tag_value_name(vpc_tag_key_name, vm_tag_short)
    return unless vm_tag_value_name

    # Delete policy rules targeting the old per-VM tag value
    policy = credential.network_firewall_policies_client.get(
      project: gcp_project_id,
      firewall_policy: firewall_policy_name
    )
  ...
  rescue Google::Cloud::Error, Google::Apis::ClientError, RuntimeError
    # Best effort cleanup — RuntimeError covers CRM LRO failures
  end
  ...
  # --- CRM operation helpers ---

  def wait_for_crm_operation(operation)
    30.times do
      if operation.done?
        raise "CRM operation #{operation.name} failed: #{operation.error.message}" if operation.error
        return operation
      end
      sleep 1
      operation = credential.crm_client.get_operation(operation.name)
    end
    raise "CRM operation #{operation.name} did not complete within timeout"
  end

  def wait_for_regional_crm_operation(operation)
    30.times do
      if operation.done?
        raise "CRM operation #{operation.name} failed: #{operation.error.message}" if operation.error
        return operation
      ...
      sleep 1
      operation = regional_crm_client.get_operation(operation.name)
    end
    raise "CRM operation #{operation.name} did not complete within timeout"
  end
  ...
  def gcp_vpc_name
    @gcp_vpc_name ||= Prog::Vnet::Gcp::SubnetNexus.vpc_name(vm.nic.private_subnet.project)
  end
