...
class Prog::Vnet::Gcp::UpdateFirewallRules < Prog::Base
  ...
  # Tag targeting decides which VMs evaluate a rule, but GCP rejects two rules at
  # the same priority within a policy, so priorities are globally unique. Each rule
  # gets the next free slot starting from TAG_RULE_BASE_PRIORITY, read from the
  # live policy. Priorities are not stored in the DB.
