...
class Prog::Vnet::NicNexus < Prog::Base
  ...
  def self.assemble(private_subnet_id, name: nil, ipv6_addr: nil, ipv4_addr: nil, exclude_availability_zones: [], availability_zone: nil)
    ...
    DB.transaction do
      ...
      Strand.create_with_id(id, prog:, label: "start", stack: [{"exclude_availability_zones" => exclude_availability_zones, "availability_zone" => availability_zone, "ipv4_addr" => ipv4_addr, "aws_subnet_id" => aws_subnet_id}])
    end
  end
  ...
  def self.select_aws_subnet(subnet, availability_zone, exclude_availability_zones)
    ...
    return nil unless ps_aws_resource
    ...
    if availability_zone
      location_aws_az = subnet.location.location_aws_azs_dataset.where(az: availability_zone).first
      if location_aws_az
        aws_subnet = AwsSubnet.where(
