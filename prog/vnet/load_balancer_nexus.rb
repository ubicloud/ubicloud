...
class Prog::Vnet::LoadBalancerNexus < Prog::Base
  ...
  def self.assemble_with_multiple_ports(private_subnet_id, name: nil, algorithm: "round_robin",
    health_check_endpoint: DEFAULT_HEALTH_CHECK_ENDPOINT, health_check_interval: 30, health_check_timeout: 15,
    health_check_up_threshold: 3, health_check_down_threshold: 2, health_check_protocol: "http", load_balancer_ports: nil,
    ...
    ports = Validation.validate_load_balancer_ports(load_balancer_ports)
    ...
    DB.transaction do
      ...
      ports.map { |port| LoadBalancerPort.create(load_balancer_id: lb.id, src_port: port["src_port"], dst_port: port["dst_port"]) }
  ...
  def self.assemble(private_subnet_id, name: nil, algorithm: "round_robin",
    ...
    load_balancer_ports = [{"src_port" => src_port, "dst_port" => dst_port}]
    assemble_with_multiple_ports(private_subnet_id, name:, algorithm:, health_check_endpoint:, health_check_interval:, health_check_timeout:,
      health_check_up_threshold:, health_check_down_threshold:, health_check_protocol:, load_balancer_ports:, custom_hostname_prefix:, custom_hostname_dns_zone_id:, stack:)
  end
