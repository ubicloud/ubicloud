...
class Prog::Vnet::SubnetNexus < Prog::Base
  ...
  def self.random_private_ipv4(location, project)
    ...
    selected_addr = random_private_ipv4(location, project) if project.private_subnets_dataset[Sequel[:net4] => selected_addr.to_s, :location => location] || banned_subnets.any? { _1.rel(selected_addr) }
