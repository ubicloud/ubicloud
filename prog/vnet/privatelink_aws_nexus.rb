...
class Prog::Vnet::PrivatelinkAwsNexus < Prog::Base
  ...
  label def register_initial_targets
    ...
    eager_vm_ports(state: "registering").each do |vm_port|
      vm = vm_port.vm
  ...
  label def update_targets
    ...
    eager_vm_ports(state: "deregistering").each do |vm_port|
      vm = vm_port.vm
      nic = privatelink_aws_resource.get_vm_nic(vm)
      next unless nic
  ...
  label def destroy
    ...
    # Delete and wait for VPC Endpoint Service to be fully gone
    if privatelink_aws_resource.service_id
      ...
      begin
        ...
        # Service is deleted, proceed
      end
    end

    # Delete listeners and target groups for each port
    privatelink_aws_resource.ports_dataset.eager(vm_ports: {privatelink_aws_vm: {vm: :nics}}).all.each do |port|
      # Delete listener
      if port.listener_arn
        begin
          elb_client.delete_listener(listener_arn: port.listener_arn)
  ...
  def eager_vm_ports(state:)
    PrivatelinkAwsVmPort
      .where(
        privatelink_aws_vm_id: privatelink_aws_resource.privatelink_aws_vms_dataset.select(:id),
        state: state
      )
      .eager(:privatelink_aws_port, privatelink_aws_vm: {vm: :nics})
      .all
  end
end
