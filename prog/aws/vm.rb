...
usermod -L ubuntu
    ...
    instance_response = client.run_instances({
      image_id: "ami-030c060f85668b37d", # AMI ID
  ...
  label def wait_instance_created
    instance_response = client.describe_instances({filters: [{name: "instance-id", values: [vm.name]}]}).reservations[0].instances[0]
