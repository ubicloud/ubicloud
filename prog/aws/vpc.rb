...
class Prog::Aws::Vpc < Prog::Base
  ...
  label def wait_vpc_created
    ...
    if vpc.state == "available"
      ...
      private_subnet.firewalls.flat_map(&:firewall_rules).each do |firewall_rule|
        ...
        allow_ingress(security_group_response.group_id, firewall_rule.protocol, firewall_rule.port_range.first, firewall_rule.port_range.last - 1, firewall_rule.cidr.to_s)
      end

      # Allow SSH ingress from the internet so that the controlplane can verify
      # that the VM is running.
      allow_ingress(security_group_response.group_id, "tcp", 22, 22, "0.0.0.0/0")
  ...
  def allow_ingress(group_id, protocol, from_port, to_port, cidr)
