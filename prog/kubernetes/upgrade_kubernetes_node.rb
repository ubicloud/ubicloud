...
class Prog::Kubernetes::UpgradeKubernetesNode < Prog::Base
  ...
  label def drain_old_node
    ...
    node = kubernetes_cluster.nodes.last
    case node.sshable.d_check("drain_node")
    when "Succeeded"
      hop_remove_old_node_from_cluster
    when "NotStarted"
      node.sshable.d_run("drain_node", "sudo", "kubectl", "--kubeconfig=/etc/kubernetes/admin.conf",
        "drain", old_node.name, "--ignore-daemonsets", "--delete-emptydir-data")
      nap 10
    when "InProgress"
      nap 10
    when "Failed"
      node.sshable.d_restart("drain_node")
      nap 10
    end
    nap 60 * 60
  end
  ...
  label def remove_old_node_from_cluster
    ...
    old_node.sshable.cmd("sudo kubeadm reset --force")
