...
#{dz.name}.          3600    NS      #{ds.name}.
  ...
  label def configure
    pop "vm retired" if ds.vms_dataset.where(id: subject_id).empty?
    ...
    # Independent zone queues can configure the same authoritative servers.
    ds.dns_zones_dataset.order(:id).select_map(:id).each do |zone_id|
      lock_key = Digest::SHA2.digest(zone_id).unpack1("q>").abs
      nap 5 unless DB.get(Sequel.function(:pg_try_advisory_xact_lock, lock_key))
    end
