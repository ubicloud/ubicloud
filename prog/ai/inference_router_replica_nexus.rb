...
class Prog::Ai::InferenceRouterReplicaNexus < Prog::Base
  ...
  def ping_inference_router
    ...
    targets = InferenceRouterTarget.order(:created_at).exclude(weight: 0).all.group_by(&:model_name)
    routes = targets.map do |model_name, targets_for_model|
      model = Option.ai_model_for_name(model_name)
      {
        model_name: model_name,
        project_rps_limit: model["max_project_rps"],
        project_tps_limit: model["max_project_tps"],
        endpoints: targets_for_model.map do |target|
          target.values.slice(:host, :inflight_limit, :weight, :tags, :inference_router_id).merge(
    ...
    new_config = {
      basic: {},
      certificates: [{
        tls_cert: inference_router.load_balancer.active_cert.cert,
        tls_key: OpenSSL::PKey.read(inference_router.load_balancer.active_cert.csr_key).to_pem,
        is_default: true,
        is_ca: false
      }],
      servers: [{
      ...
      locations: [
        {name: "health", path: "^/health$", app: "health"},
    ...
    if current_md5 != new_md5
      vm.sshable.cmd("mkdir -p /ir/workdir && sudo tee #{config_path} > /dev/null", stdin: new_config_json)
      Clog.emit("Configuration updated successfully.")
    end
    usage_response = vm.sshable.cmd(
      "sudo curl -m 5 -s -H \"Content-Type: application/json http://localhost:8080/usage"
    )
    project_usage = JSON.parse(usage_response)["projects"]
