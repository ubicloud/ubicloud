...
class Prog::Ai::InferenceRouterNexus < Prog::Base
  ...
  def self.assemble(project_id:, location_id:, name: "api", replica_count: 1)
    ...
    DB.transaction do
      ...
      inference_router = InferenceRouter.create(
        project_id:, location_id:, name:, replica_count:, load_balancer_id: lb_s.id, private_subnet_id: subnet_s.id
      ) { _1.id = ubid.to_uuid }
