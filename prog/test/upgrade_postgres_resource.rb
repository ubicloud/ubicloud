...
class Prog::Test::UpgradePostgresResource < Prog::Test::Base
  ...
  label def trigger_upgrade
    ...
    Clog.emit("Postgres servers before upgrade: #{postgres_resource.servers.map { |s| "[#{s.ubid}, version: #{s.version}, #{s.timeline_access}, #{s.strand.label}]" }.join(", ")}")
    Clog.emit("Read replica servers before upgrade: #{read_replica.servers.map { |s| "[#{s.ubid}, version: #{s.version}, #{s.timeline_access}, #{s.strand.label}]" }.join(", ")}")

    # Trigger upgrade by updating target_version
    DB.transaction do
  ...
  label def wait_upgrade
    ...
    # This involves the converge process upgrading standby, then promoting it
    upgrade_wait_started = frame["upgrade_wait_started"]
    update_stack({"upgrade_wait_started" => true})

    # Wait up to 20 minutes for upgrade to complete
    nap 1200 unless upgrade_wait_started
