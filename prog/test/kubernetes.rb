...
class Prog::Test::Kubernetes < Prog::Test::Base
  ...
  label def test_nodes
    ...
    hop_patch_csi_disk_limit
  end

  label def patch_csi_disk_limit
    nap 10 unless kubernetes_cluster.strand.label == "wait"
    kubernetes_cluster.client.kubectl("-n ubicsi patch deployment ubicsi-provisioner --type='json' -p='[{\"op\": \"add\", \"path\": \"/spec/template/spec/containers/0/env/2\", \"value\": {\"name\": \"DISK_LIMIT_GB\", \"value\": \"40\"}}]'")
    hop_test_csi
  end
...
STS
  ...
  label def test_data_write
    (1..3).each do |i|
      ...
      kubernetes_cluster.sshable.d_run(
        unit_name,
        "bash", "-c",
        "sudo kubectl --kubeconfig /etc/kubernetes/admin.conf exec -t ubuntu-statefulset-0 -- sh -c \"head -c 10G /dev/urandom | tee /etc/data/random-data-#{i} | sha256sum | awk '{print \\$1}'\" > /dev/shm/#{unit_name}.hash"
      )
    end
    hop_wait_data_write
  end
  ...
  def verify_mount
    ...
    if data_mount
      ...
      if device_name.start_with?("loop") && size == "35G" && mountpoint == "/etc/data"
        # no op
      else
