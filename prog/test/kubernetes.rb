...
STS
  ...
  label def test_data_write
    kubernetes_cluster.client.kubectl("exec -t ubuntu-statefulset-0 -- sh -c \"head -c 200M /dev/urandom > /etc/data/random-data\"")
    file_size_output = kubernetes_cluster.client.kubectl("exec -t ubuntu-statefulset-0 -- du -sh /etc/data/random-data").strip
    expected_file_size = "200M	/etc/data/random-data"
    if file_size_output != expected_file_size
      update_stack({"fail_message" => "wrong file size, expected: #{expected_file_size}, got: #{file_size_output}"})
    end
    hop_destroy_kubernetes
  end
