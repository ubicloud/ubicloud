...
class Prog::Test::UnarchivePostgresResource < Prog::Test::PostgresBase
  ...
  label def wait_backup
    ...
    when "Succeeded"
      ...
      # recovery.signal replays to end of archived WAL, so test data must
      # leave the live tail segment before destroy.
  ...
  label def wait_wal_archive
    if PostgresTimeline.any_archived_wal?(postgres_resource.timeline)
