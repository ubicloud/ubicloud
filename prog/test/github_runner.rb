...
class Prog::Test::GithubRunner < Prog::Test::Base
  ...
  label def trigger_test_run
    ...
    puts "===== #{inputs.inspect}"
    unless client.workflow_dispatch(REPOSITORY_NAME, WORKFLOW_NAME, BRANCH_NAME, {inputs:})
  ...
  label def check_test_run
    ...
    if (run = runs.find { it[:created_at] >= Time.parse(frame["created_at"]) && it[:name].include?(ENV["GITHUB_RUN_ID"]) && it[:name].include?(frame["provider"]) })
