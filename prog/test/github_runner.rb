...
class Prog::Test::GithubRunner < Prog::Test::Base
  ...
  label def enable_alien_runners
    ...
    project.set_ff_aws_alien_runners(true)
    location = Location.create(name: "eu-central-1", provider: "aws", project_id: project.id, display_name: "aws-e2e", ui_name: "aws-e2e", visible: true) { it.id = Config.github_runner_aws_location_id }
