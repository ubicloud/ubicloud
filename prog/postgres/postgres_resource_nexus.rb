...
class Prog::Postgres::PostgresResourceNexus < Prog::Base
  ...
  def self.assemble(project_id:, location_id:, name:, target_vm_size:, target_storage_size_gib:,
    desired_version:, flavor: PostgresResource::Flavor::STANDARD,
    ...
    DB.transaction do
      superuser_password, timeline_id, timeline_access, desired_version = if parent_id.nil?
        [SecureRandom.urlsafe_base64(15), Prog::Postgres::PostgresTimelineNexus.assemble(location_id: location.id).id, "push", desired_version]
      else
        unless (parent = PostgresResource[parent_id])
          fail "No existing parent"
        end

        if desired_version && desired_version != parent.version
  ...
  label def wait
    ...
    if postgres_resource.needs_upgrade? && strand.children_dataset.where(prog: "Postgres::UpgradePostgresResource").empty?
      candidate_server_id = Prog::Postgres::PostgresServerNexus.assemble(
        resource_id: postgres_resource.id,
      ...
      bud Prog::Postgres::UpgradePostgresResource, {"candidate_server_id" => candidate_server_id, "new_timeline_id" => new_timeline_id}, :start
    end
