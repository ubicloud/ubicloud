...
class Prog::Postgres::PostgresResourceNexus < Prog::Base
  ...
  def self.unarchive(postgres_resource_id)
    ...
    # Archives omit provider configuration. Older records omit storage_type and
    # therefore used local storage.
    storage_type = v["storage_type"] || PostgresResource::StorageType::LOCAL_NVME
    fail "Cannot unarchive network-backed resource #{postgres_resource_id}" if storage_type != PostgresResource::StorageType::LOCAL_NVME
