...
class Prog::Postgres::PostgresResourceNexus < Prog::Base
  ...
  def self.assemble(project_id:, location_id:, name:, target_vm_size:, target_storage_size_gib:,
    ...
    ha_type: PostgresResource::HaType::NONE, parent_id: nil, restore_target: nil, with_firewall_rules: true, user_config: {})
    ...
    DB.transaction do
      ...
      postgres_resource = PostgresResource.create(
        ...
        superuser_password:, ha_type:, target_version:, flavor:,
        parent_id:, restore_target:, hostname_version: "v2", user_config:
      )
