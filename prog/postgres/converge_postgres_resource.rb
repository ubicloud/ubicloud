...
class Prog::Postgres::ConvergePostgresResource < Prog::Base
  ...
  label def provision_servers
    ...
    if postgres_resource.servers.all? { it.vm.vm_host } || postgres_resource.location.aws?
      ...
      Prog::Postgres::PostgresServerNexus.assemble(resource_id: postgres_resource.id, timeline_id: timeline_id, timeline_access: "fetch", exclude_host_ids: exclude_host_ids, exclude_availability_zones: exclude_availability_zones, availability_zone: availability_zone)
    end
