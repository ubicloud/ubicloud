...
class Prog::Postgres::ConvergePostgresResource < Prog::Base
  ...
  label def provision_servers
    ...
    if postgres_resource.servers.all? { it.vm.vm_host } || postgres_resource.location.aws?
      ...
      if postgres_resource.location.provider == HostProvider::AWS_PROVIDER_NAME && postgres_resource.use_different_az_set?
        exclude_availability_zones = postgres_resource.servers.map { it.vm.nic.nic_aws_resource.subnet_az }.uniq
      elsif postgres_resource.location.provider == HostProvider::AWS_PROVIDER_NAME
        preferred_az = postgres_resource.representative_server.vm.nic.nic_aws_resource.subnet_az
      end

      Prog::Postgres::PostgresServerNexus.assemble(resource_id: postgres_resource.id, timeline_id: postgres_resource.timeline.id, timeline_access: "fetch", exclude_host_ids: exclude_host_ids, exclude_availability_zones: exclude_availability_zones, preferred_az: preferred_az)
    end
