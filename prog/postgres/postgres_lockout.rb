...
class Prog::Postgres::PostgresLockout < Prog::Base
  ...
  def lockout_with_pg_stop
    postgres_server.vm.sshable.cmd(
      "timeout 1 sudo pg_ctlcluster :version main stop -m immediate",
  ...
  def lockout_with_hba
    postgres_server.vm.sshable.cmd(
      "timeout 1 sudo postgres/bin/lockout-hba :version",
      version: postgres_server.version,
      timeout: 2
    )
  end
  ...
  def lockout_with_host_routing
    postgres_server.vm.vm_host.sshable.cmd(
      "timeout 1 sudo ip link set :interface down", interface: "vetho#{postgres_server.vm.inhost_name}",
      timeout: 2
    )
  end
end
