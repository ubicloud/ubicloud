...
class Prog::Postgres::UpgradePostgresResource < Prog::Base
  ...
  label def upgrade_standby
    ...
    when "Failed"
      hop_upgrade_failed
    when "NotStarted"
      candidate_server.vm.sshable.d_run("upgrade_postgres", "sudo", "postgres/bin/upgrade", postgres_resource.version)
    end
  ...
  label def update_metadata
    candidate_server.update(version: postgres_resource.version, timeline_id: new_timeline.id)
  ...
  label def prune_old_servers
    postgres_resource.servers.reject { it.version == postgres_resource.version }.each { it.incr_destroy }
    hop_finish_upgrade
  end
