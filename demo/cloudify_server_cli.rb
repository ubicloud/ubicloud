...
module CloudifyServer
  module_function

  HetznerApiConfig = Struct.new(:connection_string, :user, :password, keyword_init: true)
  ...
  def visible_locations
    Location.where(visible: true)
  end

  def location_label(location)
    "#{location.provider} #{location.name} (#{location.display_name})"
  end

  def locations
    @locations ||= visible_locations.map { |location| [location, location_label(location)] }.to_h
  end
  ...
  def select_option(msg, options, default = nil)
    ...
    while selected.nil?
      ...
      option = if option.empty?
        ...
        puts "Please enter a number between 1-#{options.count}#{" or leave empty for default" unless default.nil?}"
      else
        option.to_i
      end
  ...
  def required_env_vars_configured?
    REQUIRED_ENV_VARS.all? { Config.public_send(it.downcase) }
  end

  def hetzner_api
    Hosting::HetznerApis.new(
      HetznerApiConfig.new(
        connection_string: Config.hetzner_connection_string,
  ...
  def find_hetzner_server_identifier(hostname)
    hetzner_api.find_server_identifier(hostname)
  end

  def vm_host_dataset(hostname, host_id, location)
    VmHost
      .where(id: Sshable.where(host: hostname).select(:id))
      .where(id: HostProvider.where(server_identifier: host_id, provider_name: location.provider).select(:id))
      .where(location_id: location.id)
  end

  def get_host_identifier(hostname, location)
    return get_input("Enter host identifier") unless location.provider == HostProvider::HETZNER_PROVIDER_NAME

    if (host_id = find_hetzner_server_identifier(hostname))
      puts "Found Hetzner server identifier '#{host_id}' for '#{hostname}'"
      host_id
    else
      puts "Couldn't find the Hetzner server identifier automatically for '#{hostname}'."
      get_input("Enter host identifier")
    end
  end

  def run
    unless required_env_vars_configured?
    ...
    host_id = get_host_identifier(hostname, location)
    ...
    vm_host_ds = vm_host_dataset(hostname, host_id, location)
