...
Sequel.migration do
  up do
    create_table(:load_balancer_port) do
      ...
      check "src_port >= 1 AND src_port <= 65535", name: :src_port_range
      check "dst_port >= 1 AND dst_port <= 65535", name: :dst_port_range
    end
    ...
    DB[:load_balancer_port].import(
      ...
      DB[:load_balancer].select(
        UBID.generate(TYPE_LOAD_BALANCER_PORT).to_uuid,
        :id,
        :src_port,
        :dst_port
      )
    ...
    DB[:load_balancers_vms]
      ...
      DB[:load_balancer_vm_port].import(
        id: UBID.generate(TYPE_LOAD_BALANCER_VM_PORT).to_uuid,
        load_balancer_port_id: row[:load_balancer_port_id],
        load_balancer_vm_id: row[:load_balancer_vm_id],
        state: row[:state]
      )
    end
  end
