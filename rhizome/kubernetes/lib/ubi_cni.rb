...
options ndots:5
  ...
  def allocate_ips_for_pod(container_id, subnet_ula_ipv6, subnet_ipv6, subnet_ipv4)
    atomic_file_replace(IPAM_STORE_FILE) do |old_file, new_file|
      ipam_store = if old_file
      ...
      new_file.write(JSON.pretty_generate(ipam_store))
      [ipv4_container_ip, ipv4_gateway_ip, container_ula_ipv6, container_ipv6]
    end
  end
  ...
  def setup_ipv6(container_ip, inner_link_local, outer_link_local, cni_netns, inner_ifname, outer_ifname, setup_default_route: false)
    ...
    r "ip -6 -n #{cni_netns} route replace default via #{outer_link_local} dev #{inner_ifname}" if setup_default_route
