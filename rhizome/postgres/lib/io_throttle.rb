...
class IoThrottle
  ...
  def run
    ...
    @logger.info("Archival backlog: #{backlog} files, throttle: #{throttle_mbps} MB/s")
    apply(throttle_mbps)
  end
  ...
  def enable_io_controller
    ...
    Dir.mkdir(@throttled_cgroup) unless File.directory?(@throttled_cgroup)
    Dir.mkdir(@immune_cgroup) unless File.directory?(@immune_cgroup)
