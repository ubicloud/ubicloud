...
class HugepagesSetup
  ...
  def get_postgres_param(param)
    Integer(r("sudo -u postgres /usr/lib/postgresql/#{@version}/bin/postgres -D /dat/#{@version}/data -c config_file=/etc/postgresql/#{@version}/#{@cluster}/postgresql.conf -C #{param.shellescape}"), 10)
  end
