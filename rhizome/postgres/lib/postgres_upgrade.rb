...
class PostgresUpgrade
  ...
  def run_check
    Dir.chdir("/dat/upgrade/#{@version}") do
      r pg_upgrade_cmdline("--check")
    end
  end

  def run_pg_upgrade
    Dir.chdir("/dat/upgrade/#{@version}") do
      r pg_upgrade_cmdline("--link")
    end
  end
  ...
  def pg_upgrade_cmdline(extra_args)
    "sudo -u postgres /usr/lib/postgresql/#{@version}/bin/pg_upgrade --old-bindir /usr/lib/postgresql/#{@prev_version}/bin --old-datadir /etc/postgresql/#{@prev_version}/main/ --new-bindir /usr/lib/postgresql/#{@version}/bin --new-datadir /etc/postgresql/#{@version}/main/ #{extra_args}"
  end
end
