...
# Loading util.rb later pulls in the no-op overrider.rb, which redefines
# Overrider.load; keep the enabled one for the in-process example.
enabled_load = Overrider.method(:load)
  ...
  describe ".load" do
    ...
    it "requires the mirrored override and ignores a file without one" do
      Dir.mktmpdir do |root|
        ...
        enabled_load.call(patched, root: root)
        expect(File.read(marker)).to eq "loaded"

        unpatched = "#{root}/postgres/lib/no_override.rb"
        File.write(unpatched, "")
        expect { enabled_load.call(unpatched, root: root) }.not_to raise_error
      end
    end
  end
end
