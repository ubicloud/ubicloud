...
RSpec.describe StorageVolume do
  ...
  describe "archive-backed volumes" do
    ...
    it "detach_archive removes stripe source config, secrets config, rewrites main config and service file (unencrypted)" do
      ...
      written_content = nil
      expect(archive_vhost_sv).to receive(:write_config_file).with(config_path, anything) { |_, c| written_content = c }
    ...
    it "detach_archive preserves encryption secrets for encrypted archive volumes" do
      ...
      config_written = nil
      expect(encrypted_archive_vhost_sv).to receive(:write_config_file).with(config_path, anything) { |_, c| config_written = c }
